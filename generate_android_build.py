...
option('platforms',type='array',value=['auto'],choices=['auto','x11','wayland','haiku','android','windows','macos',],description='window systems to support. If this is set to `auto`, all '+'platforms applicable will be enabled.')
...
option('android-libperfetto',type='feature',description='Use Android\'s libperfetto',)

option('dri-drivers-path',type='string',value='',description='Location to install dri drivers. Default: $libdir/dri.')

option('unversion-libgallium',type='boolean',value=False,description='Do not include mesa version in libgallium DSO filename. '+'Do not enable unless you know what you are doing. Default: false')

option('expat',type='feature',value='auto',description='Controls the use of expat. '+'Cannot be disabled if xmlconfig is enabled.')

option('gallium-drivers',type='array',value=['auto'],choices=['all','auto','asahi','crocus','d3d12','etnaviv','freedreno','i915','iris','lima','llvmpipe','nouveau','panfrost','r300','r600','radeonsi','rocket','softpipe','svga','tegra','v3d','vc4','virgl','zink',],description='List of gallium drivers to build. If this is set to auto '+'all drivers applicable to the target OS/architecture '+'will be built.')
...
option('gallium-mediafoundation',type='feature',deprecated={'true':'enabled','false':'disabled'},description='enable gallium mediafoundation frontend.',)

option('gallium-mediafoundation-test',type='boolean',value=False,description='enable gallium mediafoundation frontend tests.',)
...
option('gallium-rusticl-enable-drivers',type='array',value=['auto','asahi','freedreno','radeonsi'],description='List of gallium drivers for which rusticl will be enabled '+'by default',)
...
option('mediafoundation-store-dll',type='boolean',value=False,description='Selects whether the gallium mediafoundation DLL is built for the store. ',)

option('mediafoundation-codecs',type='array',value=['all'],choices=['all','h264enc','h265enc','av1enc'],description='List of codecs to build mediafoundation frontend DLLs for. '+'These will generate different MFT DLLs per codec and link'+'against the gallium drivers which uses the video-codecs option')
...
option('amdgpu-virtio',type='boolean',value=False,description='use experimental virtio backend for radeonsi/radv',)
...
option('vulkan-layers',type='array',value=[],choices=['device-select','intel-nullhw','overlay','screenshot','anti-lag','vram-report-limit',],description='List of vulkan layers to build')

option('shared-glapi',type='feature',deprecated=True,description='Does nothing, left here for a while to avoid build breakages.',)
...
option('libgbm-external',type='boolean',value=False,description='Whether to use external libgbm (default: use in-tree copy)')
...
option('llvm-orcjit',type='boolean',value=False,description='Build llvmpipe with LLVM ORCJIT support. Has no effect when '+'building for architectures without LLVM MCJIT support -- '+'ORCJIT is the only choice on such architectures and will '+'always be enabled.')
...
option('build-radv-tests',type='boolean',value=False,description='Build RADV tests. These do not require an AMD GPU.')
...
option('selinux',type='boolean',deprecated=True,description='Does nothing, left here for a while to avoid build breakages.',)
...
option('power8',type='feature',description='Does nothing, left here for a while to avoid build breakages.',deprecated=True,)
...
option('platform-sdk-version',type='integer',min=25,max=10000,value=34,description='Android Platform SDK version. Default: Android14 version.')
...
option('sysprof',type='boolean',value=False,description='Enable tracing markers for sysprof. Default: false')
...
option('intel-elk',type='boolean',value=True,description='Build ELK compiler (used for Gfx8 and earlier). '+'This is required for Crocus and Hasvk, and it is '+'optional for Iris and various developer tools.')
...
option('radeonsi-build-id',type='string',value='',description='Override build id for shader cache keys (hex string). '+'Can be extracted with readelf -x .note.gnu.build-id')
...
option('legacy-wayland',type='array',value=[],description='Build legacy Wayland support features.',choices=['bind-wayland-display',],)

option('legacy-x11',type='array',value=[],description='Build legacy X11 support features.',deprecated=True,choices=[],)

option('mesa-clc',type='combo',value='auto',choices=['enabled','system','auto'],description='Build the mesa-clc compiler or use a system version.')

option('install-mesa-clc',type='boolean',value=False,description='Install the mesa-clc compiler (if needed for cross builds).')

option('mesa-clc-bundle-headers',type='combo',value='auto',choices=['enabled','auto'],description='Bundle the OpenCL headers into the mesa-clc binary (default to bundle if static LLVM is used). Note, it might require rebuilding mesa-clc if opencl-c.h or opencl-c-base.h are changed (e.g. on Clang upgrades).')

option('precomp-compiler',type='combo',value='auto',choices=['enabled','system','auto'],description='Build drivers internal shader compilers or use a system version')

option('install-precomp-compiler',type='boolean',value=False,description='Install the drivers internal shader compilers (if needed for cross builds).')

option('allow-fallback-for',type='array',value=['perfetto'],choices=['libdrm','perfetto',],description='Allows the fallback mechanism if the dependency is not available on the system, or too old.')

option('virtgpu_kumquat',type='boolean',value=False,description='Build virtgpu_kumquat (only useful with gfxstream currently)')
...
host_machine = impl.Machine(meson_translator.host_machine, meson_translator.config.cpu, meson_translator.config.cpu_family)
build_machine = impl.Machine(meson_translator.build_machine, meson_translator.config.cpu, meson_translator.config.cpu_family)
...
########################################################################################################################
project('mesa',['c','cpp'],version=files('VERSION'),license='MIT',meson_version='>= 1.4.0',default_options=['buildtype=debugoptimized','b_ndebug=if-release','c_std=c11','cpp_std=c++17','rust_std=2021','build.rust_std=2021',],)
...
cl_args=['-fmacro-prefix-map={}/='.format(relative_dir),'-cl-std=cl2.0','-D__OPENCL_VERSION__=200','-DHAVE___BUILTIN_FFS','-DHAVE___BUILTIN_CLZ',]
...
pre_args=['-D__STDC_CONSTANT_MACROS','-D__STDC_FORMAT_MACROS','-D__STDC_LIMIT_MACROS','-DPACKAGE_VERSION="{}"'.format(meson.project_version()),'-DPACKAGE_BUGREPORT="https://gitlab.freedesktop.org/mesa/mesa/-/issues"',]
...
with_vulkan_screenshot_layer=get_option('vulkan-layers').count('screenshot') > 0
with_vulkan_vram_report_limit_layer=get_option('vulkan-layers').count('vram-report-limit') > 0
with_vulkan_anti_lag_layer=get_option('vulkan-layers').count('anti-lag') > 0
with_tools=get_option('tools')
if with_tools.count('all') > 0:
  with_tools=['asahi','dlclose-skip','drm-shim','etnaviv','freedreno','glsl','imagination','intel','intel-ui','lima','nir','nouveau','panfrost',]
...
if gallium_drivers.count('auto') > 0:
  if system_has_kms_drm:
    if ['x86','x86_64'].count(host_machine.cpu_family()) > 0:
      gallium_drivers=['r300','r600','radeonsi','nouveau','virgl','svga','llvmpipe','softpipe','iris','crocus','i915','zink']
    elif ['arm','aarch64'].count(host_machine.cpu_family()) > 0:
      gallium_drivers=['v3d','vc4','freedreno','etnaviv','nouveau','svga','tegra','virgl','lima','panfrost','llvmpipe','softpipe','iris','zink']
    elif ['mips','mips64','ppc','ppc64','riscv32','riscv64'].count(host_machine.cpu_family()) > 0:
      gallium_drivers=['r300','r600','radeonsi','nouveau','virgl','llvmpipe','softpipe','zink']
    elif ['loongarch64'].count(host_machine.cpu_family()) > 0:
      gallium_drivers=['r300','r600','radeonsi','nouveau','virgl','etnaviv','llvmpipe','softpipe','zink']
    else:
  ...
  gallium_drivers=['r300','r600','radeonsi','crocus','v3d','vc4','freedreno','etnaviv','i915','nouveau','svga','tegra','virgl','lima','panfrost','llvmpipe','softpipe','iris','zink','d3d12','asahi','rocket']
...
with_gallium_softpipe=gallium_drivers.count('softpipe') > 0
with_gallium_llvmpipe=gallium_drivers.count('llvmpipe') > 0
...
with_gallium_rocket=gallium_drivers.count('rocket') > 0
...
if with_gallium_swrast:
  pre_args += ['-DHAVE_SWRAST'] if isinstance(pre_args, list) else '-DHAVE_SWRAST'
...
if _vulkan_drivers.count('auto') > 0:
  ...
  _vulkan_drivers=['amd','intel','intel_hasvk','swrast','freedreno','panfrost','virtio','broadcom','imagination-experimental','microsoft-experimental','nouveau','asahi','gfxstream']
...
with_asahi_vk=_vulkan_drivers.count('asahi') > 0
with_gfxstream_vk=_vulkan_drivers.count('gfxstream') > 0
with_any_vk=len(_vulkan_drivers)!=0

with_llvm=with_llvm.enable_if(with_swrast_vk,error_message='Lavapipe Vulkan driver requires LLVM').enable_if(with_amd_vk and with_aco_tests,error_message='RadV ACO tests require LLVM')
...
with_vdrm=[with_amdgpu_virtio,freedreno_kmds.count('virtio') > 0,with_gallium_asahi,with_asahi_vk,].count(True) > 0
...
with_intel_vk_rt=get_option('intel-rt').disable_auto_if(not with_intel_vk).allowed()

with_any_intel=[with_gallium_crocus,with_gallium_i915,with_gallium_iris,with_intel_hasvk,with_intel_tools,with_intel_vk,].count(True) > 0
...
with_platform_xcb=_platforms.count('xcb') > 0
...
with_platform_macos=_platforms.count('macos') > 0
...
if not have_mtls_dialect:
  if meson.is_cross_build() and not meson.can_run_host_binaries():
    ...
    for tlsdesc_arg in ['-mtls-dialect=gnu2','-mtls-dialect=desc']:
      tlsdesc_test=cc.run('int __thread x; int main() { return x; }',args=[tlsdesc_arg,'-fpic'],name=tlsdesc_arg)
      if tlsdesc_test.returncode()==0 and (host_machine.cpu_family()!='x86_64' or cc.links('''int __thread x; int y; int main() { __asm__(
...
if with_gallium_d3d12 or with_microsoft_clc or with_microsoft_vk or with_gfxstream_vk and host_machine.system()=='windows':
  ...
  if not dep_dxheaders.found():
    dep_dxheaders=dependency('DirectX-Headers',version='>= 1.614.1',fallback=['DirectX-Headers','dep_dxheaders'],required=with_gallium_d3d12 or with_microsoft_vk)



if dep_dxheaders.found():
  if (dep_dxheaders.version().version_compare('>= 1.700.0')):
    pre_args += ['-DUSE_D3D12_PREVIEW_HEADERS=1'] if isinstance(pre_args, list) else '-DUSE_D3D12_PREVIEW_HEADERS=1'
  else:
    pre_args += ['-DUSE_D3D12_PREVIEW_HEADERS=0'] if isinstance(pre_args, list) else '-DUSE_D3D12_PREVIEW_HEADERS=0'
...
# Meson 1.4.0
prog_glslang=find_program('glslangValidator',native=True,required=with_vulkan_overlay_layer or with_aco_tests or with_amd_vk or with_intel_vk or with_swrast_vk or with_freedreno_vk)
...
_mediafoundation_drivers=[with_gallium_d3d12_video,]

with_gallium_mediafoundation=get_option('gallium-mediafoundation').require(host_machine.system()=='windows',error_message='mediafoundation only supported on Windows').require(_mediafoundation_drivers.count(True) > 0,error_message='Media foundation state tracker requires at least one of the following gallium drivers: d3d12 (with option gallium-d3d12-video).').enabled()
...
if with_gallium_rusticl or with_nouveau_vk or with_tools.count('etnaviv') > 0 or with_virtgpu_kumquat:
  ...
  if bindgen_version=='unknown' or bindgen_version.version_compare('< 0.71.1'):
    error('Mesa requires bindgen 0.71.1 or newer. '+'If your distribution does not ship a recent enough version, '+'you can install the latest version for your user with `cargo install bindgen-cli`.')


  bindgen_output_args=['--raw-line','#![allow(clippy::all)]','--raw-line','#![allow(improper_ctypes)]','--raw-line','#![allow(unused_unsafe)]','--raw-line','#![allow(non_camel_case_types)]','--raw-line','#![allow(non_snake_case)]','--raw-line','#![allow(non_upper_case_globals)]',]
...
with_driver_using_cl=[with_gallium_iris,with_intel_vk,with_gallium_asahi,with_asahi_vk,with_gallium_panfrost,with_panfrost_vk,with_nouveau_vk,].count(True) > 0
...
if get_option('mesa-clc')=='system':
  ...
  with_clc=get_option('mesa-clc')!='auto' or with_microsoft_clc or with_gallium_rusticl or with_drivers_clc or with_driver_using_cl


with_llvm=with_llvm.enable_if(with_clc,error_message='CLC requires LLVM')
...
if with_libbacktrace:
  cpp_args += ['-DWITH_LIBBACKTRACE'] if isinstance(cpp_args, list) else '-DWITH_LIBBACKTRACE'
...
_attributes=['const','flatten','malloc','pure','unused','warn_unused_result','weak','format','packed','returns_nonnull','alias','noreturn','optimize',]
...
if ['linux','cygwin','gnu','freebsd','gnu/kfreebsd','haiku','android','managarm'].count(host_machine.system()) > 0:
  ...
  if cc.get_argument_syntax()=='msvc':
    pre_args+=['-DVC_EXTRALEAN','-D_CRT_SECURE_NO_WARNINGS','-D_CRT_SECURE_NO_DEPRECATE','-D_SCL_SECURE_NO_WARNINGS','-D_SCL_SECURE_NO_DEPRECATE','-D_ALLOW_KEYWORD_MACROS','-D_HAS_EXCEPTIONS=0','-DNOMINMAX','-D_UCRT_NOISY_NAN',]
  else:
    if cc.compiles('''
      #include <string.h>
...
ld_args_gc_sections=[]
if cc.get_argument_syntax()=='msvc':
  _trial=['/wd4018','/wd4056','/wd4244','/wd4267','/wd4305','/wd4351','/wd4756','/wd4800','/wd4996','/wd4291','/wd4146','/wd4200','/wd4624','/wd4309','/wd4838','/wd5105','/we4020','/we4024','/we4189','/Zc:__cplusplus','/Zc:preprocessor',]
  ...
  _trial_cpp=['-Werror=return-type','-Werror=empty-body','-Wmisleading-indentation','-Wno-non-virtual-dtor','-Wno-missing-field-initializers','-Wno-format-truncation','-fno-math-errno','-fno-trapping-math','-Qunused-arguments','-flifetime-dse=1','-Wno-unknown-pragmas','-Wno-microsoft-enum-value',]
  ...
  no_override_init_args+=cc.get_supported_arguments(['-Wno-override-init','-Wno-initializer-overrides'])
...
if cc.compiles('''#include <stdint.h>
  ...
  pre_args += ['-DUSE_GCC_ATOMIC_BUILTINS'] if isinstance(pre_args, list) else '-DUSE_GCC_ATOMIC_BUILTINS'
...
with_expat=get_option('expat').disable_auto_if(with_platform_android or with_platform_windows).enable_if(with_intel_tools,error_message='Intel tools require expat')
...
use_xmlconfig=get_option('xmlconfig').require(not(with_platform_android or with_platform_windows),error_message='xmlconfig not available on Android or Windows').require(dep_expat.found(),error_message='requires expat').allowed()
...
_libdrm_checks=[['intel',with_gallium_i915],['amdgpu',(with_amd_vk and not with_platform_windows) or with_gallium_radeonsi],]
...
allow_fallback_for_libdrm=get_option('allow-fallback-for').count('libdrm') > 0

# Then get each libdrm module
for d in _libdrm_checks:
  if d[1]:
    set_variable('dep_libdrm_'+d[0],dependency('libdrm_'+d[0],version='>='+_drm_ver,allow_fallback=allow_fallback_for_libdrm,))



with_gallium_drisw_kms=False
if system_has_kms_drm:
  dep_libdrm=dependency('libdrm',version='>='+_drm_ver,required=with_dri2 or with_dri or with_gbm,allow_fallback=allow_fallback_for_libdrm,)
else:
  dep_libdrm=null_dep
...
dep_llvm=dependency('llvm',method='auto' if host_machine.system()=='windows' else 'config-tool',version=_llvm_version,modules=llvm_modules,optional_modules=llvm_optional_modules,required=with_llvm,static=not _shared_llvm,fallback=['llvm','dep_llvm'],include_type='system',)
if dep_llvm.found():
...
pre_args += ['-DLLVM_AVAILABLE={}'.format(int(dep_llvm.found()))] if isinstance(pre_args, list) else '-DLLVM_AVAILABLE={}'.format(int(dep_llvm.found()))
pre_args += ['-DDRAW_LLVM_AVAILABLE={}'.format(int((dep_llvm.found() and draw_with_llvm)))] if isinstance(pre_args, list) else '-DDRAW_LLVM_AVAILABLE={}'.format(int((dep_llvm.found() and draw_with_llvm)))
...
pre_args += ['-DGALLIVM_USE_ORCJIT={}'.format(int((dep_llvm.found() and llvm_with_orcjit)))] if isinstance(pre_args, list) else '-DGALLIVM_USE_ORCJIT={}'.format(int((dep_llvm.found() and llvm_with_orcjit)))

if with_clc:
  chosen_llvm_version_array=dep_llvm.version().split('.')
  chosen_llvm_version_major=int(chosen_llvm_version_array[0])
  chosen_llvm_version_minor=int(chosen_llvm_version_array[1])
...
dep_spirv_tools=dependency('SPIRV-Tools',required=with_clc,version='>= 2024.1')
if dep_spirv_tools.found():
  pre_args += ['-DHAVE_SPIRV_TOOLS'] if isinstance(pre_args, list) else '-DHAVE_SPIRV_TOOLS'
...
dep_lua=dependency('lua54','lua5.4','lua-5.4','lua53','lua5.3','lua-5.3','lua',required=False,allow_fallback=with_tools.count('freedreno') > 0,version='>=5.3')
...
if get_option('b_sanitize').count('thread') > 0:
  pre_args += ['-DTHREAD_SANITIZER=1'] if isinstance(pre_args, list) else '-DTHREAD_SANITIZER=1'
...
with_wayland_bind_display=with_platform_wayland and get_option('legacy-wayland').count('bind-wayland-display') > 0
...
if with_platform_x11:
  ...
  if (with_any_vk or with_glx=='dri' or with_egl or (with_gallium_vdpau or with_gallium_va)):
    dep_xcb=dependency('xcb')
  ...
  if with_dri_platform=='drm':
    ...
    pre_args += ['-DHAVE_X11_DRM'] if isinstance(pre_args, list) else '-DHAVE_X11_DRM'
...
allow_fallback_for_perfetto=get_option('allow-fallback-for').count('perfetto') > 0
with_perfetto=get_option('perfetto')
with_datasources=get_option('datasources')
with_any_datasource=len(with_datasources)!=0
if with_perfetto:
  dep_perfetto=dependency('perfetto',fallback=['perfetto','dep_perfetto'] if allow_fallback_for_perfetto else [],)
  pre_args += ['-DHAVE_PERFETTO'] if isinstance(pre_args, list) else '-DHAVE_PERFETTO'


if get_option('android-libperfetto').enabled():
  pre_args += ['-DANDROID_LIBPERFETTO'] if isinstance(pre_args, list) else '-DANDROID_LIBPERFETTO'
...
if with_sysprof:
  ...
  pre_args += ['-DHAVE_SYSPROF'] if isinstance(pre_args, list) else '-DHAVE_SYSPROF'
...
gcc_lto_quirk=['-fno-lto'] if (cc.get_id()=='gcc') else []
...
opencl_headers=files('CL/cl.h','CL/cl_d3d10.h','CL/cl_d3d11.h','CL/cl_dx9_media_sharing.h','CL/cl_dx9_media_sharing_intel.h','CL/cl_egl.h','CL/cl_ext.h','CL/cl_ext_intel.h','CL/cl_function_types.h','CL/cl_gl.h','CL/cl_gl_ext.h','CL/cl_half.h','CL/cl_icd.h','CL/cl_layer.h','CL/cl_platform.h','CL/cl_va_api_media_sharing_intel.h','CL/cl_version.h','CL/opencl.h',)
...
install_megadrivers=[install_megadrivers_py.full_path(),'--libname-suffix',libname_suffix,]
...
blake3=static_library('blake3',files_blake3,c_args=blake3_defs,gnu_symbol_visibility='hidden',build_by_default=False,)
...
files_mesa_util+=custom_target(input=['process_shader_stats.py','shader_stats.rnc','shader_stats.xml'],output='shader_stats.h',command=[prog_python,'@INPUT@'],capture=True,)
...
u_format_gen_h=custom_target('u_format_gen.h',input=['u_format_table.py','u_format.yaml'],output='u_format_gen.h',command=[prog_python,'@INPUT@','--enums'],depend_files=files('u_format_pack.py','u_format_parse.py'),capture=True,)

u_format_pack_h=custom_target('u_format_pack.h',input=['u_format_table.py','u_format.yaml'],output='u_format_pack.h',command=[prog_python,'@INPUT@','--header'],depend_files=files('u_format_pack.py','u_format_parse.py'),capture=True,)

u_format_table_c=custom_target('u_format_table.c',input=['u_format_table.py','u_format.yaml'],output='u_format_table.c',command=[prog_python,'@INPUT@'],depend_files=files('u_format_pack.py','u_format_parse.py'),capture=True,)
...
deps_for_libmesa_util=[dep_zlib,dep_clock,dep_thread,dep_atomic,dep_m,dep_valgrind,dep_zstd,dep_dl,dep_unwind,dep_futex,idep_blake3,idep_mesautilc11,idep_mesautilformat,]
...
if with_sysprof:
  files_mesa_util+=files('perf/u_sysprof.c','perf/u_sysprof.h',)
  deps_for_libmesa_util+=dep_sysprof


u_trace_py=files('perf/u_trace.py')

libmesa_util_simd=static_library('mesa_util_simd',files('streaming-load-memcpy.c'),c_args=[c_msvc_compat_args,sse41_args],include_directories=[inc_util],gnu_symbol_visibility='hidden',build_by_default=False,)

_libmesa_util=static_library('mesa_util',[files_mesa_util,files_debug_stack,format_srgb],include_directories=[inc_util,include_directories('format')],dependencies=deps_for_libmesa_util,link_with=[libmesa_util_simd],c_args=[c_msvc_compat_args],gnu_symbol_visibility='hidden',build_by_default=False)
...
if with_tests:
  ...
  files_util_tests=files('tests/bitset_test.cpp','tests/blob_test.cpp','tests/dag_test.cpp','tests/fast_idiv_by_const_test.cpp','tests/fast_urem_by_const_test.cpp','tests/float8_test.cpp','tests/gc_alloc_tests.cpp','tests/half_float_test.cpp','tests/int_min_max.cpp','tests/linear_test.cpp','tests/mesa-sha1_test.cpp','tests/os_mman_test.cpp','tests/perf/u_trace_test.cpp','tests/rb_tree_test.cpp','tests/register_allocate_test.cpp','tests/roundeven_test.cpp','tests/set_test.cpp','tests/string_buffer_test.cpp','tests/timespec_test.cpp','tests/u_atomic_test.cpp','tests/u_call_once_test.cpp','tests/u_debug_stack_test.cpp','tests/u_debug_test.cpp','tests/u_memstream_test.cpp','tests/u_printf_test.cpp','tests/u_qsort_test.cpp','tests/vector_test.cpp',)
  ...
  test('util_tests',executable('util_tests',files_util_tests,dependencies=[idep_mesautil,idep_gtest],),suite=['util'],protocol='gtest',is_parallel=False,timeout=180,)
...
if with_platform_x11:
  set_relative_dir('src/x11')
  ########################################################################################################################
  ### Begin conversion from: src/x11/meson.build
  ########################################################################################################################
  inc_loader_x11=include_directories('.')
  
  loader_x11_files=files('x11_display.c',)
  
  if with_dri_platform=='drm':
    loader_x11_files+=files('x11_dri3.c')
  
  
  libloader_x11=static_library('loader_x11',loader_x11_files,gnu_symbol_visibility='hidden',include_directories=[inc_include,inc_src,inc_gallium],dependencies=[idep_mesautil,dep_libdrm,dep_xcb_dri3,dep_xcb_present,dep_xcb_sync,dep_xshmfence,dep_xcb_xfixes,],build_by_default=False,)
  
  ########################################################################################################################
  ### End conversion from: src/x11/meson.build
  ########################################################################################################################
  set_relative_dir('src')
...
if with_gallium_or_lvp or with_gbm or with_platform_wayland:
  ...
  if with_platform_wayland:
    ...
    wp_protos={'fifo-v1':'staging/fifo/fifo-v1.xml','commit-timing-v1':'staging/commit-timing/commit-timing-v1.xml','linux-dmabuf-unstable-v1':'unstable/linux-dmabuf/linux-dmabuf-unstable-v1.xml','presentation-time':'stable/presentation-time/presentation-time.xml','tearing-control-v1':'staging/tearing-control/tearing-control-v1.xml','linux-drm-syncobj-v1':'staging/linux-drm-syncobj/linux-drm-syncobj-v1.xml','color-management-v1':'staging/color-management/color-management-v1.xml',}
    wp_files={}
    for name,xml in wp_protos.items():
      code=custom_target(name+'-protocol.c',input=files(join_paths(wp_dir,xml)),output=name+'-protocol.c',command=[prog_wl_scanner,wl_scanner_arg,'@INPUT@','@OUTPUT@'],)
      header=custom_target(name+'-client-protocol.h',input=files(join_paths(wp_dir,xml)),output=name+'-client-protocol.h',command=[prog_wl_scanner,'client-header','@INPUT@','@OUTPUT@'],)
      wp_files |= {name:[code,header]}
  
  
    libloader_wayland_helper=static_library('loader_wayland_helper','loader_wayland_helper.c',wp_files['presentation-time'],gnu_symbol_visibility='hidden',include_directories=[inc_include,inc_src,inc_gallium],dependencies=[idep_mesautil,dep_wayland_client,],build_by_default=False,)
  else:
    libloader_wayland_helper=[]
  
  
  loader_c_args=['-DUSE_DRICONF',]
  libloader=static_library('loader',['loader_dri_helper.c','loader.c',sha1_h],c_args=loader_c_args,gnu_symbol_visibility='hidden',include_directories=[inc_include,inc_src,inc_util,inc_gallium],dependencies=[idep_mesautil,dep_libdrm,dep_thread,dep_xcb,dep_xcb_xrandr],build_by_default=False,)
...
if with_gallium:
  ...
  ########################################################################################################################
  glapi_xml_py_deps=files('glapi/gen/gl_API.dtd','glapi/gen/es_EXT.xml','glapi/gen/gl_API.xml','glapi/gen/gl_and_es_API.xml','glapi/gen/gl_and_glX_API.xml','glapi/gen/glX_API.xml','glapi/gen/ARB_base_instance.xml','glapi/gen/ARB_blend_func_extended.xml','glapi/gen/ARB_bindless_texture.xml','glapi/gen/ARB_clear_buffer_object.xml','glapi/gen/ARB_clear_texture.xml','glapi/gen/ARB_clip_control.xml','glapi/gen/ARB_color_buffer_float.xml','glapi/gen/ARB_compressed_texture_pixel_storage.xml','glapi/gen/ARB_compute_shader.xml','glapi/gen/ARB_compute_variable_group_size.xml','glapi/gen/ARB_copy_buffer.xml','glapi/gen/ARB_copy_image.xml','glapi/gen/ARB_debug_output.xml','glapi/gen/ARB_depth_buffer_float.xml','glapi/gen/ARB_depth_clamp.xml','glapi/gen/ARB_direct_state_access.xml','glapi/gen/ARB_draw_buffers.xml','glapi/gen/ARB_draw_buffers_blend.xml','glapi/gen/ARB_draw_elements_base_vertex.xml','glapi/gen/ARB_draw_indirect.xml','glapi/gen/ARB_draw_instanced.xml','glapi/gen/ARB_ES2_compatibility.xml','glapi/gen/ARB_ES3_compatibility.xml','glapi/gen/ARB_framebuffer_no_attachments.xml','glapi/gen/ARB_framebuffer_object.xml','glapi/gen/ARB_get_program_binary.xml','glapi/gen/ARB_get_texture_sub_image.xml','glapi/gen/ARB_gl_spirv.xml','glapi/gen/ARB_gpu_shader_fp64.xml','glapi/gen/ARB_gpu_shader_int64.xml','glapi/gen/ARB_gpu_shader5.xml','glapi/gen/ARB_indirect_parameters.xml','glapi/gen/ARB_instanced_arrays.xml','glapi/gen/ARB_internalformat_query.xml','glapi/gen/ARB_internalformat_query2.xml','glapi/gen/ARB_invalidate_subdata.xml','glapi/gen/ARB_map_buffer_range.xml','glapi/gen/ARB_multi_bind.xml','glapi/gen/ARB_pipeline_statistics_query.xml','glapi/gen/ARB_program_interface_query.xml','glapi/gen/ARB_robustness.xml','glapi/gen/ARB_sample_shading.xml','glapi/gen/ARB_sampler_objects.xml','glapi/gen/ARB_seamless_cube_map.xml','glapi/gen/ARB_separate_shader_objects.xml','glapi/gen/ARB_shader_atomic_counters.xml','glapi/gen/ARB_shader_image_load_store.xml','glapi/gen/ARB_shader_subroutine.xml','glapi/gen/ARB_shader_storage_buffer_object.xml','glapi/gen/ARB_sparse_buffer.xml','glapi/gen/ARB_sparse_texture.xml','glapi/gen/ARB_spirv_extensions.xml','glapi/gen/ARB_sync.xml','glapi/gen/ARB_tessellation_shader.xml','glapi/gen/ARB_texture_barrier.xml','glapi/gen/ARB_texture_buffer_object.xml','glapi/gen/ARB_texture_buffer_range.xml','glapi/gen/ARB_texture_compression_rgtc.xml','glapi/gen/ARB_texture_cube_map_array.xml','glapi/gen/ARB_texture_float.xml','glapi/gen/ARB_texture_gather.xml','glapi/gen/ARB_texture_multisample.xml','glapi/gen/ARB_texture_rgb10_a2ui.xml','glapi/gen/ARB_texture_rg.xml','glapi/gen/ARB_texture_storage_multisample.xml','glapi/gen/ARB_texture_storage.xml','glapi/gen/ARB_texture_view.xml','glapi/gen/ARB_uniform_buffer_object.xml','glapi/gen/ARB_vertex_array_object.xml','glapi/gen/ARB_vertex_attrib_64bit.xml','glapi/gen/ARB_vertex_attrib_binding.xml','glapi/gen/ARB_viewport_array.xml','glapi/gen/AMD_depth_clamp_separate.xml','glapi/gen/AMD_draw_buffers_blend.xml','glapi/gen/AMD_gpu_shader_int64.xml','glapi/gen/AMD_performance_monitor.xml','glapi/gen/ARB_vertex_type_2_10_10_10_rev.xml','glapi/gen/EXT_direct_state_access.xml','glapi/gen/EXT_draw_buffers2.xml','glapi/gen/EXT_external_objects.xml','glapi/gen/EXT_external_objects_fd.xml','glapi/gen/EXT_external_objects_win32.xml','glapi/gen/EXT_framebuffer_object.xml','glapi/gen/EXT_gpu_shader4.xml','glapi/gen/EXT_multisampled_render_to_texture.xml','glapi/gen/EXT_packed_depth_stencil.xml','glapi/gen/EXT_provoking_vertex.xml','glapi/gen/EXT_separate_shader_objects.xml','glapi/gen/EXT_shader_image_load_store.xml','glapi/gen/EXT_texture_array.xml','glapi/gen/EXT_texture_integer.xml','glapi/gen/EXT_texture_storage.xml','glapi/gen/EXT_texture_storage_compression.xml','glapi/gen/EXT_transform_feedback.xml','glapi/gen/EXT_vertex_attrib_64bit.xml','glapi/gen/EXT_window_rectangles.xml','glapi/gen/GREMEDY_string_marker.xml','glapi/gen/INTEL_performance_query.xml','glapi/gen/KHR_debug.xml','glapi/gen/KHR_context_flush_control.xml','glapi/gen/KHR_robustness.xml','glapi/gen/KHR_robustness_es.xml','glapi/gen/KHR_shader_subgroup.xml','glapi/gen/KHR_texture_compression_astc.xml','glapi/gen/NV_alpha_to_coverage_dither_control.xml','glapi/gen/NV_copy_image.xml','glapi/gen/NV_conditional_render.xml','glapi/gen/NV_primitive_restart.xml','glapi/gen/NV_texture_barrier.xml','glapi/gen/NV_vdpau_interop.xml','glapi/gen/NV_viewport_swizzle.xml','glapi/gen/OES_EGL_image.xml','glapi/gen/OES_fixed_point.xml','glapi/gen/OES_single_precision.xml','glapi/gen/OES_texture_compression_astc.xml','glapi/gen/OVR_multiview.xml','glapi/gen/GL3x.xml','glapi/gen/GL4x.xml','glapi/registry/gl.xml','glapi/gen/apiexec.py','glapi/gen/gl_XML.py','glapi/gen/glX_XML.py','glapi/gen/glX_proto_common.py','glapi/gen/license.py','glapi/gen/marshal_XML.py','glapi/gen/static_data.py','glapi/gen/typeexpr.py','new/genCommon.py','../glx/libgl-symbols.txt',)
  ...
  ########################################################################################################################
  glapi_mapi_tmp_h=custom_target('glapi_mapi_tmp.h',input=['../../mapi_abi.py','gl_and_es_API.xml'],output='glapi_mapi_tmp.h',command=[prog_python,'@INPUT0@','--printer','glapi','@INPUT1@'],depend_files=glapi_xml_py_deps,capture=True,)
  
  glapi_gentable_c=custom_target('glapi_gentable.c',input=['gl_gentable.py','gl_and_es_API.xml'],output='glapi_gentable.c',command=[prog_python,'@INPUT0@','-f','@INPUT1@'],depend_files=glapi_xml_py_deps,capture=True,)
  ...
  main_api_exec_c=custom_target('api_exec_init.c',input=['api_exec_init.py','gl_and_es_API.xml'],output='api_exec_init.c',command=[prog_python,'@INPUT0@','-f','@INPUT1@'],depend_files=glapi_xml_py_deps,capture=True,)
  
  main_api_exec_decl_h=custom_target('api_exec_decl.h',input=['api_exec_decl_h.py','gl_and_es_API.xml'],output='api_exec_decl.h',command=[prog_python,'@INPUT0@','-f','@INPUT1@'],depend_files=glapi_xml_py_deps,capture=True,)
  
  main_api_save_init_h=custom_target('api_save_init.h',input=['api_save_init_h.py','gl_and_es_API.xml'],output='api_save_init.h',command=[prog_python,'@INPUT0@','-f','@INPUT1@'],depend_files=glapi_xml_py_deps,capture=True,)
  
  main_api_save_h=custom_target('api_save.h',input=['api_save_h.py','gl_and_es_API.xml'],output='api_save.h',command=[prog_python,'@INPUT0@','-f','@INPUT1@'],depend_files=glapi_xml_py_deps,capture=True,)
  
  main_api_beginend_init_h=custom_target('api_beginend_init.h',input=['api_beginend_init_h.py','gl_and_es_API.xml'],output='api_beginend_init.h',command=[prog_python,'@INPUT0@','-f','@INPUT1@'],depend_files=glapi_xml_py_deps,capture=True,)
  
  main_api_hw_select_init_h=custom_target('api_hw_select_init.h',input=['api_hw_select_init_h.py','gl_API.xml'],output='api_hw_select_init.h',command=[prog_python,'@INPUT0@','-f','@INPUT1@'],depend_files=glapi_xml_py_deps,capture=True,)
  
  main_dispatch_h=custom_target('dispatch.h',input=['gl_table.py','gl_and_es_API.xml'],output='dispatch.h',command=[prog_python,'@INPUT0@','-f','@INPUT1@','-m','dispatch'],depend_files=glapi_xml_py_deps,capture=True,)
  
  idep_dispatch_h=declare_dependency(sources=[main_dispatch_h])
  
  main_marshal_generated_h=custom_target('marshal_generated.h',input=['marshal_generated_h.py','gl_and_es_API.xml'],output='marshal_generated.h',command=[prog_python,'@INPUT0@','@INPUT1@',sizeof_pointer],depend_files=glapi_xml_py_deps,capture=True,)
  
  main_unmarshal_table_c=custom_target('unmarshal_table.c',input=['unmarshal_table_c.py','gl_and_es_API.xml'],output='unmarshal_table.c',command=[prog_python,'@INPUT0@','@INPUT1@',sizeof_pointer],depend_files=glapi_xml_py_deps,capture=True,)
  
  main_marshal_generated_c=[]
  for x in ['0','1','2','3','4','5','6','7']:
    main_marshal_generated_c+=custom_target('marshal_generated'+x+'.c',input=['marshal_generated_c.py','gl_and_es_API.xml'],output='marshal_generated'+x+'.c',command=[prog_python,'@INPUT0@','@INPUT1@',x,'8',sizeof_pointer],depend_files=glapi_xml_py_deps,capture=True,)
  
  
  glx_generated=[]
  
  for x in [['indirect.c','proto'],['indirect.h','init_h'],['indirect_init.c','init_c']]:
    glx_generated+=custom_target(x[0],input=['glX_proto_send.py','gl_API.xml'],output=x[0],command=[prog_python,'@INPUT0@','-f','@INPUT1@','-m',x[1]],depend_files=glapi_xml_py_deps,capture=True,)
  
  
  for x in [['indirect_size.h',['-m','size_h','--header-tag','_INDIRECT_SIZE_H_']],['indirect_size.c',['-m','size_c']]]:
    glx_generated+=custom_target(x[0],input=['glX_proto_size.py','gl_API.xml'],output=x[0],command=[prog_python,'@INPUT0@','-f','@INPUT1@','--only-set',x[1]],depend_files=glapi_xml_py_deps,capture=True,)
  ...
  bridge_glapi_files=files('libgl_public.c','../entry_x86-64_tls.h','../entry_x86_tls.h','../entry_ppc64le_tls.h',)
  ...
  libglapi_bridge=static_library('glapi_bridge',bridge_glapi_files,include_directories=[inc_mesa,inc_include,inc_src,inc_mapi,inc_gallium],c_args=[c_msvc_compat_args,bridge_glapi_args],dependencies=[dep_thread,idep_mesautil],build_by_default=False,)
  
  ########################################################################################################################
  ### End conversion from: src/mapi/glapi/meson.build
  ########################################################################################################################
  set_relative_dir('src/mapi')
  set_relative_dir('src/mapi/shared-glapi')
  ########################################################################################################################
  ### Begin conversion from: src/mapi/shared-glapi/meson.build
  ########################################################################################################################
  shared_glapi_mapi_tmp_h=custom_target('shared_glapi_mapi_tmp.h',input=['../mapi_abi.py','../glapi/gen/gl_and_es_API.xml'],output='shared_glapi_mapi_tmp.h',command=[prog_python,'@INPUT0@','--printer','shared-glapi','@INPUT1@'],depend_files=glapi_xml_py_deps,capture=True,)
  
  libglapi=static_library('glapi',['core.c',shared_glapi_mapi_tmp_h],c_args=[c_msvc_compat_args,'-DMAPI_MODE_SHARED_GLAPI',gcc_lto_quirk,],gnu_symbol_visibility='hidden',link_args=[ld_args_gc_sections],include_directories=[inc_src,inc_include,inc_mapi],dependencies=[dep_thread,idep_mesautil],install=False,)
  libglapi_build_dir=meson.current_build_dir()
  
  ########################################################################################################################
  ### End conversion from: src/mapi/shared-glapi/meson.build
  ########################################################################################################################
  set_relative_dir('src/mapi')
...
builtin_types_h=custom_target('builtin_types.h',input='builtin_types_h.py',output='builtin_types.h',command=[prog_python,'@INPUT@','@OUTPUT@'],depend_files=files('builtin_types.py'),depends=u_format_gen_h,)
...
files_libcompiler=files('glsl_types.c','glsl_types.h','list.h','shader_enums.c','shader_enums.h','shader_info.h',)
...
if not with_nir_headers_only:
  custom_targets_libnir+=[nir_opt_algebraic_c,nir_opcodes_c,nir_opcodes_h,nir_constant_expressions_c,nir_builder_opcodes_h,nir_intrinsics_c,nir_intrinsics_h,nir_intrinsics_indices_h]


_libnir=static_library('nir',[files_libnir,custom_targets_libnir],include_directories=[inc_include,inc_src],c_args=[c_msvc_compat_args,no_override_init_args,no_misleading_indentation],gnu_symbol_visibility='hidden',dependencies=[idep_compiler,dep_valgrind,idep_mesautil],build_by_default=False,)

inc_nir=include_directories('.')

# Headers-only dependency
idep_nir_headers=declare_dependency(sources=[nir_opcodes_h,nir_builder_opcodes_h,nir_intrinsics_h,nir_intrinsics_indices_h,ir_expression_operation_h,builtin_types_h],include_directories=[inc_nir,inc_compiler,inc_src,inc_include],)
...
files_libvtn=files('GLSL.ext.AMD.h','GLSL.std.450.h','OpenCL.std.h','gl_spirv.c','nir_spirv.h','spirv.h','spirv_to_nir.c','vtn_alu.c','vtn_amd.c','vtn_cfg.c','vtn_cmat.c','vtn_debug.c','vtn_glsl450.c','vtn_opencl.c','vtn_private.h','vtn_structured_cfg.c','vtn_subgroup.c','vtn_variables.c',)

libvtn=static_library('vtn',[files_libvtn,spirv_info,vtn_gather_types_c,vtn_generator_ids_h,],include_directories=[inc_include,inc_src,inc_mapi,inc_mesa],c_args=[c_msvc_compat_args,no_override_init_args],gnu_symbol_visibility='hidden',dependencies=[dep_spirv_tools,idep_nir,dep_valgrind],build_by_default=False,)

idep_vtn=declare_dependency(dependencies=[dep_spirv_tools,idep_nir,idep_mesautil],link_with=libvtn,sources=[spirv_info_h],)
...
if get_option('mesa-clc')!='system' and (with_driver_using_cl or get_option('install-mesa-clc')):
  prog_vtn_bindgen2=executable('vtn_bindgen2',['vtn_bindgen2.c'],include_directories=[inc_include,inc_src],c_args=[c_msvc_compat_args,no_override_init_args],dependencies=[idep_vtn,idep_mesautil,idep_nir],native=not meson.can_run_host_binaries(),install=get_option('install-mesa-clc'),)


if with_tests:
  test('spirv_tests',executable('spirv_tests',files('tests/helpers.h','tests/avail_vis.cpp','tests/volatile.cpp','tests/cmat.cpp','tests/control_flow_tests.cpp','tests/non_semantic.cpp','tests/workarounds.cpp',),c_args=[c_msvc_compat_args,no_override_init_args],gnu_symbol_visibility='hidden',include_directories=[inc_include,inc_src],dependencies=[idep_vtn,dep_thread,idep_gtest,idep_nir,idep_mesautil],),suite=['compiler','spirv'],protocol='gtest',)
...
if with_clc:
  ...
  if not _shared_llvm or get_option('mesa-clc-bundle-headers')=='enabled':
  ...
  has_spirv_link_workaround=cpp.has_member('spvtools::LinkerOptions','SetAllowPtrTypeMismatch(true)',prefix=['#include <spirv-tools/linker.hpp>',],dependencies=dep_spirv_tools,)
  ...
  if get_option('mesa-clc')!='system' and (with_driver_using_cl or get_option('install-mesa-clc')):
    prog_mesa_clc=executable('mesa_clc',['mesa_clc.c'],include_directories=[inc_include,inc_src],c_args=[pre_args,no_override_init_args],link_args=[ld_args_build_id],dependencies=[idep_mesaclc,dep_llvm,dep_spirv_tools,idep_getopt],native=not meson.can_run_host_binaries(),install=get_option('install-mesa-clc'),)
...
if with_gallium:
  ...
  ########################################################################################################################
  libglsl_util=static_library('glsl_util',glsl_util_files,include_directories=[inc_include,inc_src,inc_mapi,inc_mesa,inc_gallium,inc_gallium_aux],dependencies=[dep_valgrind,idep_mesautil],gnu_symbol_visibility='hidden',build_by_default=False,)
  ...
  files_libglsl=files('ast.h','ast_array_index.cpp','ast_expr.cpp','ast_function.cpp','ast_to_hir.cpp','ast_type.cpp','builtin_functions.cpp','builtin_functions.h','builtin_types.cpp','builtin_variables.cpp','gl_nir_detect_function_recursion.c','gl_nir_lower_atomics.c','gl_nir_lower_images.c','gl_nir_lower_blend_equation_advanced.c','gl_nir_lower_buffers.c','gl_nir_lower_discard_flow.c','gl_nir_lower_named_interface_blocks.c','gl_nir_lower_packed_varyings.c','gl_nir_lower_samplers.c','gl_nir_lower_samplers_as_deref.c','gl_nir_lower_xfb_varying.c','gl_nir_link_atomics.c','gl_nir_link_functions.c','gl_nir_link_interface_blocks.c','gl_nir_link_uniform_blocks.c','gl_nir_link_uniform_initializers.c','gl_nir_link_uniforms.c','gl_nir_link_varyings.c','gl_nir_link_varyings.h','gl_nir_link_xfb.c','gl_nir_linker.c','gl_nir_linker.h','gl_nir.h','glsl_parser_extras.cpp','glsl_parser_extras.h','glsl_symbol_table.cpp','glsl_symbol_table.h','glsl_to_nir.cpp','glsl_to_nir.h','hir_field_selection.cpp','ir_basic_block.cpp','ir_basic_block.h','ir_builder.cpp','ir_builder.h','ir_clone.cpp','ir_constant_expression.cpp','ir.cpp','ir.h','ir_expression_flattening.cpp','ir_expression_flattening.h','ir_function_detect_recursion.cpp','ir_function.cpp','ir_hierarchical_visitor.cpp','ir_hierarchical_visitor.h','ir_hv_accept.cpp','ir_list.h','ir_optimization.h','ir_print_visitor.cpp','ir_print_visitor.h','ir_rvalue_visitor.cpp','ir_rvalue_visitor.h','ir_validate.cpp','ir_variable_refcount.cpp','ir_variable_refcount.h','ir_visitor.h','linker_util.h','linker_util.cpp','lower_builtins.cpp','lower_instructions.cpp','lower_jumps.cpp','lower_mat_op_to_vec.cpp','lower_packing_builtins.cpp','lower_precision.cpp','lower_subroutine.cpp','lower_vec_index_to_cond_assign.cpp','lower_vector_derefs.cpp','opt_algebraic.cpp','opt_dead_builtin_variables.cpp','opt_dead_code.cpp','opt_flatten_nested_if_blocks.cpp','opt_function_inlining.cpp','opt_if_simplification.cpp','opt_minmax.cpp','opt_rebalance_tree.cpp','opt_tree_grafting.cpp','propagate_invariance.cpp','string_to_uint_map.cpp','string_to_uint_map.h','serialize.cpp','serialize.h','shader_cache.cpp','shader_cache.h',)
  ...
  libglsl=static_library('glsl',[files_libglsl,glsl_parser,glsl_lexer_cpp,libglsl_headers,ir_expression_operation_strings_h,ir_expression_operation_constant_h,float64_glsl_h],c_args=[c_msvc_compat_args,no_override_init_args],cpp_args=[cpp_msvc_compat_args],gnu_symbol_visibility='hidden',link_with=libglcpp,include_directories=[inc_include,inc_src,inc_mapi,inc_mesa,inc_gallium,inc_gallium_aux],dependencies=[idep_nir,idep_mesautil],build_by_default=False,)
  
  idep_libglsl=declare_dependency(sources=libglsl_headers,link_with=libglsl,)
  
  libglsl_standalone=static_library('glsl_standalone',[files_libglsl_standalone,ir_expression_operation_h],c_args=[c_msvc_compat_args,no_override_init_args],cpp_args=[cpp_msvc_compat_args],gnu_symbol_visibility='hidden',include_directories=[inc_include,inc_src,inc_mapi,inc_mesa,inc_gallium,inc_gallium_aux],link_with=[libglsl,libglsl_util,libglcpp_standalone],dependencies=[idep_mesautil,idep_getopt,idep_compiler,idep_nir],build_by_default=False,)
  ...
  if with_any_opengl and with_tests:
    ...
    ########################################################################################################################
    general_ir_test_files=files('builtin_variable_test.cpp','general_ir_test.cpp',)
    ...
    test('list_iterators',executable('list_iterators',['list_iterators.cpp'],cpp_args=[cpp_msvc_compat_args],gnu_symbol_visibility='hidden',include_directories=[inc_include,inc_src,inc_glsl],link_with=[libglsl,libglsl_util],dependencies=[dep_thread,idep_gtest],),suite=['compiler','glsl'],protocol='gtest',)
...
libisaspec=static_library('isaspec',['isaspec.c'],dependencies=[idep_mesautil],gnu_symbol_visibility='hidden',build_by_default=False,)
...
if with_nouveau_vk:
  set_relative_dir('src/compiler/rust')
  ########################################################################################################################
  ### Begin conversion from: src/compiler/rust/meson.build
  ########################################################################################################################
  _compiler_rs_sources=['as_slice.rs','bitset.rs','cfg.rs','memstream.rs','nir_instr_printer.rs','nir.rs','smallvec.rs',]
  
  _compiler_binding_types=['exec_list','exec_node','float_controls','gc_ctx','gl_access_qualifier','gl_frag_result','gl_interp_mode','gl_shader_stage','gl_subgroup_size','gl_system_value','gl_tess_spacing','gl_varying_slot','gl_vert_attrib','glsl_type','nir_.*','mesa_scope','mesa_prim','pipe_shader_type','shader_info','tess_primitive_mode','u_printf_info',]
  
  _compiler_bindgen_args=[bindgen_output_args,'--allowlist-var','NIR_.*','--allowlist-var','nir_.*_infos','--allowlist-var','rust_.*','--allowlist-function','glsl_.*','--allowlist-function','_mesa_shader_stage_to_string','--allowlist-function','nir_.*','--allowlist-function','compiler_rs.*','--allowlist-function','u_memstream.*','--allowlist-type','u_memstream','--no-prepend-enum-name','--with-derive-default',]
  
  for type in _compiler_binding_types:
    _compiler_bindgen_args+=['--allowlist-type',type]
  
  
  _libcompiler_c_sources=files('rust_helpers.c')
  
  _libcompiler_c=static_library('compiler_c_helpers',[_libcompiler_c_sources],include_directories=[inc_include,inc_util],c_args=[no_override_init_args],gnu_symbol_visibility='hidden',)
  
  _idep_libcompiler_c=declare_dependency(include_directories=include_directories('.'),link_with=_libcompiler_c,)
  
  _compiler_bindings_rs=rust.bindgen(input=['bindings.h'],output='bindings.rs',c_args=[pre_args,],args=_compiler_bindgen_args,dependencies=[idep_nir_headers,idep_mesautil,],)
  ...
  _compiler_rs_sources=structured_sources(['lib.rs',_compiler_bindings_rs,_compiler_rs_sources,])
  
  _libcompiler_rs=static_library('compiler',_compiler_rs_sources,gnu_symbol_visibility='hidden',rust_abi='rust',dependencies=[_idep_libcompiler_c],)
  ...
  if with_tests and get_option('b_sanitize')=='none':
    rust.test('compiler_test',_libcompiler_rs,suite=['compiler','rs'],dependencies=[idep_mesautil.partial_dependency(link_args=True,links=True),],rust_args=['-C','default-linker-libraries'],)
  
  
  idep_compiler_rs=declare_dependency(link_with=_libcompiler_rs,)
  
  dep_syn=dependency('syn',version='>= 2.0.15',fallback=['syn-2-rs','dep_syn'],required=True,)
  
  _libcompiler_proc_rs=static_library('compiler_proc','proc/lib.rs',gnu_symbol_visibility='hidden',dependencies=[dep_syn],rust_abi='rust',native=True,)
  
  idep_compiler_proc_rs=declare_dependency(link_with=_libcompiler_proc_rs,)
  
  ########################################################################################################################
  ### End conversion from: src/compiler/rust/meson.build
  ########################################################################################################################
  set_relative_dir('src/compiler')
...
if with_tools.count('drm-shim') > 0:
  set_relative_dir('src/drm-shim')
  ########################################################################################################################
  ### Begin conversion from: src/drm-shim/meson.build
  ########################################################################################################################
  drm_shim=static_library('drm_shim',['device.c','drm_shim.c',],include_directories=[inc_include,inc_src],dependencies=[dep_libdrm,idep_mesautil,dep_dl],gnu_symbol_visibility='hidden',)
  dep_drm_shim=declare_dependency(link_with=drm_shim,dependencies=dep_libdrm,)
  
  ########################################################################################################################
  ### End conversion from: src/drm-shim/meson.build
  ########################################################################################################################
  set_relative_dir('src')
...
if with_wayland_bind_display:
  set_relative_dir('src/egl/wayland/wayland-drm')
  ########################################################################################################################
  ### Begin conversion from: src/egl/wayland/wayland-drm/meson.build
  ########################################################################################################################
  inc_wayland_drm=include_directories('.')
  
  wayland_drm_protocol_c=custom_target('wayland-drm-protocol.c',input='wayland-drm.xml',output='wayland-drm-protocol.c',command=[prog_wl_scanner,wl_scanner_arg,'@INPUT@','@OUTPUT@'],)
  
  wayland_drm_client_protocol_h=custom_target('wayland-drm-client-protocol.h',input='wayland-drm.xml',output='wayland-drm-client-protocol.h',command=[prog_wl_scanner,'client-header','@INPUT@','@OUTPUT@'],)
  
  wayland_drm_server_protocol_h=custom_target('wayland-drm-server-protocol.h',input='wayland-drm.xml',output='wayland-drm-server-protocol.h',command=[prog_wl_scanner,'server-header','@INPUT@','@OUTPUT@'],)
  
  libwayland_drm=static_library('wayland_drm',['wayland-drm.c',wayland_drm_protocol_c,wayland_drm_server_protocol_h],gnu_symbol_visibility='hidden',dependencies=[dep_wayland_server],include_directories=inc_include,build_by_default=False,)
  
  ########################################################################################################################
  ### End conversion from: src/egl/wayland/wayland-drm/meson.build
  ########################################################################################################################
  set_relative_dir('src')
...
if with_any_vk or with_gallium_zink:
  ...
  if with_platform_macos:
    ...
    vulkan_wsi_deps+=dependency('appleframeworks',modules=['QuartzCore','Metal'])
  ...
  vk_physical_device_features_gen_depend_files=[files('vk_extensions.py'),vk_extensions_depend_files,]
  vk_physical_device_properties_gen_depend_files=[files('vk_extensions.py'),vk_extensions_depend_files,]
  vk_physical_device_spirv_caps_gen_depend_files=[files('vk_extensions.py'),vk_extensions_depend_files,files('vk_physical_device_features_gen.py'),vk_physical_device_features_gen_depend_files,]
  vk_synchronization_helpers_gen_depend_files=[files('vk_extensions.py'),vk_extensions_depend_files,]
  vk_enum_to_str_depend_files=[files('vk_extensions.py'),vk_extensions_depend_files,]
  vk_struct_type_cast_depend_files=[files('vk_extensions.py'),vk_extensions_depend_files,]
  ...
  vk_enum_to_str=custom_target('vk_enum_to_str',input=['gen_enum_to_str.py',vk_api_xml],output=['vk_enum_to_str.c','vk_enum_to_str.h','vk_enum_defines.h'],command=[prog_python,'@INPUT0@','--xml','@INPUT1@','--outdir',meson.current_build_dir(),'--beta',str(with_vulkan_beta)],depend_files=vk_enum_to_str_depend_files,)
  
  vk_struct_type_cast=custom_target('vk_struct_type_cast',input=['vk_struct_type_cast_gen.py',vk_api_xml],output=['vk_struct_type_cast.h'],command=[prog_python,'@INPUT0@','--xml','@INPUT1@','--outdir',meson.current_build_dir(),'--beta',str(with_vulkan_beta)],depend_files=vk_struct_type_cast_depend_files,)
  
  vk_extensions=custom_target('vk_extensions',input=[vk_extensions_gen,vk_api_xml],output=['vk_extensions.c','vk_extensions.h'],command=[prog_python,'@INPUT0@','--xml','@INPUT1@','--out-c','@OUTPUT0@','--out-h','@OUTPUT1@'],depend_files=vk_extensions_gen_depend_files,)
  ...
  idep_vulkan_util=declare_dependency(link_with=libvulkan_util,dependencies=[idep_vulkan_util_headers,idep_mesautil],)
  ...
  vulkan_lite_runtime_files+=[vk_cmd_enqueue_entrypoints,vk_cmd_queue,vk_common_entrypoints,vk_dispatch_trampolines,vk_format_info,vk_physical_device_features,vk_physical_device_properties,vk_physical_device_spirv_caps,vk_synchronization_helpers,]
  
  # as a runtime library dep to ensure header gen order
  vulkan_lite_runtime_header_gen_deps=declare_dependency(sources=[spirv_info_h,vk_cmd_enqueue_entrypoints[0],vk_cmd_queue[1],vk_common_entrypoints[0],vk_dispatch_trampolines[1],vk_format_info[1],vk_physical_device_features[1],vk_physical_device_properties[1],],)
  ...
  libvulkan_lite_instance=static_library('vulkan_lite_instance',['vk_instance.c'],include_directories=[inc_include,inc_src],dependencies=vulkan_lite_runtime_deps,c_args=['-DVK_LITE_RUNTIME_INSTANCE=1',c_msvc_compat_args],gnu_symbol_visibility='hidden',build_by_default=False,)
  ...
  # - idep_vulkan_runtime
  idep_vulkan_lite_runtime_headers=declare_dependency(sources=[spirv_info_h,vk_cmd_enqueue_entrypoints[0],vk_cmd_queue[1],vk_common_entrypoints[0],vk_physical_device_features[1],vk_physical_device_properties[1],],include_directories=include_directories('.'),)
  ...
  vulkan_runtime_files=files('vk_meta.c','vk_meta_blit_resolve.c','vk_meta_clear.c','vk_meta_copy_fill_update.c','vk_meta_draw_rects.c','vk_nir.c','vk_nir_convert_ycbcr.c','vk_pipeline.c','vk_pipeline_cache.c','vk_shader.c','vk_shader_module.c','vk_texcompress_etc2.c',)
  
  vulkan_runtime_deps=[vulkan_lite_runtime_deps,idep_nir,idep_vtn,]
  
  if prog_glslang.found():
    set_relative_dir('src/vulkan/runtime/radix_sort')
    ########################################################################################################################
    ### Begin conversion from: src/vulkan/runtime/radix_sort/meson.build
    ########################################################################################################################
    set_relative_dir('src/vulkan/runtime/radix_sort/shaders')
    ########################################################################################################################
    ### Begin conversion from: src/vulkan/runtime/radix_sort/shaders/meson.build
    ########################################################################################################################
    ...
    shader_include_files=files('bufref.h','prefix_limits.h','prefix.h','push.h','scatter.glsl','config.h',)
    
    defines=['-DRS_KEYVAL_DWORDS=2']
    
    radix_sort_spv=[]
    for s in radix_sort_shaders:
      radix_sort_spv+=custom_target(s+'.spv.h',input=s,output=s+'.spv.h',command=[prog_glslang,'-V','--target-env','spirv1.5','-x','-o','@OUTPUT@','@INPUT@']+defines+glslang_quiet+(['-g'] if with_mesa_debug else []),depend_files=shader_include_files)
    
    
    ########################################################################################################################
    ### End conversion from: src/vulkan/runtime/radix_sort/shaders/meson.build
    ########################################################################################################################
    set_relative_dir('src/vulkan/runtime/radix_sort')
    
    radix_sort_files=files('common/vk/barrier.c','common/vk/barrier.h','common/macros.h','common/util.c','common/util.h','shaders/push.h','radix_sort_u64.c','radix_sort_u64.h','radix_sort_vk_devaddr.h','radix_sort_vk_ext.h','radix_sort_vk.c','radix_sort_vk.h','target.h')
    
    ########################################################################################################################
    ### End conversion from: src/vulkan/runtime/radix_sort/meson.build
    ########################################################################################################################
    set_relative_dir('src/vulkan/runtime')
    set_relative_dir('src/vulkan/runtime/bvh')
    ########################################################################################################################
    ### Begin conversion from: src/vulkan/runtime/bvh/meson.build
    ########################################################################################################################
    bvh_shaders=[['lbvh_generate_ir.comp','lbvh_generate_ir',],['lbvh_main.comp','lbvh_main',],['leaf.comp','leaf',],['morton.comp','morton',],['ploc_internal.comp','ploc_internal',],]
    
    vk_bvh_include_dir=dir_source_root+'/src/vulkan/runtime/bvh'
    
    vk_bvh_includes=files('vk_build_helpers.h','vk_build_interface.h','vk_bvh.h',)
    
    bvh_spv=[]
    for s in bvh_shaders:
      command=[prog_glslang,'-V','-I'+vk_bvh_include_dir,'--target-env','spirv1.5','-x','-o','@OUTPUT@','@INPUT@']+(['-g'] if with_mesa_debug else [])
      command+=glslang_quiet
    
      bvh_spv+=custom_target(s[1]+'.spv.h',input=s[0],output=s[1]+'.spv.h',command=command,depend_files=vk_bvh_includes)
    
    
    ########################################################################################################################
    ### End conversion from: src/vulkan/runtime/bvh/meson.build
    ########################################################################################################################
    set_relative_dir('src/vulkan/runtime')
  ...
  idep_vulkan_runtime_headers+=declare_dependency(include_directories=include_directories('bvh'),)
  ...
  idep_vulkan_wsi_headers=declare_dependency(sources=wsi_entrypoints[0],dependencies=idep_vulkan_wsi_defines,include_directories=include_directories('.'))
  ...
  if with_vulkan_overlay_layer:
    set_relative_dir('src/vulkan/overlay-layer')
    ########################################################################################################################
    ### Begin conversion from: src/vulkan/overlay-layer/meson.build
    ########################################################################################################################
    overlay_shaders=['overlay.frag','overlay.vert',]
    overlay_spv=[]
    for s in ['overlay.frag','overlay.vert']:
      _name=f'{s}.spv.h'
      overlay_spv+=custom_target(_name,input=s,output=_name,command=[prog_glslang,'-V','-x','-o','@OUTPUT@','@INPUT@',glslang_quiet,glslang_depfile,],depfile=f'{_name}.d',)
    
    
    vklayer_files=files('overlay.cpp','overlay_params.c',)
    
    vklayer_mesa_overlay=shared_library('VkLayer_MESA_overlay',vklayer_files,overlay_spv,sha1_h,c_args=[no_override_init_args],gnu_symbol_visibility='hidden',dependencies=[idep_vulkan_util,idep_mesautil,vulkan_wsi_deps,libimgui_core_dep,dep_dl],include_directories=[inc_include,inc_src],link_args=cc.get_supported_link_arguments(['-Wl,-Bsymbolic-functions','-Wl,-z,relro']),install=True)
    
    configure_file(configuration={'library_path':'libVkLayer_MESA_overlay.so'},input='VkLayer_MESA_overlay.json.in',output='@BASENAME@',install=True,install_dir=join_paths(get_option('datadir'),'vulkan','explicit_layer.d'),install_tag='runtime',)
    
    configure_file(configuration={'library_path':vklayer_mesa_overlay.full_path()},input='VkLayer_MESA_overlay.json.in',output='@PLAINNAME@_dev.json',)
    devenv.append('VK_ADD_LAYER_PATH',meson.current_build_dir())
    
    install_data('mesa-overlay-control.py',install_dir=get_option('bindir'),install_mode='r-xr-xr-x',)
    
    ########################################################################################################################
    ### End conversion from: src/vulkan/overlay-layer/meson.build
    ########################################################################################################################
    set_relative_dir('src/vulkan')
  
  if with_vulkan_device_select_layer:
    set_relative_dir('src/vulkan/device-select-layer')
    ########################################################################################################################
    ### Begin conversion from: src/vulkan/device-select-layer/meson.build
    ########################################################################################################################
    vklayer_files=files('device_select_layer.c',)
    ...
    vklayer_mesa_device_select=shared_library('VkLayer_MESA_device_select',vklayer_files,c_args=[no_override_init_args,vklayer_flags],gnu_symbol_visibility='hidden',dependencies=[idep_vulkan_util,idep_vulkan_wsi_headers,idep_mesautil,vklayer_deps,dep_libdrm,dep_dl],include_directories=[inc_include,inc_util,inc_src],link_args=cc.get_supported_link_arguments(['-Wl,-Bsymbolic-functions','-Wl,-z,relro']),install=True)
    
    configure_file(configuration={'library_path':'libVkLayer_MESA_device_select.so'},input='VkLayer_MESA_device_select.json.in',output='@BASENAME@',install=True,install_dir=join_paths(get_option('datadir'),'vulkan','implicit_layer.d'),install_tag='runtime',)
    
    configure_file(configuration={'library_path':vklayer_mesa_device_select.full_path()},input='VkLayer_MESA_device_select.json.in',output='@PLAINNAME@_dev.json',)
    devenv.append('VK_ADD_IMPLICIT_LAYER_PATH',meson.current_build_dir())
    
    ########################################################################################################################
    ### End conversion from: src/vulkan/device-select-layer/meson.build
    ########################################################################################################################
    set_relative_dir('src/vulkan')
  
  if with_vulkan_screenshot_layer:
    set_relative_dir('src/vulkan/screenshot-layer')
    ########################################################################################################################
    ### Begin conversion from: src/vulkan/screenshot-layer/meson.build
    ########################################################################################################################
    vklayer_files=files('screenshot.cpp','screenshot_params.c',)
    
    vklayer_mesa_screenshot=shared_library('VkLayer_MESA_screenshot',vklayer_files,sha1_h,c_args=[no_override_init_args],gnu_symbol_visibility='hidden',dependencies=[idep_vulkan_util,idep_mesautil,vulkan_wsi_deps,dep_dl,dependency('libpng')],include_directories=[inc_include,inc_src],link_args=cc.get_supported_link_arguments(['-Wl,-Bsymbolic-functions','-Wl,-z,relro']),install=True)
    
    configure_file(configuration={'library_path':'libVkLayer_MESA_screenshot.so'},input='VkLayer_MESA_screenshot.json.in',output='@BASENAME@',install=True,install_dir=join_paths(get_option('datadir'),'vulkan','explicit_layer.d'),install_tag='runtime',)
    
    configure_file(configuration={'library_path':vklayer_mesa_screenshot.full_path()},input='VkLayer_MESA_screenshot.json.in',output='@PLAINNAME@_dev.json',)
    devenv.append('VK_ADD_LAYER_PATH',meson.current_build_dir())
    
    install_data('mesa-screenshot-control.py',install_dir=get_option('bindir'),install_mode='r-xr-xr-x',)
    
    ########################################################################################################################
    ### End conversion from: src/vulkan/screenshot-layer/meson.build
    ########################################################################################################################
    set_relative_dir('src/vulkan')
  
  if with_vulkan_vram_report_limit_layer:
    set_relative_dir('src/vulkan/vram-report-limit-layer')
    ########################################################################################################################
    ### Begin conversion from: src/vulkan/vram-report-limit-layer/meson.build
    ########################################################################################################################
    vklayer_files=files('vram_report_limit_layer.c',)
    ...
    configure_file(configuration={'library_path':'libVkLayer_MESA_vram_report_limit.so'},input='VkLayer_MESA_vram_report_limit.json.in',output='@BASENAME@',install=True,install_dir=join_paths(get_option('datadir'),'vulkan','explicit_layer.d'),install_tag='runtime',)
    
    configure_file(configuration={'library_path':vklayer_mesa_vram_report_limit.full_path()},input='VkLayer_MESA_vram_report_limit.json.in',output='@PLAINNAME@_dev.json',)
    devenv.append('VK_ADD_LAYER_PATH',meson.current_build_dir())
    
    ########################################################################################################################
    ### End conversion from: src/vulkan/vram-report-limit-layer/meson.build
    ########################################################################################################################
    set_relative_dir('src/vulkan')
  
  if with_vulkan_anti_lag_layer:
    set_relative_dir('src/vulkan/anti-lag-layer')
    ########################################################################################################################
    ### Begin conversion from: src/vulkan/anti-lag-layer/meson.build
    ########################################################################################################################
    vklayer_files=files('anti_lag_layer.c','anti_lag_layer_interface.c',)
    
    vklayer_mesa_anti_lag=shared_library('VkLayer_MESA_anti_lag',vklayer_files,c_args=[no_override_init_args],gnu_symbol_visibility='hidden',dependencies=[idep_vulkan_util,idep_mesautil,],include_directories=[inc_include,inc_util,inc_src],link_args=cc.get_supported_link_arguments(['-Wl,-Bsymbolic-functions','-Wl,-z,relro']),install=True)
    
    configure_file(configuration={'library_path':'libVkLayer_MESA_anti_lag.so'},input='VkLayer_MESA_anti_lag.json.in',output='@BASENAME@',install=True,install_dir=join_paths(get_option('datadir'),'vulkan','implicit_layer.d'),install_tag='runtime',)
    
    configure_file(configuration={'library_path':vklayer_mesa_anti_lag.full_path()},input='VkLayer_MESA_anti_lag.json.in',output='@PLAINNAME@_dev.json',)
    devenv.append('VK_ADD_IMPLICIT_LAYER_PATH',meson.current_build_dir())
    
    ########################################################################################################################
    ### End conversion from: src/vulkan/anti-lag-layer/meson.build
    ########################################################################################################################
    set_relative_dir('src/vulkan')
...
if with_any_intel:
  ...
  ########################################################################################################################
  gen_xml_files=['gen40.xml','gen45.xml','gen50.xml','gen60.xml','gen70.xml','gen75.xml','gen80.xml','gen90.xml','gen110.xml','gen120.xml','gen125.xml','gen200.xml','gen300.xml',]
  
  gen_xml_rt_files=['gen125_rt.xml','gen200_rt.xml','gen300_rt.xml',]
  ...
  genX_bits_included_symbols=['MI_BATCH_BUFFER_START::Batch Buffer Start Address','MI_REPORT_PERF_COUNT::Memory Address','MI_STORE_DATA_IMM::Address','MI_STORE_DATA_IMM::Immediate Data','MI_STORE_REGISTER_MEM::Memory Address','3DSTATE_DEPTH_BUFFER::Surface Base Address','3DSTATE_DEPTH_BUFFER::Surface Pitch','3DSTATE_STENCIL_BUFFER::Surface Base Address','3DSTATE_STENCIL_BUFFER::Surface Pitch','3DSTATE_HIER_DEPTH_BUFFER::Surface Base Address','3DSTATE_HIER_DEPTH_BUFFER::Surface Pitch','3DSTATE_CLEAR_PARAMS','3DSTATE_SO_BUFFER::Surface Base Address','3DSTATE_SO_BUFFER::Stream Offset','3DSTATE_CPSIZE_CONTROL_BUFFER::Surface Base Address','3DSTATE_CPSIZE_CONTROL_BUFFER::Surface Pitch','RENDER_SURFACE_STATE::Surface Base Address','RENDER_SURFACE_STATE::Surface Pitch','RENDER_SURFACE_STATE::Surface QPitch','RENDER_SURFACE_STATE::Auxiliary Surface Base Address','RENDER_SURFACE_STATE::Auxiliary Surface Pitch','RENDER_SURFACE_STATE::Clear Value Address','RENDER_SURFACE_STATE::Red Clear Color','RENDER_SURFACE_STATE::Green Clear Color','RENDER_SURFACE_STATE::Blue Clear Color','RENDER_SURFACE_STATE::Alpha Clear Color','RENDER_SURFACE_STATE::Width','RENDER_SURFACE_STATE::Height','RENDER_SURFACE_STATE::Depth','RENDER_SURFACE_STATE::Surface Format','RENDER_SURFACE_STATE::Surface Type','RENDER_SURFACE_STATE::Render Target View Extent','RENDER_SURFACE_STATE::Tile Mode','CLEAR_COLOR','VERTEX_BUFFER_STATE::Buffer Starting Address','CPS_STATE','RT_DISPATCH_GLOBALS::Hit Group Table','RT_DISPATCH_GLOBALS::Miss Group Table','RT_DISPATCH_GLOBALS::Callable Group Table','RT_DISPATCH_GLOBALS::Launch Width','RT_DISPATCH_GLOBALS::Launch Height','RT_DISPATCH_GLOBALS::Launch Depth',]
  ...
  for f in gen_xml_files+gen_xml_rt_files:
    ...
    gen_xml_pack+=custom_target(_name,input=['gen_pack_header.py',f],output=_name,command=[prog_python,'@INPUT@','--engines=render,blitter,compute'],capture=True,depend_files=gen_pack_header_deps)
  
  
  gen_xml_video_pack=[]
  for f in gen_xml_files:
    _name='{}_video_pack.h'.format(f.split('.')[0])
    gen_xml_pack+=custom_target(_name,input=['gen_pack_header.py',f],output=_name,command=[prog_python,'@INPUT@','--engines=video'],capture=True,depend_files=gen_pack_header_deps)
  
  
  genX_cl_included_symbols=['3DSTATE_DS','3DSTATE_HS','3DSTATE_INDEX_BUFFER','3DSTATE_VERTEX_BUFFERS','3DPRIMITIVE','3DPRIMITIVE_EXTENDED','MI_ARB_CHECK','MI_BATCH_BUFFER_START','VERTEX_BUFFER_STATE',]
  
  gen_cl_xml_pack=[]
  for f in gen_xml_files+gen_xml_rt_files:
    _name='{}_cl_pack.h'.format(f.split('.')[0])
    gen_cl_xml_pack+=custom_target(_name,input=['gen_pack_header.py',f],output=_name,command=[prog_python,'@INPUT@','--engines=render,blitter,compute','--opencl','--repack','--include-symbols',','.join(genX_cl_included_symbols)],capture=True,depend_files=gen_pack_header_deps)
  
  
  gen_pack_header_py=files('gen_pack_header.py')
  
  idep_genxml=declare_dependency(include_directories=include_directories('.'),dependencies=idep_mesautil,sources=[gen_xml_pack,gen_xml_video_pack,gen_cl_xml_pack,genX_bits_h,genX_xml_h])
  
  gen_xml_test_files=['../decoder/tests/gentest.xml']
  
  for f in gen_xml_files+gen_xml_rt_files+gen_xml_test_files:
  ...
  intel_dev_serialize_src=custom_target('intel_device_info_serialize_gen.c',input='intel_device_info_serialize_gen_c.py',depend_files='intel_device_info.py',output=['intel_device_info_serialize_gen.c'],command=[prog_python,'@INPUT@','@OUTPUT@'])
  ...
  idep_intel_dev=declare_dependency(link_with=libintel_dev,dependencies=[idep_intel_dev_wa,idep_intel_dev_info_gen,idep_mesautil],sources=[intel_dev_info_gen_src,intel_dev_wa_src[0]],)
  ...
  isl_per_hw_ver_libs=[]
  for g in [['40',isl_gfx4_files],['50',[]],['60',isl_gfx6_files],['70',isl_gfx7_files],['75',[]],['80',isl_gfx8_files],['90',isl_gfx9_files],['110',[]],['120',isl_gfx12_files],['125',isl_gfx12_files],['200',isl_gfx20_files],['300',isl_gfx20_files]]:
    _gfx_ver=g[0]
  ...
  if with_tests:
    ...
    test('isl_tile_std_y',executable('isl_tile_std_y_test','tests/isl_tile_std_y_test.c',dependencies=[dep_m,idep_mesautil],include_directories=[inc_include,inc_src,inc_intel],link_with=[libisl,libintel_dev],),suite=['intel'],)
    ...
    test('isl_surf_get_image_range_B_tile',executable('isl_surf_get_image_range_B_tile_test','tests/isl_surf_get_image_range_B_tile_test.c',dependencies=[dep_m,idep_mesautil,idep_intel_dev],link_with=libisl,include_directories=[inc_include,inc_src,inc_intel],),suite=['intel'],)
  ...
  if with_tests and not with_platform_android:
    for g in [['70','gfx7'],['75','hsw'],['80','gfx8'],['90','gfx9'],['110','gfx11'],['120','gfx12'],['125','gfx125'],['200','gfx20'],['300','gfx30']]:
  ...
  libintel_compiler_brw=static_library('intel_compiler',[libintel_compiler_brw_files,intel_nir_files,brw_nir_lower_fsign,brw_nir_trig,ir_expression_operation_h,[brw_device_sha1_gen_src]],include_directories=[inc_include,inc_src,inc_intel],c_args=[no_override_init_args],cpp_args=['-Werror=vla'],gnu_symbol_visibility='hidden',dependencies=[idep_nir_headers,idep_mesautil,idep_intel_dev],build_by_default=False,)
  
  idep_intel_compiler_brw=declare_dependency(link_with=[libintel_compiler_brw,libisl],dependencies=[idep_nir,idep_mesautil,idep_vtn,],)
  
  if with_tests:
    test('intel_compiler_brw_tests',executable('intel_compiler_brw_tests',files('test_eu_compact.cpp','test_eu_validate.cpp','test_helpers.cpp','test_helpers.h','test_insert_load_reg.cpp','test_lower_scoreboard.cpp','test_opt_algebraic.cpp','test_opt_cmod_propagation.cpp','test_opt_combine_constants.cpp','test_opt_copy_propagation.cpp','test_opt_cse.cpp','test_opt_register_coalesce.cpp','test_opt_saturate_propagation.cpp','test_simd_selection.cpp','test_vf_float_conversions.cpp',),ir_expression_operation_h,include_directories=[inc_include,inc_src,inc_intel],cpp_args=['-Werror=vla'],link_with=libisl,dependencies=[idep_gtest,idep_nir,idep_mesautil,idep_intel_dev,idep_intel_compiler_brw],),suite=['intel'],protocol='gtest',)
  ...
  if with_intel_tools:
    ...
    brw_asm_deps=[dep_thread,idep_intel_compiler_brw,idep_intel_dev,idep_mesautil,]
  
    brw_asm=static_library('brw_asm',['brw_asm.c',brw_gram_tab[0],brw_gram_tab[1],brw_lex_yy_c],dependencies=brw_asm_deps,include_directories=[inc_include,inc_src,inc_intel],c_args=[no_override_init_args],gnu_symbol_visibility='hidden',build_by_default=False,)
  
    idep_brw_asm=declare_dependency(link_with=brw_asm,dependencies=brw_asm_deps,)
  
    brw_asm_tool=executable('brw_asm',['brw_asm_tool.c'],dependencies=idep_brw_asm,include_directories=[inc_include,inc_src,inc_intel],c_args=[no_override_init_args],gnu_symbol_visibility='hidden',install=True)
  ...
  libintel_decoder=static_library('intel_decoder',[libintel_decoder_files,genX_xml_h,sha1_h],include_directories=[inc_include,inc_src,inc_intel],c_args=[no_override_init_args,sse2_args],gnu_symbol_visibility='hidden',dependencies=libintel_decoder_deps,build_by_default=False,)
  
  idep_intel_decoder=declare_dependency(link_with=[libintel_decoder],dependencies=libintel_decoder_deps,)
  
  libintel_decoder_brw=static_library('intel_decoder_brw',[libintel_decoder_brw_files,genX_xml_h,sha1_h],include_directories=[inc_include,inc_src,inc_intel],c_args=[no_override_init_args,sse2_args],gnu_symbol_visibility='hidden',dependencies=idep_intel_decoder,build_by_default=False,)
  
  idep_intel_decoder_brw=declare_dependency(link_with=[libintel_decoder_brw],dependencies=idep_intel_decoder,)
  
  if with_intel_elk:
    libintel_decoder_elk=static_library('intel_decoder_elk',[libintel_decoder_elk_files,genX_xml_h,sha1_h],include_directories=[inc_include,inc_src,inc_intel],c_args=[no_override_init_args,sse2_args],gnu_symbol_visibility='hidden',dependencies=idep_intel_decoder,build_by_default=False,)
  
    idep_intel_decoder_elk=declare_dependency(link_with=[libintel_decoder_elk],dependencies=idep_intel_decoder,)
  else:
  ...
  if with_intel_vk or with_gallium_iris:
    ...
    intel_shader_files=files('libintel_shaders.h','generate.cl','generate_draws.cl','generate_draws_iris.cl','query_copy.cl','util.cl',)
    
    intel_shaders_gens=[[80,8],[90,9],[110,11],[120,12],[125,125],[200,20],[300,30]]
    intel_shaders=[]
    intel_shaders_headers=[]
    for gen in intel_shaders_gens:
      intel_shaders_spv=custom_target(input=intel_shader_files,output='intel_gfx{}_shaders.spv'.format(gen[0]),command=[prog_mesa_clc,intel_shader_files,'-o','@OUTPUT@','--depfile','@DEPFILE@','--','-DGFX_VERx10={}'.format(gen[0]),'-I'+join_paths(meson.current_source_dir(),'.'),'-I'+join_paths(dir_source_root,'src/compiler/libcl'),'-I'+join_paths(dir_source_root,'src'),'-I'+join_paths(dir_source_root,'src/intel'),'-I'+join_paths(meson.project_build_root(),'src/intel'),'-I'+join_paths(dir_source_root,'src/intel/genxml'),cl_args,],depends=[gen_cl_xml_pack,intel_dev_wa_src],depfile='intel_gfx{}_shaders.spv.d'.format(gen[0]),)
    
      _intel_shaders=custom_target(input=intel_shaders_spv,output=['intel_gfx{}_shaders_binding.cpp'.format(gen[0]),'intel_gfx{}_shaders_binding.h'.format(gen[0])],command=[prog_vtn_bindgen2,intel_shaders_spv,'@OUTPUT0@','@OUTPUT1@'],)
    
      intel_shaders+=_intel_shaders
      intel_shaders_headers+=_intel_shaders[1]
    
    
    libintel_shaders=static_library('intel_shaders',intel_shaders,dependencies=idep_nir,)
    
    idep_intel_shaders=declare_dependency(link_with=libintel_shaders,sources=intel_shaders_headers,include_directories=include_directories('.'),)
  ...
  if with_intel_hasvk or with_intel_vk or with_gallium_crocus or with_gallium_iris or with_intel_tools:
    ...
    ########################################################################################################################
    intel_hw_metrics=['hsw','bdw','chv','sklgt2','sklgt3','sklgt4','kblgt2','kblgt3','cflgt2','cflgt3','bxt','glk','icl','ehl','tglgt1','tglgt2','rkl','dg1','adl','acmgt1','acmgt2','acmgt3','mtlgt2','mtlgt3','arlgt1','arlgt2','lnl','bmg','ptl',]
    ...
    idep_intel_perf=declare_dependency(link_with=libintel_perf,dependencies=[idep_intel_dev,idep_libintel_common],sources=intel_perf_sources)
  ...
  if with_intel_tools:
    ...
    ########################################################################################################################
    libintel_tools=static_library('intel_tools',files('aub_mem.c','aub_mem.h','aub_read.c','aub_read.h','intel_tools.c','intel_tools.h'),dependencies=[idep_mesautil,idep_intel_dev,idep_intel_decoder_brw,idep_intel_decoder_elk,idep_intel_compiler_brw,idep_intel_compiler_elk,idep_libintel_common,],include_directories=[inc_include,inc_src,inc_intel],c_args=[intel_common_args,no_override_init_args],gnu_symbol_visibility='hidden',install=False,)
    
    idep_libintel_tools=declare_dependency(link_with=libintel_tools,dependencies=[idep_intel_decoder_brw,idep_intel_decoder_elk,])
    
    aubinator=executable('aubinator',files('aubinator.c'),dependencies=[idep_mesautil,dep_expat,dep_zlib,dep_dl,dep_thread,dep_m,idep_intel_dev,idep_libintel_common,idep_libintel_tools],include_directories=[inc_include,inc_src,inc_intel],c_args=[no_override_init_args],gnu_symbol_visibility='hidden',install=True)
    
    aubinator_error_decode=executable('aubinator_error_decode',files('aubinator_error_decode.c','aubinator_error_decode_lib.c','aubinator_error_decode_lib.h','aubinator_error_decode_xe.c','aubinator_error_decode_xe.h','error_decode_lib.c','error_decode_lib.h','error_decode_xe_lib.c','error_decode_xe_lib.h'),dependencies=[idep_mesautil,dep_zlib,dep_thread,idep_intel_dev,idep_libintel_common,idep_libintel_tools],include_directories=[inc_include,inc_src,inc_intel],c_args=[no_override_init_args],gnu_symbol_visibility='hidden',install=True)
    ...
    intel_hang_replay=executable('intel_hang_replay',files('intel_hang_replay.c'),dependencies=[idep_intel_dev,idep_libintel_common,dep_libdrm,dep_dl,dep_thread,dep_m,idep_libintel_tools],include_directories=[inc_include,inc_src,inc_intel],c_args=[no_override_init_args],gnu_symbol_visibility='hidden',install=True)
    
    intel_monitor=executable('intel_monitor',files('intel_monitor.c','intel_monitor_eustall.c','intel_monitor_eustall.h'),dependencies=[idep_intel_dev,idep_intel_perf,idep_mesautil,dep_libdrm,dep_valgrind],include_directories=[inc_include,inc_src,inc_intel],c_args=[no_override_init_args],gnu_symbol_visibility='hidden',install=True)
    ...
    libintel_sanitize_gpu=shared_library('intel_sanitize_gpu',files('intel_sanitize_gpu.c'),dependencies=[idep_mesautil,dep_dl,dep_thread,idep_intel_dev,idep_libintel_common],include_directories=[inc_include,inc_src,inc_intel],c_args=[no_override_init_args,sse2_args],gnu_symbol_visibility='hidden',install_dir=get_option('libexecdir'),install=True)
    ...
    libintel_dump_gpu=shared_library('intel_dump_gpu',files('intel_context.h','gfx8_context.h','gfx10_context.h','intel_aub.h','aub_write.h','aub_write.c','intel_dump_gpu.c',),dependencies=[dep_dl,idep_mesautil,idep_intel_dev,idep_libintel_common],include_directories=[inc_include,inc_src,inc_intel],c_args=[no_override_init_args],gnu_symbol_visibility='hidden',install_dir=get_option('libexecdir'),install=True)
    ...
    if with_tools.count('intel-ui') > 0:
      ...
      aubinator_viewer=executable('aubinator_viewer',files('aubinator_viewer.cpp','aubinator_viewer_decoder.cpp'),dependencies=[idep_mesautil,dep_zlib,dep_dl,dep_thread,dep_m,libintel_imgui_gtk_dep,idep_intel_dev,idep_libintel_common,idep_libintel_tools],include_directories=[inc_include,inc_src,inc_intel],c_args=[no_override_init_args],gnu_symbol_visibility='hidden',cpp_args=['-fpermissive','-Wno-parentheses'],install=True)
    ...
    install_data('intel_measure.py',install_dir=get_option('bindir'),install_mode='rwxr-xr-x')
    ########################################################################################################################
    ### End conversion from: src/intel/tools/meson.build
    ########################################################################################################################
    set_relative_dir('src/intel')
    set_relative_dir('src/intel/executor')
    ########################################################################################################################
    ### Begin conversion from: src/intel/executor/meson.build
    ########################################################################################################################
    if not dep_lua.found():
      subdir_done()
    
    
    executor_flags=[no_override_init_args,sse2_args,]
    
    executor_includes=[inc_include,inc_src,inc_intel,]
    
    executor_hw_libs=[]
    for v in ['90','110','120','125','200','300']:
      executor_hw_libs+=static_library('executor_hw_ver{}'.format(v),['executor_genx.c',gen_xml_pack],include_directories=[executor_includes],c_args=[executor_flags,'-DGFX_VERx10={}'.format(v),],gnu_symbol_visibility='hidden',dependencies=[dep_valgrind,idep_genxml,idep_intel_dev,],)
    
    
    executor=executable('executor',['executor_main.c','executor_macros.c',],dependencies=[dep_libdrm,dep_lua,dep_valgrind,idep_brw_asm,idep_genxml,idep_intel_decoder_brw,idep_intel_dev,idep_libintel_common,],include_directories=[executor_includes],link_with=[executor_hw_libs],c_args=[executor_flags],gnu_symbol_visibility='hidden',install=True)
    
    ########################################################################################################################
    ### End conversion from: src/intel/executor/meson.build
    ########################################################################################################################
    set_relative_dir('src/intel')
  
  if get_option('vulkan-layers').count('intel-nullhw') > 0:
    set_relative_dir('src/intel/nullhw-layer')
    ########################################################################################################################
    ### Begin conversion from: src/intel/nullhw-layer/meson.build
    ########################################################################################################################
    vklayer_intel_nullhw_files=files('intel_nullhw.c',)
    ...
    configure_file(configuration={'library_path':'libVkLayer_INTEL_nullhw.so'},input='VkLayer_INTEL_nullhw.json.in',output='@BASENAME@',install=True,install_dir=join_paths(get_option('datadir'),'vulkan','explicit_layer.d'),install_tag='runtime',)
    
    configure_file(configuration={'library_path':vklayer_intel_nullhw.full_path()},input='VkLayer_INTEL_nullhw.json.in',output='@PLAINNAME@_dev.json',)
    devenv.append('VK_ADD_LAYER_PATH',meson.current_build_dir())
    
    ########################################################################################################################
    ### End conversion from: src/intel/nullhw-layer/meson.build
    ########################################################################################################################
    set_relative_dir('src/intel')
  ...
  if with_intel_vk:
    ...
    if with_intel_vk_rt:
      ...
      for shader in bvh_shaders:
        for gfx in gfx_versions:
          ...
          bvh_spv+=custom_target(output_name,input=shader,output=output_name,command=command,depend_files=[vk_bvh_includes,anv_bvh_includes],)
      
      
      
      ########################################################################################################################
      ### End conversion from: src/intel/vulkan/bvh/meson.build
      ########################################################################################################################
      set_relative_dir('src/intel/vulkan')
    ...
    for _gfx_ver in ['90','110','120','125','200','300']:
      libanv_per_hw_ver_libs+=static_library('anv_per_hw_ver{}'.format(_gfx_ver),[anv_per_hw_ver_files,anv_entrypoints[0]]+bvh_spv,include_directories=[inc_include,inc_src,inc_intel,],c_args=anv_flags+['-DGFX_VERx10={}'.format(_gfx_ver)],gnu_symbol_visibility='hidden',dependencies=[dep_libdrm,dep_valgrind,idep_nir_headers,idep_genxml,idep_vulkan_util_headers,idep_vulkan_wsi_headers,idep_vulkan_runtime_headers,idep_mesautil,idep_intel_driver_ds_headers,idep_intel_shaders,idep_intel_blorp,],)
    
    
    libanv_files=files('i915/anv_batch_chain.c','i915/anv_batch_chain.h','i915/anv_device.c','i915/anv_device.h','i915/anv_gem.c','i915/anv_gem.h','i915/anv_kmd_backend.c','i915/anv_queue.c','i915/anv_queue.h','layers/anv_android_layer.c','layers/anv_doom64.c','layers/anv_hitman3.c','layers/anv_rmv_layer.c','xe/anv_batch_chain.c','xe/anv_batch_chain.h','xe/anv_kmd_backend.c','xe/anv_device.c','xe/anv_device.h','xe/anv_queue.c','xe/anv_queue.h','anv_allocator.c','anv_android.h','anv_astc_emu.c','anv_batch_chain.c','anv_blorp.c','anv_bo_sync.c','anv_buffer.c','anv_buffer_view.c','anv_cmd_buffer.c','anv_descriptor_set.c','anv_device.c','anv_embedded_sampler.c','anv_event.c','anv_formats.c','anv_genX.h','anv_image.c','anv_image_host_copy.c','anv_image_view.c','anv_internal_kernels.c','anv_internal_kernels.h','anv_instance.c','anv_kmd_backend.c','anv_kmd_backend.h','anv_measure.c','anv_measure.h','anv_nir.h','anv_nir_apply_pipeline_layout.c','anv_nir_compute_push_layout.c','anv_nir_lower_multiview.c','anv_nir_lower_ubo_loads.c','anv_nir_lower_resource_intel.c','anv_nir_push_descriptor_analysis.c','anv_perf.c','anv_physical_device.c','anv_pipeline.c','anv_pipeline_cache.c','anv_private.h','anv_queue.c','anv_rmv.c','anv_rmv.h','anv_sampler.c','anv_slab_bo.c','anv_slab_bo.h','anv_sparse.c','anv_util.c','anv_utrace.c','anv_va.c','anv_video.c','anv_wsi.c','bvh/anv_bvh.h','bvh/anv_build_interface.h',)
    
    anv_deps=[dep_libdrm,dep_valgrind,idep_genxml,idep_mesautil,idep_nir_headers,idep_vulkan_util_headers,idep_vulkan_runtime_headers,idep_vulkan_wsi_headers,idep_intel_shaders,idep_intel_blorp,]
    ...
    if with_tests:
      libvulkan_intel_test=static_library('vulkan_intel_test',[files('anv_gem_stubs.c'),anv_entrypoints[0]],include_directories=[inc_include,inc_src,inc_intel,],link_whole=libanv_common,link_with=[libanv_per_hw_ver_libs,libintel_common,libisl,libintel_perf,],dependencies=[dep_thread,dep_dl,dep_m,anv_deps,idep_nir,idep_vulkan_util,idep_vulkan_wsi,idep_vulkan_runtime,idep_mesautil,idep_intel_dev,idep_intel_blorp,idep_intel_compiler_brw,idep_intel_decoder_brw,],c_args=anv_flags,gnu_symbol_visibility='hidden',)
  ...
  if with_intel_hasvk:
    ...
    _dev_icd=custom_target('intel_hasvk_devenv_icd',input=[vk_icd_gen,vk_api_xml],output=_dev_icdname,command=[prog_python,'@INPUT0@','--api-version','1.3','--xml','@INPUT1@','--sizeof-pointer',sizeof_pointer,'--lib-path',(meson.current_build_dir() + 'libvulkan_intel_hasvk.so') if isinstance(meson.current_build_dir(), str) else (meson.current_build_dir() / 'libvulkan_intel_hasvk.so'),'--out','@OUTPUT@',],build_by_default=True,)
    ...
    anv_deps=[dep_libdrm,dep_valgrind,idep_genxml,idep_mesautil,idep_nir_headers,idep_vulkan_util_headers,idep_vulkan_runtime_headers,idep_vulkan_wsi_headers,]
    anv_flags=[no_override_init_args,sse2_args,]
...
if with_virtio_vk:
  ...
  libvn_files=files('vn_acceleration_structure.c','vn_buffer.c','vn_command_buffer.c','vn_common.c','vn_cs.c','vn_descriptor_set.c','vn_device.c','vn_device_memory.c','vn_feedback.c','vn_host_copy.c','vn_icd.c','vn_image.c','vn_instance.c','vn_physical_device.c','vn_pipeline.c','vn_query_pool.c','vn_queue.c','vn_render_pass.c','vn_ring.c','vn_renderer_internal.c','vn_renderer_util.c','vn_renderer_vtest.c',)
  
  vn_deps=[dep_thread,idep_mesautil,idep_vulkan_util,idep_vulkan_lite_runtime,idep_vulkan_wsi,idep_xmlconfig,]
  ...
  if with_platform_wayland or with_platform_x11 or (system_has_kms_drm and not with_platform_android):
...
if with_vdrm:
  ...
  libvdrm=static_library('libvdrm',['vdrm.c','vdrm.h','vdrm_virtgpu.c','vdrm_vpipe.c',],include_directories=[inc_include,inc_src,inc_virtio,inc_virtio_gpu,],c_args=[no_override_init_args],gnu_symbol_visibility='hidden',dependencies=[dep_libdrm,dep_valgrind,idep_mesautil,],link_with=[_libmesa_util,],build_by_default=False,)
...
if with_gallium_radeonsi or with_amd_vk or with_gallium_r300 or with_gallium_r600:
  ...
  if with_amd_vk or with_gallium_radeonsi:
    ...
    if with_mesa_ndebug:
      cpp_args_addrlib += ['-DDEBUG=0'] if isinstance(cpp_args_addrlib, list) else '-DDEBUG=0'
    else:
      cpp_args_addrlib += ['-DDEBUG=1'] if isinstance(cpp_args_addrlib, list) else '-DDEBUG=1'
    
    
    cpp_args_addrlib+=cpp.get_supported_arguments(['-Wno-unused-variable','-Wno-unused-local-typedefs','-Wno-unused-but-set-variable','-Wno-maybe-uninitialized','-Wno-self-assign','/wd4189','-Wno-uninitialized'])
    ...
    gfx10_format_table_c=custom_target('gfx10_format_table.c',input=files('gfx10_format_table.py','../../util/format/u_format.yaml','../registers/gfx10-rsrc.json','../registers/gfx11-rsrc.json'),output='gfx10_format_table.c',command=[prog_python,'@INPUT@'],capture=True,depend_files=['../registers/regdb.py'])
    
    amd_common_files=files('ac_binary.c','ac_binary.h','ac_cmdbuf.c','ac_cmdbuf.h','ac_shader_args.c','ac_shader_args.h','ac_shader_util.c','ac_shader_util.h','ac_gather_context_rolls.c','ac_gpu_info.c','ac_gpu_info.h','ac_surface.c','ac_surface.h','ac_debug.c','ac_debug.h','ac_descriptors.c','ac_descriptors.h','ac_formats.c','ac_formats.h','ac_linux_drm.h','ac_shadowed_regs.c','ac_shadowed_regs.h','ac_spm.c','ac_spm.h','ac_sqtt.c','ac_sqtt.h','ac_rgp.c','ac_rgp.h','ac_msgpack.c','ac_msgpack.h','amd_family.c','ac_parse_ib.c','ac_perfcounter.c','ac_perfcounter.h','ac_pm4.c','ac_pm4.h','ac_vcn_av1_default.h','ac_vcn_vp9_default.h','ac_vcn_dec.c','ac_vcn_enc.c','ac_uvd_dec.c','nir/ac_nir.c','nir/ac_nir.h','nir/ac_nir_helpers.h','nir/ac_nir_opt_outputs.c','nir/ac_nir_cull.c','nir/ac_nir_create_gs_copy_shader.c','nir/ac_nir_lower_esgs_io_to_mem.c','nir/ac_nir_lower_global_access.c','nir/ac_nir_lower_image_opcodes_cdna.c','nir/ac_nir_lower_intrinsics_to_args.c','nir/ac_nir_lower_legacy_gs.c','nir/ac_nir_lower_legacy_vs.c','nir/ac_nir_lower_mem_access_bit_sizes.c','nir/ac_nir_lower_resinfo.c','nir/ac_nir_lower_taskmesh_io_to_mem.c','nir/ac_nir_lower_tess_io_to_mem.c','nir/ac_nir_lower_tex.c','nir/ac_nir_lower_ngg.c','nir/ac_nir_lower_ngg_gs.c','nir/ac_nir_lower_ngg_mesh.c','nir/ac_nir_lower_ps_early.c','nir/ac_nir_lower_ps_late.c','nir/ac_nir_lower_sin_cos.c','nir/ac_nir_meta.h','nir/ac_nir_meta_cs_blit.c','nir/ac_nir_meta_cs_clear_copy_buffer.c','nir/ac_nir_meta_ps_resolve.c','nir/ac_nir_opt_pack_half.c','nir/ac_nir_opt_shared_append.c','nir/ac_nir_prerast_utils.c','nir/ac_nir_surface.c','nir/ac_nir_surface.h',)
    
    if not with_platform_windows:
      amd_common_files+=files('ac_linux_drm.c',)
    ...
    if with_amdgpu_virtio:
      ...
      amd_common_files+=files('virtio/amdgpu_virtio.c','virtio/amdgpu_virtio_bo.c','virtio/amdgpu_virtio_device.c','virtio/amdgpu_virtio_private.h','virtio/amdgpu_virtio_proto.h')
      link_with+=libvdrm
    ...
    libamd_common=static_library('amd_common',[amd_common_files,sid_tables_h,amdgfxregs_h,gfx10_format_table_c],include_directories=[inc_include,inc_src,inc_amd,inc_virtio_gpu,],dependencies=[dep_llvm.partial_dependency(compile_args=True,includes=True)]+[dep_thread,dep_elf,dep_libdrm_amdgpu,dep_valgrind,idep_mesautil,idep_nir_headers,idep_nir],link_with=[link_with],gnu_symbol_visibility='hidden',c_args=c_args)
    ...
    if with_tests and not with_platform_windows:
      test('ac_surface_modifier_test',executable('ac_surface_modifier_test',['ac_surface_modifier_test.c'],link_with=[libamd_common,libamdgpu_addrlib],include_directories=[inc_amd,inc_include,inc_src,],c_args=cpp_args_addrlib,dependencies=[idep_amdgfxregs_h,dep_libdrm_amdgpu,idep_mesautil],),suite=['amd'])
    
      # Limit this to only a few architectures for the Gitlab CI.
      if ['x86','x86_64','aarch64'].count(host_machine.cpu_family()) > 0:
        test('ac_surface_meta_address_test',executable('ac_surface_meta_address_test',['ac_surface_meta_address_test.c'],link_with=[libamd_common,libamdgpu_addrlib],include_directories=[inc_amd,inc_include,inc_src,],c_args=cpp_args_addrlib,dependencies=[idep_amdgfxregs_h,dep_libdrm_amdgpu,idep_mesautil,dep_openmp],),suite=['amd'])
    ...
    libaco_files=files('instruction_selection/aco_instruction_selection.h','instruction_selection/aco_isel_cfg.cpp','instruction_selection/aco_isel_helpers.cpp','instruction_selection/aco_isel_setup.cpp','instruction_selection/aco_select_nir_alu.cpp','instruction_selection/aco_select_nir_intrinsics.cpp','instruction_selection/aco_select_nir.cpp','instruction_selection/aco_select_ps_epilog.cpp','instruction_selection/aco_select_ps_prolog.cpp','instruction_selection/aco_select_rt_prolog.cpp','instruction_selection/aco_select_trap_handler.cpp','instruction_selection/aco_select_vs_prolog.cpp','aco_dead_code_analysis.cpp','aco_dominance.cpp','aco_interface.cpp','aco_interface.h','aco_ir.cpp','aco_ir.h','aco_assembler.cpp','aco_form_hard_clauses.cpp','aco_insert_delay_alu.cpp','aco_insert_exec_mask.cpp','aco_insert_fp_mode.cpp','aco_insert_NOPs.cpp','aco_insert_waitcnt.cpp','aco_reduce_assign.cpp','aco_register_allocation.cpp','aco_live_var_analysis.cpp','aco_lower_branches.cpp','aco_lower_phis.cpp','aco_lower_subdword.cpp','aco_lower_to_cssa.cpp','aco_lower_to_hw_instr.cpp','aco_optimizer.cpp','aco_optimizer_postRA.cpp','aco_opt_value_numbering.cpp','aco_print_asm.cpp','aco_print_ir.cpp','aco_reindex_ssa.cpp','aco_repair_ssa.cpp','aco_scheduler.cpp','aco_scheduler_ilp.cpp','aco_spill.cpp','aco_ssa_elimination.cpp','aco_statistics.cpp','aco_util.h','aco_validate.cpp',)
    
    cpp_args_aco=cpp.get_supported_arguments(['-fno-exceptions','-fno-rtti','-Wimplicit-fallthrough','-Wshadow'])
    
    _libaco=static_library('aco',[libaco_files,aco_opcodes_c,aco_opcodes_h,aco_builder_h],include_directories=[inc_include,inc_src,inc_amd,inc_amd_common,],dependencies=[dep_llvm,dep_thread,dep_valgrind,idep_nir_headers,idep_amdgfxregs_h,idep_mesautil,],gnu_symbol_visibility='hidden',build_by_default=True,cpp_args=[cpp_args_aco,cpp_msvc_compat_args],)
  ...
  if with_amd_vk:
    ...
    libradv_files=files('bvh/bvh.h','layers/radv_ctx_roll_layer.c','layers/radv_metro_exodus.c','layers/radv_rage2.c','layers/radv_quantic_dream.c','layers/radv_rmv_layer.c','layers/radv_rra_layer.c','layers/radv_sqtt_layer.c','meta/radv_meta.c','meta/radv_meta.h','meta/radv_meta_astc_decode.c','meta/radv_meta_blit.c','meta/radv_meta_blit2d.c','meta/radv_meta_buffer.c','meta/radv_meta_bufimage.c','meta/radv_meta_clear.c','meta/radv_meta_copy.c','meta/radv_meta_copy_vrs_htile.c','meta/radv_meta_dcc_retile.c','meta/radv_meta_decompress.c','meta/radv_meta_etc_decode.c','meta/radv_meta_fast_clear.c','meta/radv_meta_fmask_copy.c','meta/radv_meta_fmask_expand.c','meta/radv_meta_resolve.c','meta/radv_meta_resolve_cs.c','meta/radv_meta_resolve_fs.c','nir/radv_meta_nir.c','nir/radv_meta_nir.h','nir/radv_nir.h','nir/radv_nir_apply_pipeline_layout.c','nir/radv_nir_export_multiview.c','nir/radv_nir_lower_abi.c','nir/radv_nir_lower_cooperative_matrix.c','nir/radv_nir_lower_fs_barycentric.c','nir/radv_nir_lower_fs_intrinsics.c','nir/radv_nir_lower_hit_attrib_derefs.c','nir/radv_nir_lower_intrinsics_early.c','nir/radv_nir_lower_io.c','nir/radv_nir_lower_primitive_shading_rate.c','nir/radv_nir_lower_printf.c','nir/radv_nir_lower_ray_queries.c','nir/radv_nir_lower_view_index.c','nir/radv_nir_lower_viewport_to_zero.c','nir/radv_nir_lower_vs_inputs.c','nir/radv_nir_opt_fs_builtins.c','nir/radv_nir_opt_tid_function.c','nir/radv_nir_remap_color_attachment.c','nir/radv_nir_rt_common.c','nir/radv_nir_rt_shader.c','winsys/null/radv_null_bo.c','winsys/null/radv_null_bo.h','winsys/null/radv_null_cs.c','winsys/null/radv_null_cs.h','winsys/null/radv_null_winsys.c','winsys/null/radv_null_winsys_public.h','radv_acceleration_structure.c','radv_android.c','radv_android.h','radv_buffer.c','radv_buffer.h','radv_buffer_view.c','radv_buffer_view.h','radv_cmd_buffer.c','radv_cmd_buffer.h','radv_cp_dma.c','radv_cp_dma.h','radv_cp_reg_shadowing.c','radv_cp_reg_shadowing.h','radv_cs.c','radv_cs.h','radv_debug.c','radv_debug.h','radv_device.c','radv_device.h','radv_device_memory.c','radv_device_memory.h','radv_descriptors.c','radv_descriptors.h','radv_descriptor_pool.c','radv_descriptor_pool.h','radv_descriptor_set.c','radv_descriptor_set.h','radv_descriptor_update_template.c','radv_descriptor_update_template.h','radv_dgc.c','radv_dgc.h','radv_event.c','radv_event.h','radv_formats.c','radv_formats.h','radv_host_image_copy.c','radv_image.c','radv_image.h','radv_image_view.c','radv_image_view.h','radv_instance.c','radv_instance.h','radv_perfcounter.c','radv_physical_device.c','radv_pipeline.c','radv_pipeline.h','radv_pipeline_binary.c','radv_pipeline_binary.h','radv_pipeline_cache.c','radv_pipeline_cache.h','radv_pipeline_compute.c','radv_pipeline_compute.h','radv_pipeline_graphics.c','radv_pipeline_layout.c','radv_pipeline_layout.h','radv_pipeline_rt.c','radv_pipeline_rt.h','radv_printf.c','radv_printf.h','radv_queue.c','radv_queue.h','radv_radeon_winsys.h','radv_rmv.c','radv_rmv.h','radv_rra_gfx10_3.c','radv_rra_gfx12.c','radv_rra.c','radv_rra.h','radv_sampler.c','radv_sampler.h','radv_sdma.c','radv_sdma.h','radv_shader.c','radv_shader.h','radv_shader_args.c','radv_shader_args.h','radv_shader_info.c','radv_shader_info.h','radv_shader_object.c','radv_shader_object.h','radv_spm.c','radv_spm.h','radv_sqtt.c','radv_sqtt.h','radv_query.c','radv_query.h','radv_video.c','radv_video.h','radv_video_enc.c','radv_wsi.c','radv_wsi.h',)
    
    if not with_platform_windows:
      libradv_files+=files('winsys/amdgpu/radv_amdgpu_bo.c','winsys/amdgpu/radv_amdgpu_bo.h','winsys/amdgpu/radv_amdgpu_cs.c','winsys/amdgpu/radv_amdgpu_cs.h','winsys/amdgpu/radv_amdgpu_winsys.c','winsys/amdgpu/radv_amdgpu_winsys.h','winsys/amdgpu/radv_amdgpu_winsys_public.h',)
    ...
    ########################################################################################################################
    bvh_shaders=[['copy_blas_addrs_gfx12.comp','copy_blas_addrs_gfx12',],['copy.comp','copy',],['encode_gfx12.comp','encode_gfx12',],['encode.comp','encode',],['header.comp','header',],['update.comp','update',],['update_gfx12.comp','update_gfx12',],['leaf.comp','radv_leaf',],]
    
    bvh_include_dir=dir_source_root+'/src/amd/vulkan/bvh'
    vk_bvh_include_dir=dir_source_root+'/src/vulkan/runtime/bvh'
    
    bvh_includes=files('build_helpers.h','build_interface.h','bvh.h','encode.h','update.h',vk_bvh_include_dir+'/vk_build_helpers.h',vk_bvh_include_dir+'/vk_bvh.h',)
    
    bvh_spv=[]
    for s in bvh_shaders:
      command=[prog_glslang,'-V','-I'+bvh_include_dir,'-I'+vk_bvh_include_dir,'--target-env','spirv1.5','-x','-o','@OUTPUT@','@INPUT@',glslang_depfile,glslang_quiet,]
    ...
    if with_amdgpu_virtio:
      radv_flags += ['-DHAVE_AMDGPU_VIRTIO'] if isinstance(radv_flags, list) else '-DHAVE_AMDGPU_VIRTIO'
    ...
    icd_command=[prog_python,'@INPUT0@','--api-version','1.4','--xml','@INPUT1@','--sizeof-pointer',sizeof_pointer,'--lib-path',join_paths(icd_lib_path,icd_file_name),'--out','@OUTPUT@',]
    if with_platform_windows:
    ...
    if with_radv_tests:
      test('radv_tests',executable('radv_tests',files('nir/radv_nir_lower_hit_attrib_derefs.c','tests/radv_nir_lower_hit_attrib_derefs_tests.cpp','tests/helpers.cpp','tests/helpers.h','tests/drirc.cpp','tests/misc.cpp',),cpp_args=[cpp_msvc_compat_args],gnu_symbol_visibility='hidden',include_directories=[inc_include,inc_src,inc_amd,inc_amd_common,inc_compiler,inc_util,include_directories('.'),],link_with=[libvulkan_radeon],dependencies=[dep_llvm,dep_thread,idep_gtest,idep_nir,idep_mesautil,idep_vulkan_util_headers],),suite=['compiler','nir'],protocol='gtest',)
    ...
    if with_aco_tests:
      ...
      ########################################################################################################################
      aco_tests_files=files('framework.h','helpers.cpp','helpers.h','main.cpp','test_assembler.cpp','test_builder.cpp','test_d3d11_derivs.cpp','test_hard_clause.cpp','test_insert_nops.cpp','test_insert_waitcnt.cpp','test_isel.cpp','test_lower_branches.cpp','test_lower_subdword.cpp','test_optimizer.cpp','test_reduce_assign.cpp','test_regalloc.cpp','test_optimizer_postRA.cpp','test_scheduler.cpp','test_sdwa.cpp','test_to_hw_instr.cpp','test_tests.cpp',)
  ...
  if with_tools.count('drm-shim') > 0:
    set_relative_dir('src/amd/drm-shim')
    ########################################################################################################################
    ### Begin conversion from: src/amd/drm-shim/meson.build
    ########################################################################################################################
    libradeon_noop_drm_shim=shared_library('radeon_noop_drm_shim','radeon_noop_drm_shim.c',include_directories=[inc_include,inc_src,inc_amd],dependencies=dep_drm_shim,gnu_symbol_visibility='hidden',install=True,)
    
    libamdgpu_noop_drm_shim=shared_library('amdgpu_noop_drm_shim',['amdgpu_noop_drm_shim.c','amdgpu_devices.c'],include_directories=[inc_include,inc_src,inc_amd],dependencies=dep_drm_shim,gnu_symbol_visibility='hidden',install=True,)
    
    amdgpu_dump=executable('amdgpu_dump_states','amdgpu_dump_states.c',include_directories=[inc_include,inc_src,inc_amd],gnu_symbol_visibility='hidden',dependencies=[dep_libdrm],install=False,)
    
    ########################################################################################################################
    ### End conversion from: src/amd/drm-shim/meson.build
    ########################################################################################################################
    set_relative_dir('src/amd')
  ...
  if with_gallium_radeonsi:
    ...
    ########################################################################################################################
    c_args_vpe=cc.get_supported_arguments(['-Wall','-Wextra','-Wno-unused','-Wno-unused-parameter','-Wno-unused-command-line-argument','-Wno-ignored-qualifiers','-Wno-missing-field-initializers','-Wno-self-assign','-Wno-implicit-fallthrough','-Werror=comment','-Werror=missing-braces','-Werror=override-init','-Werror=enum-conversion','-Werror=enum-compare','-Werror=maybe-uninitialized','-Werror=parentheses',])
    
    c_args_vpe+=['-DLITTLEENDIAN_CPU','-DVPE_BUILD_1_0','-DVPE_BUILD_1_X','-DVPE_BUILD_1_1',]
    
    vpe_files=files('src/core/inc/reg_helper.h','src/core/inc/vpe_priv.h','src/core/inc/vpe_command.h','src/core/inc/color_gamut.h','src/core/inc/vpe_assert.h','src/core/inc/vpec.h','src/core/inc/plane_desc_writer.h','src/core/inc/color_table.h','src/core/inc/hw_shared.h','src/core/inc/cdc.h','src/core/inc/dpp.h','src/core/inc/color_test_values.h','src/core/inc/vpe_visual_confirm.h','src/core/inc/color_cs.h','src/core/inc/shaper_builder.h','src/core/inc/color_bg.h','src/core/inc/transform.h','src/core/inc/common.h','src/core/inc/color.h','src/core/inc/mpc.h','src/core/inc/3dlut_builder.h','src/core/inc/geometric_scaling.h','src/core/inc/cmd_builder.h','src/core/inc/background.h','src/core/inc/color_gamma.h','src/core/inc/opp.h','src/core/inc/resource.h','src/core/inc/vpe_desc_writer.h','src/core/inc/config_writer.h','src/core/color_gamma.c','src/core/color_bg.c','src/core/vpe_scl_filters.c','src/core/background.c','src/core/vpe_visual_confirm.c','src/core/mpc.c','src/core/config_writer.c','src/core/color_gamut.c','src/core/vpelib.c','src/core/3dlut_builder.c','src/core/geometric_scaling.c','src/core/color_test_values.c','src/core/resource.c','src/core/color_table.c','src/core/color.c','src/core/color_cs.c','src/core/common.c','src/core/shaper_builder.c','src/utils/inc/custom_fp16.h','src/utils/inc/custom_float.h','src/utils/inc/fixed31_32.h','src/utils/inc/conversion.h','src/utils/inc/calc_u64.h','src/utils/inc/vector.h','src/utils/custom_fp16.c','src/utils/custom_float.c','src/utils/conversion.c','src/utils/fixpt31_32.c','src/utils/vector.c','src/chip/vpe10/inc/vpe10_command.h','src/chip/vpe10/inc/vpe10_plane_desc_writer.h','src/chip/vpe10/inc/vpe10_vpe_desc_writer.h','src/chip/vpe10/inc/vpe10_config_writer.h','src/chip/vpe10/inc/vpe10_background.h','src/chip/vpe10/inc/vpe10_cm_common.h','src/chip/vpe10/inc/vpe10_vpec.h','src/chip/vpe10/inc/vpe10_mpc.h','src/chip/vpe10/inc/vpe10_cmd_builder.h','src/chip/vpe10/inc/vpe10_opp.h','src/chip/vpe10/inc/asic/bringup_vpe_6_1_0_default.h','src/chip/vpe10/inc/asic/bringup_vpe_6_1_0_offset.h','src/chip/vpe10/inc/asic/bringup_vpe_6_1_0_sh_mask.h','src/chip/vpe10/inc/asic/vpe_1_0_offset.h','src/chip/vpe10/inc/vpe10_resource.h','src/chip/vpe10/inc/vpe10_cdc_fe.h','src/chip/vpe10/inc/vpe10_cdc_be.h','src/chip/vpe10/inc/vpe10_dpp.h','src/chip/vpe10/vpe10_plane_desc_writer.c','src/chip/vpe10/vpe10_vpe_desc_writer.c','src/chip/vpe10/vpe10_cm_common.c','src/chip/vpe10/vpe10_dpp.c','src/chip/vpe10/vpe10_resource.c','src/chip/vpe10/vpe10_mpc.c','src/chip/vpe10/vpe10_cmd_builder.c','src/chip/vpe10/vpe10_dpp_dscl.c','src/chip/vpe10/vpe10_dpp_cm.c','src/chip/vpe10/vpe10_opp.c','src/chip/vpe10/vpe10_background.c','src/chip/vpe10/vpe10_cdc_fe.c','src/chip/vpe10/vpe10_cdc_be.c','src/chip/vpe10/vpe10_vpec.c','src/chip/vpe10/vpe10_config_writer.c','src/chip/vpe11/inc/vpe11_command.h','src/chip/vpe11/inc/vpe11_cmd_builder.h','src/chip/vpe11/inc/vpe11_resource.h','src/chip/vpe11/inc/vpe11_vpe_desc_writer.h','src/chip/vpe11/vpe11_cmd_builder.c','src/chip/vpe11/vpe11_resource.c','src/chip/vpe11/vpe11_vpe_desc_writer.c')
    ...
    ########################################################################################################################
    set_relative_dir('src/amd')
    set_relative_dir('src/amd/gmlib')
    ########################################################################################################################
    ### Begin conversion from: src/amd/gmlib/meson.build
    ########################################################################################################################
    c_args_gm=cc.get_supported_arguments(['-Wall','-Wextra','-Wno-unused','-Wno-unused-parameter','-Wno-unused-command-line-argument','-Wno-ignored-qualifiers','-Wno-missing-field-initializers','-Wno-self-assign','-Wno-implicit-fallthrough','-Werror=comment','-Werror=missing-braces','-Werror=override-init','-Werror=enum-conversion','-Werror=enum-compare','-Werror=maybe-uninitialized',])
    
    c_args_gm+=['-DGM_SIM',]
    
    gm_files=files('tonemap_adaptor.h','tonemap_adaptor.c','gm/csc_api_funcs.c','gm/csc_api_funcs.h','gm/csc_funcs.c','gm/csc_funcs.h','gm/cs_funcs.c','gm/cs_funcs.h','gm/cvd_api_funcs.c','gm/cvd_api_funcs.h','gm/cvd_funcs.c','gm/cvd_funcs.h','gm/gm_api_funcs.c','gm/gm_api_funcs.h','gm/gm_funcs.c','gm/gm_funcs.h','gm/mat_funcs.c','gm/mat_funcs.h','ToneMapGenerator/inc/ToneMapGenerator.h','ToneMapGenerator/inc/ToneMapTypes.h','ToneMapGenerator/src/inc/AGMGenerator.h','ToneMapGenerator/src/inc/CSCGenerator.h','ToneMapGenerator/src/src/AGMGenerator.c','ToneMapGenerator/src/src/ToneMapGenerator.c',)
    
    inc_amd_gm=include_directories('gm','ToneMapGenerator/inc','ToneMapGenerator/src/inc',)
    
    libgm=static_library('libgm.a',gm_files,install=False,c_args=c_args_gm,include_directories=inc_amd_gm)
    
    ########################################################################################################################
    ### End conversion from: src/amd/gmlib/meson.build
    ########################################################################################################################
    set_relative_dir('src/amd')
    set_relative_dir('src/amd/lanczoslib')
    ########################################################################################################################
    ### Begin conversion from: src/amd/lanczoslib/meson.build
    ########################################################################################################################
    c_args_lanczos=cc.get_supported_arguments(['-Wall','-Wextra','-Wno-unused','-Wno-unused-parameter','-Wno-unused-command-line-argument','-Wno-ignored-qualifiers','-Wno-missing-field-initializers','-Wno-self-assign','-Wno-implicit-fallthrough','-Werror=comment','-Werror=missing-braces','-Werror=override-init','-Werror=enum-conversion','-Werror=enum-compare','-Werror=maybe-uninitialized',])
    
    c_args_lanczos+=[]
    
    lanczos_files=files('lanczos_adaptor.h','lanczos_adaptor.cpp','lanczosFilter/src/lanczosFilterGenerator.h','lanczosFilter/src/lanczosFilterGenerator.cpp',)
    
    inc_amd_lanczos=include_directories('lanczosFilter/src',)
    
    liblanczos=static_library('liblanczos.a',lanczos_files,install=False,c_args=c_args_lanczos,include_directories=inc_amd_lanczos)
    
    ########################################################################################################################
    ### End conversion from: src/amd/lanczoslib/meson.build
    ########################################################################################################################
    set_relative_dir('src/amd')
...
if with_any_broadcom:
  ...
  v3d_simulator_arg='-DUSE_V3D_SIMULATOR={}'.format(int(with_v3d_hw))
  ...
  if with_v3d_hw:
    ...
    for ver in v3d_versions:
      per_version_libs+=static_library('v3d-simulator-v'+ver,[files_per_version,v3d_xml_pack],include_directories=[inc_src,inc_include,inc_broadcom,],c_args=[v3d_simulator_arg,'-DV3D_VERSION='+ver],gnu_symbol_visibility='hidden',dependencies=[dep_v3d_hw,dep_libdrm,dep_valgrind],)
    
    
    libbroadcom_simulator=static_library('broadcom_simulator',[libbroadcom_simulator_files],include_directories=[inc_src,inc_include],c_args=[v3d_simulator_arg,no_override_init_args],cpp_args=[v3d_simulator_arg],gnu_symbol_visibility='hidden',dependencies=[dep_v3d_hw,dep_libdrm,dep_valgrind],link_with=[per_version_libs],build_by_default=False,)
  ...
  if with_tools.count('drm-shim') > 0:
    set_relative_dir('src/broadcom/drm-shim')
    ########################################################################################################################
    ### Begin conversion from: src/broadcom/drm-shim/meson.build
    ########################################################################################################################
    libvc4_noop_drm_shim=shared_library('vc4_noop_drm_shim','vc4_noop.c',include_directories=[inc_include,inc_src],dependencies=dep_drm_shim,gnu_symbol_visibility='hidden',install=True,)
    
    libv3d_noop_drm_shim=shared_library('v3d_noop_drm_shim','v3d_noop.c',include_directories=[inc_include,inc_src],dependencies=dep_drm_shim,gnu_symbol_visibility='hidden',install=True,)
    
    ########################################################################################################################
    ### End conversion from: src/broadcom/drm-shim/meson.build
    ########################################################################################################################
    set_relative_dir('src/broadcom')
  ...
  libv3d_neon=static_library('v3d_neon','common/v3d_tiling.c',include_directories=[inc_src,inc_include,inc_broadcom,],c_args=[v3d_args,v3d_neon_c_args],gnu_symbol_visibility='hidden',dependencies=[dep_libdrm,dep_valgrind,idep_nir_headers,idep_mesautil],)
  
  libbroadcom_v3d=static_library('broadcom_v3d',[files('common/v3d_debug.c','common/v3d_device_info.c','clif/clif_dump.c','common/v3d_util.c'),v3d_xml_pack,],include_directories=[inc_include,inc_src,inc_broadcom],c_args=[no_override_init_args],gnu_symbol_visibility='hidden',link_whole=v3d_libs+per_version_libs,link_with=[libv3d_neon],build_by_default=False,dependencies=[dep_valgrind,dep_thread,idep_mesautil,dep_libdrm],)
  
  set_relative_dir('src/broadcom/perfcntrs')
  ########################################################################################################################
  ### Begin conversion from: src/broadcom/perfcntrs/meson.build
  ########################################################################################################################
  libbroadcom_perfcntrs_files=files('v3d_perfcntrs.c',)
  
  files_per_version=files('v3dx_counter.c',)
  
  per_version_libs=[]
  for ver in v3d_versions:
    per_version_libs+=static_library('v3d-perfcntrs-v'+ver,[files_per_version,v3d_xml_pack],include_directories=[inc_src,inc_include,inc_broadcom,],c_args=[v3d_simulator_arg,'-DV3D_VERSION='+ver],gnu_symbol_visibility='hidden',dependencies=[dep_v3d_hw,dep_libdrm,dep_valgrind,idep_mesautil],)
  
  
  libbroadcom_perfcntrs=static_library('broadcom_perfcntrs',[libbroadcom_perfcntrs_files],include_directories=[inc_src,inc_include,inc_broadcom],c_args=[no_override_init_args],gnu_symbol_visibility='hidden',dependencies=[dep_v3d_hw,dep_libdrm,dep_valgrind,idep_mesautil],link_with=[per_version_libs],build_by_default=False,)
  
  idep_broadcom_perfcntrs=declare_dependency(link_with=[libbroadcom_perfcntrs,],include_directories=[inc_src,],)
  
  v3d_libs+=libbroadcom_perfcntrs
  
  ########################################################################################################################
  ### End conversion from: src/broadcom/perfcntrs/meson.build
  ########################################################################################################################
  set_relative_dir('src/broadcom')
  
  if with_perfetto and (with_datasources.count('v3d') > 0 or with_datasources.count('auto') > 0):
    set_relative_dir('src/broadcom/ds')
    ########################################################################################################################
    ### Begin conversion from: src/broadcom/ds/meson.build
    ########################################################################################################################
    pps_broadcom_lib=static_library('broadcom-dds',sources=['v3d_pps_driver.cc','v3d_pps_driver.h',],include_directories=[inc_tool,inc_broadcom,inc_include,],dependencies=[dep_libdrm,dep_perfetto,idep_broadcom_perfcntrs,],c_args=[no_override_init_args],)
    
    pps_broadcom_dep=declare_dependency(link_with=[pps_broadcom_lib,libbroadcom_v3d,],include_directories=[inc_tool,],compile_args=['-DPPS_V3D',],)
    
    pps_datasources+=pps_broadcom_dep
    pps_includes+=[inc_include,inc_broadcom,]
    
    ########################################################################################################################
    ### End conversion from: src/broadcom/ds/meson.build
    ########################################################################################################################
    set_relative_dir('src/broadcom')
  ...
  if with_broadcom_vk:
    ...
    libv3dv_files=files('v3dv_bo.c','v3dv_cl.c','v3dv_cmd_buffer.c','v3dv_descriptor_set.c','v3dv_device.c','v3dv_event.c','v3dv_formats.c','v3dv_image.c','v3dv_limits.h','v3dv_meta_clear.c','v3dv_meta_copy.c','v3dv_pass.c','v3dv_pipeline.c','v3dv_pipeline_cache.c','v3dv_private.h','v3dv_query.c','v3dv_queue.c','v3dv_uniforms.c','v3dv_wsi.c',)+[v3d_xml_pack]
    
    files_per_version=files('v3dvx_cmd_buffer.c','v3dvx_descriptor_set.c','v3dvx_device.c','v3dvx_formats.c','v3dvx_image.c','v3dvx_pipeline.c','v3dvx_meta_common.c','v3dvx_pipeline.c','v3dvx_queue.c',)
    
    v3d_versions=['42','71']
    
    v3dv_flags=[v3d_simulator_arg]
    
    v3dv_deps=[dep_dl,dep_libdrm,dep_valgrind,dep_v3d_hw,idep_broadcom_perfcntrs,idep_nir,idep_nir_headers,idep_vulkan_util,idep_vulkan_runtime,idep_vulkan_wsi,idep_mesautil,]
    ...
    broadcom_icd=custom_target('broadcom_icd',input=[vk_icd_gen,vk_api_xml],output='broadcom_icd.{}.json'.format(host_machine.cpu()),command=[prog_python,'@INPUT0@','--api-version','1.3','--xml','@INPUT1@','--sizeof-pointer',sizeof_pointer,'--lib-path',join_paths(get_option('prefix'),get_option('libdir'),'libvulkan_broadcom.so'),'--out','@OUTPUT@',],build_by_default=True,install_dir=with_vulkan_icd_dir,install_tag='runtime',install=True,)
    
    _dev_icdname='broadcom_devenv_icd.{}.json'.format(host_machine.cpu())
    _dev_icd=custom_target('broadcom_devenv_icd',input=[vk_icd_gen,vk_api_xml],output=_dev_icdname,command=[prog_python,'@INPUT0@','--api-version','1.3','--xml','@INPUT1@','--sizeof-pointer',sizeof_pointer,'--lib-path',(meson.current_build_dir() + 'libvulkan_broadcom.so') if isinstance(meson.current_build_dir(), str) else (meson.current_build_dir() / 'libvulkan_broadcom.so'),'--out','@OUTPUT@',],build_by_default=True,)
...
if with_gallium_etnaviv:
  ...
  libetnaviv_drm=static_library('etnaviv_drm',libetnaviv_drm_files,include_directories=[inc_etnaviv,inc_include,inc_src,],c_args=[no_override_init_args],gnu_symbol_visibility='hidden',dependencies=[dep_libdrm,dep_valgrind,idep_mesautil,idep_etna_common,],link_with=[libetnaviv_hwdb],build_by_default=False,)
  ...
  etnaviv_enums_h=custom_target('enums.h',input='etnaviv.xml',output='enums.h',command=[prog_python,prog_enums_h,'--xml','@INPUT@','--output','@OUTPUT@'],depend_files=[isa_depend_files,prog_enums_h],)
  
  idep_etnaviv_enums_h=declare_dependency(sources=[etnaviv_enums_h],include_directories=include_directories('.'),)
  ...
  encode_files=[etnaviv_enums_h,encode_h,'encode.c',]
  ...
  if with_tools.count('etnaviv') > 0:
    isa_bindings_rs=rust.bindgen(input=['bindings.h',etnaviv_enums_h],output='isa_bindings.rs',include_directories=[inc_src],args=[bindgen_output_args,'--disable-header-comment','--ignore-functions','--allowlist-type','etna_asm_result','--allowlist-type','etna_inst','--allowlist-type','isa_swiz','--default-enum-style','rust','--with-derive-default',],)
  
    _libetnaviv_isa_bindings_gen=static_library('isa_bindings',isa_bindings_rs,gnu_symbol_visibility='hidden',rust_abi='rust',)
  
    dep_indexmap=dependency('indexmap',version='>= 2.2.6',fallback=['indexmap-2-rs','dep_indexmap'],required=True,)
  
    dep_roxmltree=dependency('roxmltree',version='>= 0.20.roxmltree',fallback=['roxmltree-0.20-rs','dep_roxmltree'],required=True,)
  
    dep_syn=dependency('syn',version='>= 2.0.15',fallback=['syn-2-rs','dep_syn'],required=True,)
  
    dep_pest_generator=dependency('pest_generator',version='>= 2.7.6',fallback=['pest_generator-2-rs','dep_pest_generator'],required=True,)
  
    dep_pest=dependency('pest',version='>= 2.7.6',fallback=['pest-2-rs','dep_pest'],required=True,)
  ...
  ########################################################################################################################
  test('etnaviv_isa_disasm',executable('etnaviv_disasm','disasm.cpp',cpp_args=[pre_args,],include_directories=[inc_etnaviv,inc_src],dependencies=[idep_gtest,idep_etnaviv_enums_h,idep_libetnaviv_decode],link_with=[libetnaviv_isa_rs,libetnaviv_encode],))
  ...
  if with_tools.count('drm-shim') > 0:
    set_relative_dir('src/etnaviv/drm-shim')
    ########################################################################################################################
    ### Begin conversion from: src/etnaviv/drm-shim/meson.build
    ########################################################################################################################
    libetnaviv_noop_drm_shim=shared_library('etnaviv_noop_drm_shim','etnaviv_noop.c',include_directories=[inc_include,inc_src],dependencies=[dep_drm_shim,idep_mesautil],gnu_symbol_visibility='hidden',install=True,)
    
    ########################################################################################################################
    ### End conversion from: src/etnaviv/drm-shim/meson.build
    ########################################################################################################################
    set_relative_dir('src/etnaviv')
...
if with_gallium_freedreno or with_freedreno_vk or with_tools.count('freedreno') > 0:
  ...
  ########################################################################################################################
  xml_reg_files=['a2xx.xml','a3xx.xml','a4xx.xml','a5xx.xml','a6xx.xml','a6xx_enums.xml','a6xx_descriptors.xml','a6xx_perfcntrs.xml','a7xx_enums.xml','a7xx_perfcntrs.xml',]
  ...
  ir3_nir_triop_bitwise_c=custom_target('ir3_nir_triop_bitwise.c',input='ir3_nir_triop_bitwise.py',output='ir3_nir_triop_bitwise.c',command=[prog_python,'@INPUT@','-p',dir_compiler_nir,],capture=True,depend_files=nir_algebraic_depends,)
  ...
  ########################################################################################################################
  libfreedreno_layout=static_library('freedreno_layout',['fd5_layout.c','fd6_format_table.c','fd6_layout.c','fd6_tiled_memcpy.cc','fd6_view.c','freedreno_layout.c','freedreno_lrz_layout.c',freedreno_xml_header_files,],include_directories=[inc_freedreno,inc_include,inc_src],c_args=[no_override_init_args],gnu_symbol_visibility='hidden',dependencies=[idep_nir_headers,idep_mesautil,idep_libfreedreno_common],build_by_default=False,)
  
  for major in ['5','6']:
    test('fd'+major+'_layout',executable('fd'+major+'_layout',['fd_layout_test.c','fd'+major+'_layout_test.c',freedreno_xml_header_files,],link_with=libfreedreno_layout,dependencies=[idep_mesautil,idep_libfreedreno_common],include_directories=[inc_include,inc_src,inc_freedreno],),suite=['freedreno'],)
  ...
  if system_has_kms_drm:
    ...
    libfreedreno_drm=static_library('freedreno_drm',[libfreedreno_drm_files,freedreno_xml_header_files,],include_directories=libfreedreno_drm_includes,c_args=[no_override_init_args,libfreedreno_drm_flags],gnu_symbol_visibility='hidden',dependencies=libfreedreno_drm_deps,link_with=libfreedreno_drm_link_with,build_by_default=False,)
  ...
  if system_has_kms_drm:
    ...
    computerator_cpp_args=cpp.get_supported_arguments(['-Wno-array-bounds','-Wno-sign-compare',])
  ...
  if system_has_kms_drm and with_perfetto and (with_datasources.count('freedreno') > 0 or with_datasources.count('auto') > 0):
  ...
  if dep_libxml2.found() and with_tools.count('freedreno') > 0:
    ...
    if dep_lua.found() and dep_libarchive.found():
      ...
      if with_tests:
        ...
        for cffdump_test in cffdump_tests:
          ...
          log=custom_target(name+'.log',output=name+'.log',command=[cffdump,'--unit-test',args,files('../tests/traces/'+name+'.rd.gz')],capture=True,)
          test('cffdump-'+name,diff,args=['-u',files('../tests/reference/'+name+'.log'),log],suite='freedreno',workdir=dir_source_root)
    ...
    dumpregs=executable('dumpregs',['dumpregs.c'],include_directories=[inc_freedreno,inc_freedreno_rnn,inc_include,inc_src,],gnu_symbol_visibility='hidden',dependencies=[],link_with=[libfreedreno_cffdec,],build_by_default=with_tools.count('freedreno') > 0,install=False,)
    ...
    if with_tests:
      ...
      for crashdec_test in crashdec_tests:
        ...
        log=custom_target(name+'.log',output=name+'.log',command=[crashdec,args,files('../tests/traces/'+name+'.devcore')],capture=True,env={'GALLIUM_DUMP_CPU':'false'},)
    
        test('crashdec-'+name,diff,args=['-u',files('../tests/reference/'+name+'.log'),log],suite='freedreno',workdir=dir_source_root)
    ...
    if with_tests:
      asm_fw=custom_target('afuc_test.fw',output='afuc_test.fw',command=[asm,files('../tests/traces/afuc_test.asm'),'@OUTPUT@'],)
      asm_fw_a7xx=custom_target('afuc_test_a7xx.fw',output='afuc_test_a7xx.fw',command=[asm,files('../tests/traces/afuc_test_a7xx.asm'),'@OUTPUT@'],)
      test('afuc-asm',diff,args=['-u',files('../tests/reference/afuc_test.fw'),asm_fw],suite='freedreno',workdir=dir_source_root)
      test('afuc-asm-a7xx',diff,args=['-u',files('../tests/reference/afuc_test_a7xx.fw'),asm_fw_a7xx],suite='freedreno',workdir=dir_source_root)
    
    
    afuc_isa=custom_target('afuc-isa',input=['afuc.xml'],output=['afuc-isa.c','afuc-isa.h'],command=[prog_isaspec_decode,'--xml','@INPUT@','--out-c','@OUTPUT0@','--out-h','@OUTPUT1@',],)
    ...
    if cc.sizeof('size_t')>4:
      ...
      if with_tests:
        disasm_fw=custom_target('afuc_test.asm',output='afuc_test.asm',command=[disasm,'-u',files('../tests/reference/afuc_test.fw')],capture=True)
        disasm_fw_a7xx=custom_target('afuc_test_a7xx.asm',output='afuc_test_a7xx.asm',command=[disasm,'-u',files('../tests/reference/afuc_test_a7xx.fw')],capture=True)
        test('afuc-disasm',diff,args=['-u',files('../tests/reference/afuc_test.asm'),disasm_fw],suite='freedreno',workdir=dir_source_root)
        test('afuc-disasm-a7xx',diff,args=['-u',files('../tests/reference/afuc_test_a7xx.asm'),disasm_fw_a7xx],suite='freedreno',workdir=dir_source_root)
  ...
  if with_tools.count('drm-shim') > 0:
    set_relative_dir('src/freedreno/drm-shim')
    ########################################################################################################################
    ### Begin conversion from: src/freedreno/drm-shim/meson.build
    ########################################################################################################################
    libfreedreno_noop_drm_shim=shared_library('freedreno_noop_drm_shim','freedreno_noop.c',include_directories=[inc_include,inc_src],dependencies=dep_drm_shim,gnu_symbol_visibility='hidden',install=True,)
    
    ########################################################################################################################
    ### End conversion from: src/freedreno/drm-shim/meson.build
    ########################################################################################################################
    set_relative_dir('src/freedreno')
  ...
  if with_freedreno_vk:
    ...
    libtu_files=files('bvh/tu_bvh.h','bvh/tu_build_interface.h','layers/tu_rmv_layer.cc','tu_acceleration_structure.cc','tu_autotune.cc','tu_buffer.cc','tu_buffer_view.cc','tu_clear_blit.cc','tu_cmd_buffer.cc','tu_cs_breadcrumbs.cc','tu_cs.cc','tu_device.cc','tu_descriptor_set.cc','tu_dynamic_rendering.cc','tu_event.cc','tu_formats.cc','tu_image.cc','tu_knl.cc','tu_lrz.cc','tu_nir_lower_multiview.cc','tu_nir_lower_ray_query.cc','tu_pass.cc','tu_pipeline.cc','tu_sampler.cc','tu_query_pool.cc','tu_queue.cc','tu_rmv.cc','tu_shader.cc','tu_suballoc.cc','tu_util.cc',)
    
    set_relative_dir('src/freedreno/vulkan/bvh')
    ########################################################################################################################
    ### Begin conversion from: src/freedreno/vulkan/bvh/meson.build
    ########################################################################################################################
    bvh_shaders=[['encode.comp','encode',[],],['header.comp','header',[],],['copy.comp','copy',[]],]
    
    tu_bvh_include_dir=dir_source_root+'/src/freedreno/vulkan/bvh'
    
    tu_bvh_includes=files('tu_build_helpers.h','tu_build_interface.h','tu_bvh.h',)
    
    bvh_spv=[]
    for s in bvh_shaders:
      command=[prog_glslang,'-V','-I'+vk_bvh_include_dir,'-I'+tu_bvh_include_dir,'--target-env','spirv1.5','-x','-o','@OUTPUT@','@INPUT@']
      command+=glslang_quiet
      ...
      bvh_spv+=custom_target(s[1]+'.spv.h',input=s[0],output=s[1]+'.spv.h',command=command,depend_files=[vk_bvh_includes,tu_bvh_includes],)
    
    
    ########################################################################################################################
    ### End conversion from: src/freedreno/vulkan/bvh/meson.build
    ########################################################################################################################
    set_relative_dir('src/freedreno/vulkan')
    ...
    tu_cpp_args=[]
    tu_cpp_args+=cpp.get_supported_arguments(['-fno-exceptions','-fno-rtti','-Wno-address-of-temporary','-Wno-array-bounds','-Wno-c++11-narrowing','-Wno-c99-designator','-Wno-class-memaccess','-Wno-missing-braces','-Wno-narrowing','-Wno-pointer-arith','-Wno-reorder-init-list','-Wno-sign-compare','-Wno-switch','-Wno-unused-function','-Wno-vla-cxx-extension','-Wno-writable-strings','-Wno-write-strings',])
    ...
    freedreno_icd=custom_target('freedreno_icd',input=[vk_icd_gen,vk_api_xml],output='freedreno_icd.{}.json'.format(host_machine.cpu()),command=[prog_python,'@INPUT0@','--api-version','1.4','--xml','@INPUT1@','--sizeof-pointer',sizeof_pointer,'--lib-path',join_paths(get_option('prefix'),get_option('libdir'),'libvulkan_freedreno.so'),'--out','@OUTPUT@',],build_by_default=True,install_dir=with_vulkan_icd_dir,install_tag='runtime',install=True,)
    
    _dev_icdname='freedreno_devenv_icd.{}.json'.format(host_machine.cpu())
    _dev_icd=custom_target('freedreno_devenv_icd',input=[vk_icd_gen,vk_api_xml],output=_dev_icdname,command=[prog_python,'@INPUT0@','--api-version','1.4','--xml','@INPUT1@','--sizeof-pointer',sizeof_pointer,'--lib-path',(meson.current_build_dir() + 'libvulkan_freedreno.so') if isinstance(meson.current_build_dir(), str) else (meson.current_build_dir() / 'libvulkan_freedreno.so'),'--out','@OUTPUT@',],build_by_default=True,)
...
if with_imagination_vk:
  ...
  if with_imagination_vk:
    ...
    ########################################################################################################################
    set_relative_dir('src/imagination')
    set_relative_dir('src/imagination/pco')
    ########################################################################################################################
    ### Begin conversion from: src/imagination/pco/meson.build
    ########################################################################################################################
    inc_powervr_compiler=include_directories(['.'])
    
    libpowervr_compiler_files=files('pco.c','pco_binary.c','pco_const_imms.c','pco_debug.c','pco_end.c','pco_group_instrs.c','pco_index.c','pco_ir.c','pco_legalize.c','pco_nir.c','pco_nir_pvfio.c','pco_nir_vk.c','pco_opt.c','pco_print.c','pco_ra.c','pco_schedule.c','pco_trans_nir.c','pco_validate.c',)
    
    pco_pygen_dep_files=files('pco_pygen_common.py','pco_isa.py','pco_ops.py','pco_map.py')
    
    pco_builder_ops_h=custom_target('pco_builder_ops.h',input=['pco_builder_ops.h.py'],output='pco_builder_ops.h',command=[prog_python,'@INPUT@'],capture=True,depend_files=pco_pygen_dep_files,)
    
    pco_common_h=custom_target('pco_common.h',input=['pco_common.h.py'],output='pco_common.h',command=[prog_python,'@INPUT@'],capture=True,depend_files=pco_pygen_dep_files,)
    
    pco_info_c=custom_target('pco_info.c',input=['pco_info.c.py'],output='pco_info.c',command=[prog_python,'@INPUT@'],capture=True,depend_files=pco_pygen_dep_files,)
    
    pco_isa_h=custom_target('pco_isa.h',input=['pco_isa.h.py'],output='pco_isa.h',command=[prog_python,'@INPUT@'],capture=True,depend_files=pco_pygen_dep_files,)
    
    pco_map_h=custom_target('pco_map.h',input=['pco_map.h.py'],output='pco_map.h',command=[prog_python,'@INPUT@'],capture=True,depend_files=pco_pygen_dep_files,)
    
    pco_ops_h=custom_target('pco_ops.h',input=['pco_ops.h.py'],output='pco_ops.h',command=[prog_python,'@INPUT@'],capture=True,depend_files=pco_pygen_dep_files,)
    
    idep_pco_pygen=declare_dependency(sources=[pco_builder_ops_h,pco_common_h,pco_info_c,pco_isa_h,pco_map_h,pco_ops_h,],include_directories=inc_powervr_compiler,)
    
    pco_pygen_c_files=[pco_info_c]
    
    pco_nir_algebraic_c=custom_target('pco_nir_algebraic.c',input='pco_nir_algebraic.py',output='pco_nir_algebraic.c',command=[prog_python,'@INPUT@','-p',dir_compiler_nir,],capture=True,depend_files=nir_algebraic_depends,)
    
    libpowervr_compiler=static_library('powervr_compiler',[libpowervr_compiler_files,pco_pygen_c_files,pco_nir_algebraic_c],include_directories=[inc_imagination,inc_powervr_compiler,inc_include,inc_compiler,inc_src,],c_args=[imagination_c_args,no_override_init_args,'-Wno-psabi'],dependencies=[idep_mesautil,idep_nir,idep_pco_pygen],gnu_symbol_visibility='hidden',install=False,)
    
    ########################################################################################################################
    ### End conversion from: src/imagination/pco/meson.build
    ########################################################################################################################
    set_relative_dir('src/imagination')
    ...
    pvr_includes=[include_directories('winsys'),libpowervr_pds_includes,]
    ...
    libvulkan_powervr_mesa=shared_library('vulkan_powervr_mesa',[pvr_files,pvr_entrypoints,sha1_h,u_format_pack_h],include_directories=[pvr_includes,inc_imagination,inc_include,inc_src,],link_with=[libpowervr_common,libpowervr_compiler,libpowervr_pds,libpowervr_rogue,libvulkan_wsi,],dependencies=[pvr_deps,idep_nir_headers,],c_args=pvr_flags,link_args=[ld_args_build_id,ld_args_bsymbolic,ld_args_gc_sections],gnu_symbol_visibility='hidden',install=True,)
    ...
    powervr_mesa_icd=custom_target('powervr_mesa_icd',input=[vk_icd_gen,vk_api_xml],output='powervr_mesa_icd.{}.json'.format(host_machine.cpu()),command=[prog_python,'@INPUT0@','--api-version','1.0','--xml','@INPUT1@','--sizeof-pointer',sizeof_pointer,'--lib-path',((get_option('prefix') + get_option('libdir')) if isinstance(get_option('prefix'), str) else (get_option('prefix') / get_option('libdir')) + 'libvulkan_powervr_mesa.so') if isinstance((get_option('prefix') + get_option('libdir')) if isinstance(get_option('prefix'), str) else (get_option('prefix') / get_option('libdir')), str) else ((get_option('prefix') + get_option('libdir')) if isinstance(get_option('prefix'), str) else (get_option('prefix') / get_option('libdir')) / 'libvulkan_powervr_mesa.so'),'--out','@OUTPUT@',],build_by_default=True,install_dir=with_vulkan_icd_dir,install_tag='runtime',install=True,)
    
    _dev_icd=custom_target('powervr_mesa_devenv_icd',input=[vk_icd_gen,vk_api_xml],output='powervr_mesa_devenv_icd.{}.json'.format(host_machine.cpu()),command=[prog_python,'@INPUT0@','--api-version','1.0','--xml','@INPUT1@','--sizeof-pointer',sizeof_pointer,'--lib-path',(meson.current_build_dir() + 'libvulkan_powervr_mesa.so') if isinstance(meson.current_build_dir(), str) else (meson.current_build_dir() / 'libvulkan_powervr_mesa.so'),'--out','@OUTPUT@',],build_by_default=True,)
...
if with_gallium_panfrost or with_gallium_lima or with_panfrost_vk or with_tools.count('panfrost') > 0:
  ...
  ########################################################################################################################
  inc_panfrost=include_directories(['.','shared','midgard','compiler','lib','libpan',])
  ...
  if with_tests:
    test('panfrost_tiling',executable('panfrost_tiling',files('test/test-tiling.cpp',),cpp_args=cpp_msvc_compat_args,gnu_symbol_visibility='hidden',include_directories=[inc_include,inc_src,inc_panfrost],dependencies=[idep_gtest,idep_mesautil],link_with=[libpanfrost_shared],),suite=['panfrost'],protocol='gtest',)
  ...
  ########################################################################################################################
  libpanfrost_util_files=files('pan_collect_varyings.c','pan_ir.c','pan_ir.h','pan_lower_frag_coord_zw.c','pan_lower_framebuffer.c','pan_lower_helper_invocation.c','pan_lower_image_index.c','pan_lower_image_ms.c','pan_lower_noperspective.c','pan_lower_sample_position.c','pan_lower_store_component.c','pan_lower_vertex_id.c','pan_lower_writeout.c','pan_lower_xfb.c',)
  
  libpanfrost_util=static_library('panfrost_util',[libpanfrost_util_files],include_directories=[inc_include,inc_src],dependencies=[idep_nir,idep_mesautil],c_args=[no_override_init_args],gnu_symbol_visibility='hidden',build_by_default=False,)
  ...
  libpanfrost_midgard_disasm=static_library('panfrost_midgard_disasm',['disassemble.c','midgard_ops.c','midgard_print_constant.c'],include_directories=[inc_include,inc_src],c_args=[no_override_init_args],gnu_symbol_visibility='hidden',build_by_default=False,)
  
  libpanfrost_midgard=static_library('panfrost_midgard',[libpanfrost_midgard_files,midgard_nir_algebraic_c],include_directories=[inc_include,inc_src],dependencies=[idep_nir,idep_mesautil,],link_with=[libpanfrost_util,libpanfrost_midgard_disasm],c_args=[no_override_init_args],gnu_symbol_visibility='hidden',build_by_default=False,)
  ...
  bifrost_gen_disasm_c=custom_target('bifrost_gen_disasm.c',input=['gen_disasm.py','bifrost/ISA.xml'],output='bifrost_gen_disasm.c',command=[prog_python,'@INPUT@'],capture=True,depend_files=files('bifrost_isa.py'),)
  
  bi_opcodes_c=custom_target('bi_opcodes.c',input=['bi_opcodes.c.py','IR_pseudo.xml','bifrost/ISA.xml','valhall/ISA.xml'],output='bi_opcodes.c',command=[prog_python,'@INPUT@'],capture=True,depend_files=files('bifrost_isa.py'),)
  
  bi_swizzles_c=custom_target('bi_swizzles.c',input=['bi_swizzles.c.py','IR_pseudo.xml','bifrost/ISA.xml'],output='bi_swizzles.c',command=[prog_python,'@INPUT@'],capture=True,depend_files=files('bifrost_isa.py'),)
  
  bi_printer_c=custom_target('bi_printer.c',input=['bi_printer.c.py','IR_pseudo.xml','bifrost/ISA.xml','valhall/ISA.xml'],output='bi_printer.c',command=[prog_python,'@INPUT@'],capture=True,depend_files=files('bifrost_isa.py'),)
  
  bi_packer_c=custom_target('bi_packer.c',input=['bi_packer.c.py','IR_pseudo.xml','bifrost/ISA.xml','valhall/ISA.xml'],output='bi_packer.c',command=[prog_python,'@INPUT@'],capture=True,depend_files=files('bifrost_isa.py'),)
  
  bi_opcodes_h=custom_target('bi_opcodes.h',input=['bi_opcodes.h.py','IR_pseudo.xml','bifrost/ISA.xml','valhall/ISA.xml'],output='bi_opcodes.h',command=[prog_python,'@INPUT@'],capture=True,depend_files=files('bifrost_isa.py'),)
  
  idep_bi_opcodes_h=declare_dependency(sources=[bi_opcodes_h],include_directories=include_directories('.'),)
  
  bi_builder_h=custom_target('bi_builder.h',input=['bi_builder.h.py','IR_pseudo.xml','bifrost/ISA.xml','valhall/ISA.xml'],output='bi_builder.h',command=[prog_python,'@INPUT@'],capture=True,depend_files=files('bifrost_isa.py'),)
  ...
  if with_gallium_panfrost or with_panfrost_vk or with_tools.count('panfrost') > 0:
    set_relative_dir('src/panfrost/genxml')
    ########################################################################################################################
    ### Begin conversion from: src/panfrost/genxml/meson.build
    ########################################################################################################################
    pan_packers=[]
    for packer in ['common','v4','v5','v6','v7','v9','v10','v12','v13']:
    ...
    for ver in ['4','5','6','7','9','10','12','13']:
      libpanfrost_decode_per_arch+=static_library('pandecode-arch-v'+ver,['decode.c','decode_jm.c','decode_csf.c',pan_packers],include_directories=[inc_include,inc_src,inc_panfrost],dependencies=[dep_libdrm,idep_pan_packers,idep_nir,idep_mesautil],c_args=[no_override_init_args,'-DPAN_ARCH='+ver],gnu_symbol_visibility='hidden',build_by_default=False,)
    ...
    if with_tests:
      test('cs_builder_tests',executable('cs_builder_tests',['test/cs_builder.cpp'],cpp_args=[cpp_msvc_compat_args,'-DPAN_ARCH=10'],gnu_symbol_visibility='hidden',include_directories=[inc_include,inc_src,inc_panfrost],dependencies=[idep_gtest,idep_pan_packers,idep_mesautil],),suite=['panfrost'],protocol='gtest',)
    
    
    ########################################################################################################################
    ### End conversion from: src/panfrost/genxml/meson.build
    ########################################################################################################################
    set_relative_dir('src/panfrost')
    ...
    for ver in pixel_format_versions:
      libpanfrost_pixel_format+=static_library('pan-format-v'+ver,['pan_format.c',pan_packers],include_directories=[inc_src,inc_include,inc_panfrost],c_args=['-DPAN_ARCH='+ver],gnu_symbol_visibility='hidden',dependencies=[deps_for_libpanfrost,dep_valgrind],)
    
    
    libpanfrost_per_arch=[]
    
    for ver in ['4','5','6','7','9','10','12','13']:
      libpanfrost_per_arch+=static_library('pan-arch-v'+ver,['pan_blend.c','pan_desc.c','pan_mod.c','pan_texture.c',],include_directories=[inc_include,inc_src,inc_panfrost],c_args=['-DPAN_ARCH='+ver],gnu_symbol_visibility='hidden',dependencies=[deps_for_libpanfrost,idep_nir],)
    
    
    libpanfrost_lib_files=files('pan_encoder.h','pan_blend.c','pan_clear.c','pan_earlyzs.c','pan_samples.c','pan_shader.c','pan_tiler.c','pan_layout.c','pan_scratch.c','pan_props.c','pan_util.c','pan_afbc.c',)
    
    libpanfrost_lib=static_library('panfrost_lib',[libpanfrost_lib_files,pan_packers],include_directories=[inc_include,inc_src,inc_panfrost],c_args=[no_override_init_args],gnu_symbol_visibility='hidden',dependencies=[dep_libdrm,idep_nir,idep_mesautil],build_by_default=False,link_with=[libpanfrost_pixel_format,libpanfrost_per_arch,libpankmod_lib],)
    
    libpanfrost_dep=declare_dependency(link_with=[libpanfrost_lib,libpanfrost_decode,libpanfrost_midgard,libpanfrost_bifrost,libpanfrost_pixel_format,libpanfrost_per_arch],include_directories=[inc_include,inc_src,inc_panfrost],dependencies=[deps_for_libpanfrost,libpankmod_dep,idep_nir],)
    
    if with_tests:
      test('panfrost_attribute',executable('panfrost_attribute_test',files('tests/test-attribute.c'),c_args=[c_msvc_compat_args,no_override_init_args],gnu_symbol_visibility='hidden',include_directories=[inc_include,inc_src],dependencies=[libpanfrost_dep],),suite=['panfrost'],)
      ...
      test('panfrost_tests',executable('panfrost_tests',files('tests/test-earlyzs.cpp','tests/test-layout.cpp',),cpp_args=[cpp_msvc_compat_args,no_override_init_args],gnu_symbol_visibility='hidden',include_directories=[inc_include,inc_src,inc_panfrost],dependencies=[idep_gtest,libpanfrost_dep],),suite=['panfrost'],protocol='gtest',)
    ...
    ########################################################################################################################
    set_relative_dir('src/panfrost')
    set_relative_dir('src/panfrost/clc')
    ########################################################################################################################
    ### Begin conversion from: src/panfrost/clc/meson.build
    ########################################################################################################################
    if get_option('precomp-compiler')=='system':
      prog_panfrost_compile=find_program('panfrost_compile',native=True)
    else:
      prog_panfrost_compile=executable('panfrost_compile',['pan_compile.c'],link_with=[libpanfrost_bifrost],include_directories=[inc_include,inc_src],c_args=[c_msvc_compat_args,no_override_init_args],link_args=[ld_args_build_id],dependencies=[idep_vtn,idep_nir,idep_mesautil,libpanfrost_dep],native=not meson.can_run_host_binaries(),install=get_option('install-precomp-compiler'),)
    
    
    ########################################################################################################################
    ### End conversion from: src/panfrost/clc/meson.build
    ########################################################################################################################
    set_relative_dir('src/panfrost')
  
  
  if with_gallium_panfrost or with_panfrost_vk:
    set_relative_dir('src/panfrost/libpan')
    ########################################################################################################################
    ### Begin conversion from: src/panfrost/libpan/meson.build
    ########################################################################################################################
    libpan_shader_files=files('query_pool.cl','indirect_dispatch.cl',)
    
    idep_libpan_per_arch={}
    
    for ver in ['4','5','6','7','9','10','12','13']:
      libpan_spv=custom_target(input=libpan_shader_files,output='libpan_v'+ver+'.spv',command=[prog_mesa_clc,'-o','@OUTPUT@','--depfile','@DEPFILE@',libpan_shader_files,'--','-DPAN_ARCH={}'.format(ver),'-I'+join_paths(meson.project_source_root(),'include'),'-I'+join_paths(meson.project_source_root(),'src/compiler/libcl'),'-I'+join_paths(meson.current_source_dir(),'.'),'-I'+join_paths(meson.current_source_dir(),'../../'),'-I'+join_paths(meson.current_source_dir(),'../'),'-I'+join_paths(meson.current_build_dir(),'../'),cl_args,],depends=[pan_packers],depfile='libpan_shaders_v'+ver+'.h.d',)
    
      libpan_nir_cur_ver=custom_target(input=libpan_spv,output=['libpan_v'+ver+'.cpp','libpan_v'+ver+'.h'],command=[prog_vtn_bindgen2,libpan_spv,'@OUTPUT@'],)
    
      libpan_shaders_cur_ver=custom_target(input=libpan_spv,output=['libpan_shaders_v'+ver+'.h','libpan_shaders_v'+ver+'.c'],command=[prog_panfrost_compile,'libpan_shaders',ver,'@INPUT0@','@OUTPUT@'],)
    
      idep_libpan_cur_ver=declare_dependency(sources=[libpan_shaders_cur_ver,libpan_nir_cur_ver],include_directories=include_directories('.'),)
      idep_libpan_per_arch |= {ver:idep_libpan_cur_ver}
    
    
    
    ########################################################################################################################
    ### End conversion from: src/panfrost/libpan/meson.build
    ########################################################################################################################
    set_relative_dir('src/panfrost')
    ...
    panfrost_quick=executable('panquick','quick.c',include_directories=[inc_include,inc_src,inc_panfrost],dependencies=[dep_libdrm,libpanfrost_dep,dep_panfrost_perf,],build_by_default=with_tools.count('panfrost') > 0)
  ...
  bifrost_compiler=executable('bifrost_compiler',[files_bifrost],include_directories=[inc_include,inc_src,inc_panfrost,],dependencies=[idep_nir,idep_mesautil,idep_bi_opcodes_h,dep_libdrm,],link_with=[libpanfrost_bifrost,],build_by_default=with_tools.count('panfrost') > 0)
  ...
  if with_panfrost_vk:
    ...
    ########################################################################################################################
    panvk_entrypoints=custom_target('panvk_entrypoints.[ch]',input=[vk_entrypoints_gen,vk_api_xml],output=['panvk_entrypoints.h','panvk_entrypoints.c'],command=[prog_python,'@INPUT0@','--xml','@INPUT1@','--proto','--weak','--out-h','@OUTPUT0@','--out-c','@OUTPUT1@','--prefix','panvk','--device-prefix','panvk_v6','--device-prefix','panvk_v7','--device-prefix','panvk_v9','--device-prefix','panvk_v10','--device-prefix','panvk_v12','--device-prefix','panvk_v13','--beta',str(with_vulkan_beta)],depend_files=vk_entrypoints_gen_depend_files,)
    
    panvk_tracepoints=custom_target('panvk_tracepoints.[ch]',input='panvk_tracepoints.py',output=['panvk_tracepoints.h','panvk_tracepoints_perfetto.h','panvk_tracepoints.c'],command=[prog_python,'@INPUT@','--import-path',join_paths(dir_source_root,'src/util/perf/'),'--utrace-hdr','@OUTPUT0@','--perfetto-hdr','@OUTPUT1@','--utrace-src','@OUTPUT2@',],depend_files=u_trace_py,)
    
    libpanvk_files=files('panvk_buffer.c','panvk_cmd_pool.c','panvk_device_memory.c','panvk_host_copy.c','panvk_image.c','panvk_instance.c','panvk_mempool.c','panvk_physical_device.c','panvk_priv_bo.c','panvk_utrace.c','panvk_wsi.c',)
    ...
    bifrost_files=['bifrost/panvk_vX_meta_desc_copy.c',]
    ...
    jm_archs=[6,7]
    jm_inc_dir=['jm']
    jm_files=['jm/panvk_vX_cmd_buffer.c','jm/panvk_vX_cmd_dispatch.c','jm/panvk_vX_cmd_draw.c','jm/panvk_vX_cmd_event.c','jm/panvk_vX_cmd_query.c','jm/panvk_vX_cmd_precomp.c','jm/panvk_vX_event.c','jm/panvk_vX_gpu_queue.c',]
    
    csf_archs=[10,12,13]
    csf_inc_dir=['csf']
    csf_files=['csf/panvk_vX_cmd_buffer.c','csf/panvk_vX_cmd_dispatch.c','csf/panvk_vX_cmd_draw.c','csf/panvk_vX_cmd_event.c','csf/panvk_vX_cmd_query.c','csf/panvk_vX_cmd_precomp.c','csf/panvk_vX_event.c','csf/panvk_vX_exception_handler.c','csf/panvk_vX_gpu_queue.c','csf/panvk_vX_instr.c','csf/panvk_vX_utrace.c',]
    
    common_per_arch_files=[panvk_entrypoints[0],panvk_tracepoints[0],'panvk_vX_blend.c','panvk_vX_buffer_view.c','panvk_vX_cmd_fb_preload.c','panvk_vX_cmd_desc_state.c','panvk_vX_cmd_dispatch.c','panvk_vX_cmd_draw.c','panvk_vX_cmd_meta.c','panvk_vX_cmd_push_constant.c','panvk_vX_descriptor_set.c','panvk_vX_descriptor_set_layout.c','panvk_vX_device.c','panvk_vX_physical_device.c','panvk_vX_precomp_cache.c','panvk_vX_query_pool.c','panvk_vX_image_view.c','panvk_vX_nir_lower_descriptors.c','panvk_vX_sampler.c','panvk_vX_shader.c',]
    
    for arch in [6,7,10,12,13]:
    ...
    libvulkan_panfrost=shared_library('vulkan_panfrost',[libpanvk_files,panvk_entrypoints,panvk_tracepoints],include_directories=[inc_include,inc_src,inc_panfrost,],link_whole=[panvk_per_arch_libs],link_with=[libpanfrost_shared,libpanfrost_midgard,libpanfrost_bifrost,libpanfrost_decode,libpanfrost_lib,libpanfrost_util,],dependencies=[dep_dl,dep_elf,dep_libdrm,dep_m,dep_thread,dep_valgrind,idep_nir,idep_pan_packers,panvk_deps,idep_vulkan_util,idep_vulkan_runtime,idep_vulkan_wsi,idep_mesautil,],c_args=[no_override_init_args,panvk_flags],link_args=[ld_args_bsymbolic,ld_args_gc_sections,ld_args_build_id],install=True,)
    
    panfrost_icd=custom_target('panfrost_icd',input=[vk_icd_gen,vk_api_xml],output='panfrost_icd.{}.json'.format(host_machine.cpu()),command=[prog_python,'@INPUT0@','--api-version','1.4','--xml','@INPUT1@','--sizeof-pointer',sizeof_pointer,'--lib-path',join_paths(get_option('prefix'),get_option('libdir'),'libvulkan_panfrost.so'),'--out','@OUTPUT@',],build_by_default=True,install_dir=with_vulkan_icd_dir,install_tag='runtime',install=True,)
    
    _dev_icdname='panfrost_devenv_icd.{}.json'.format(host_machine.cpu())
    _dev_icd=custom_target('panfrost_devenv_icd',input=[vk_icd_gen,vk_api_xml],output=_dev_icdname,command=[prog_python,'@INPUT0@','--api-version','1.4','--xml','@INPUT1@','--sizeof-pointer',sizeof_pointer,'--lib-path',(meson.current_build_dir() + 'libvulkan_panfrost.so') if isinstance(meson.current_build_dir(), str) else (meson.current_build_dir() / 'libvulkan_panfrost.so'),'--out','@OUTPUT@',],build_by_default=True,)
  ...
  if with_tools.count('drm-shim') > 0:
    set_relative_dir('src/panfrost/drm-shim')
    ########################################################################################################################
    ### Begin conversion from: src/panfrost/drm-shim/meson.build
    ########################################################################################################################
    libpanfrost_noop_drm_shim=shared_library('panfrost_noop_drm_shim','panfrost_noop.c',include_directories=[inc_include,inc_src],dependencies=dep_drm_shim,gnu_symbol_visibility='hidden',install=True,)
    
    ########################################################################################################################
    ### End conversion from: src/panfrost/drm-shim/meson.build
    ########################################################################################################################
    set_relative_dir('src/panfrost')
...
if with_microsoft_clc or with_gallium_d3d12 or with_spirv_to_dxil or with_microsoft_vk:
  ...
  if with_microsoft_clc or with_spirv_to_dxil or with_microsoft_vk or (with_gallium_d3d12 and not get_option('gallium-d3d12-graphics').disabled()):
    ...
    if with_microsoft_vk:
      ...
      dzn_deps=[idep_libdxil_compiler,idep_libspirv_to_dxil,idep_nir,idep_nir_headers,idep_vulkan_util,idep_vulkan_runtime,idep_vulkan_wsi,idep_mesautil,dep_dxheaders,]
      ...
      icd_command=[prog_python,'@INPUT0@','--api-version','1.1','--xml','@INPUT1@','--sizeof-pointer',sizeof_pointer,'--lib-path',join_paths(icd_lib_path,icd_file_name),'--out','@OUTPUT@',]
      
      icd_dev_command=[prog_python,'@INPUT0@','--api-version','1.1','--xml','@INPUT1@','--sizeof-pointer',sizeof_pointer,'--lib-path',join_paths(meson.current_build_dir(),icd_file_name),'--out','@OUTPUT@',]
...
if with_any_nouveau:
  ...
  ########################################################################################################################
  nv_classes=['cl902d','cl9039','cl906f','cl9097','cl90b5','cl90c0','cla040','cla0b5','cla097','cla140','cla297','clb097','clb0c0','clb197','clb1c0','clc097','clc197','clc1b5','cla0c0','clc0c0','clc397','clc3c0','clc56f','clc597','clc5c0','clc697','clc6c0','clc797','clc7c0','clc86f','clcab5','clcb97','clcbc0','clcd97','clcdc0','clcec0',]
  
  hwref_gens=['maxwell/gm107','pascal/gp100','hopper/gh100','turing/tu102',]
  
  cl_generated=[]
  for cl in nv_classes:
    cl_generated+=custom_target(cl+'.h',input=['class_parser.py','nvidia/classes/'+cl+'.h','util.py'],output=['nv_push_'+cl+'.h','nv_push_'+cl+'.c'],command=[prog_python,'@INPUT0@','--in-h','@INPUT1@','--out-h','@OUTPUT0@','--out-c','@OUTPUT1@'],)
  ...
  executable('nv_push_dump','nv_push_dump.c',dependencies=[idep_mesautil,idep_nvidia_headers],build_by_default=with_tools.count('nouveau') > 0,install=with_tools.count('nouveau') > 0,)
  ...
  if with_nouveau_vk:
    cl_rs_generated=[]
    for cl in nv_classes:
      cl_rs_generated+=custom_target('nvh_classes_'+cl+'.rs',input=['class_parser.py','nvidia/classes/'+cl+'.h','util.py'],output=['nvh_classes_'+cl+'.rs'],command=[prog_python,'@INPUT0@','--in-h','@INPUT1@','--out-rs','@OUTPUT0@'],)
  
      prev_cl_h=[]
      prev_cl_args=[]
      for prev_cl in nv_classes:
        if prev_cl==cl:
          break
      ...
      cl_rs_generated+=custom_target('nvh_'+cl+'_mthd.rs',input=['class_parser.py','nvidia/classes/'+cl+'.h',prev_cl_h,'util.py'],output=['nvh_classes_'+cl+'_mthd.rs'],command=[prog_python,'@INPUT0@','--in-h','@INPUT1@',prev_cl_args,'--out-rs-mthd','@OUTPUT0@'],)
  
      if cl.endswith('c0') and fs.is_file('nvidia/classes/'+cl+'qmd.h'):
        cl_rs_generated+=custom_target('nvh_classes_'+cl+'_qmd.rs',input=['struct_parser.py','nvidia/classes/'+cl+'qmd.h','util.py'],output=['nvh_classes_'+cl+'_qmd.rs'],command=[prog_python,'@INPUT0@','--in-h','@INPUT1@','--out-rs','@OUTPUT0@'])
  
  
      if cl.endswith('97'):
        cl_rs_generated+=custom_target('nvh_classes_'+cl+'_tex.rs',input=['struct_parser.py','nvidia/classes/'+cl+'tex.h','util.py'],output=['nvh_classes_'+cl+'_tex.rs'],command=[prog_python,'@INPUT0@','--in-h','@INPUT1@','--out-rs','@OUTPUT0@'])
  
  
      if fs.is_file(f'nvidia/classes/{cl}sph.h'):
        cl_rs_generated+=custom_target(input=['struct_parser.py',f'nvidia/classes/{cl}sph.h','util.py'],output=[f'nvh_classes_{cl}_sph.rs'],command=[prog_python,'@INPUT0@','--in-h','@INPUT1@','--out-rs','@OUTPUT0@'])
    ...
    _nvidia_headers_lib_rs=custom_target('lib.rs',input=['lib_rs_gen.py','util.py'],output=['lib.rs'],command=[prog_python,'@INPUT0@','--out-rs','@OUTPUT0@',cl_rs_generated])
    ...
    _libnv_push_rs=static_library('nv_push_rs',files('nv_push_rs/lib.rs'),gnu_symbol_visibility='hidden',rust_abi='rust',dependencies=[idep_nvidia_headers_rs],)
  
    idep_nv_push_rs=declare_dependency(link_with=_libnv_push_rs,)
  ...
  set_relative_dir('src/nouveau')
  if with_nouveau_vk:
    set_relative_dir('src/nouveau/rust')
    ########################################################################################################################
    ### Begin conversion from: src/nouveau/rust/meson.build
    ########################################################################################################################
    nouveau_rust_args=['-Aclippy::identity_op','-Aclippy::len_zero','-Aclippy::manual_range_contains','-Aclippy::needless_range_loop','-Aclippy::redundant_field_names','-Aclippy::upper_case_acronyms','-Aclippy::vec_box','-Aclippy::write_with_newline','-Aclippy::not_unsafe_ptr_arg_deref','-Anon_snake_case',]
    
    _libbitview_rs=static_library('bitview',files('bitview/lib.rs'),gnu_symbol_visibility='hidden',rust_abi='rust',rust_args=nouveau_rust_args,)
    
    idep_bitview_rs=declare_dependency(link_with=_libbitview_rs,)
    
    ########################################################################################################################
    ### End conversion from: src/nouveau/rust/meson.build
    ########################################################################################################################
    set_relative_dir('src/nouveau')
    ...
    dep_paste=dependency('paste',version='>= 1.0.14',fallback=['paste-1-rs','dep_paste'],required=True,)
    
    _nil_format_table=custom_target('nil_format_table',input=files('nil_format_table_gen.py','nil_formats.csv'),output=['nil_format_table.h','nil_format_table.c'],depends=u_format_gen_h,command=[prog_python,'@INPUT0@','--csv','@INPUT1@','--out-h','@OUTPUT0@','--out-c','@OUTPUT1@',],)
    
    _libnil_format_table=static_library('libnil_format_table',_nil_format_table,include_directories=[inc_include,inc_src],dependencies=[idep_nvidia_headers,idep_mesautil],gnu_symbol_visibility='hidden',)
    
    _libnil_deps=[dep_paste,idep_bitview_rs,idep_nvidia_headers_rs,]
    
    _nil_bindings_rs=rust.bindgen(input=['nil_bindings.h',_nil_format_table],output='nil_bindings.rs',c_args=[pre_args,],include_directories=[inc_include,inc_src,include_directories('.')],args=[bindgen_output_args,'--allowlist-function','util_format_description','--allowlist-function','util_format_get_blocksize','--allowlist-function','util_format_is_compressed','--allowlist-function','util_format_is_pure_integer','--allowlist-function','util_format_is_srgb','--allowlist-function','drm_format_mod_block_linear_2D','--allowlist-function','drm_mod_is_nvidia','--allowlist-type','nil_format_support_flags','--allowlist-type','nv_device_info','--allowlist-type','nv_device_type','--allowlist-type','pipe_format','--allowlist-type','pipe_swizzle','--allowlist-var','nil_format_table','--allowlist-var','drm_format_mod_invalid','--allowlist-var','drm_format_mod_linear','--no-prepend-enum-name',],dependencies=_libnil_deps,)
    
    _libnil_rs_bindings=static_library('nil_rs_bindings',_nil_bindings_rs,gnu_symbol_visibility='hidden',rust_abi='rust',)
    
    _libnil=static_library('libnil',files('lib.rs'),gnu_symbol_visibility='hidden',rust_abi='c',rust_args=nouveau_rust_args,link_with=[_libnil_format_table,_libnil_rs_bindings],dependencies=_libnil_deps,)
    ...
    idep_nil_h=declare_dependency(include_directories=include_directories('.'),sources=[_nil_h],)
    ...
    ########################################################################################################################
    set_relative_dir('src/nouveau/compiler/latencies')
    ########################################################################################################################
    ### Begin conversion from: src/nouveau/compiler/latencies/meson.build
    ########################################################################################################################
    _nak_lat_sm100_files=['pred_raw.csv','pred_war.csv','pred_waw.csv','reg_raw.csv','reg_war.csv','reg_waw.csv','upred_raw.csv','upred_war.csv','upred_waw.csv','ureg_raw.csv','ureg_war.csv','ureg_waw.csv',]
    
    _nak_lat_sms={'sm100':_nak_lat_sm100_files,}
    
    _lat_rs_gen=files('lat_rs_gen.py')
    
    _lat_rs_generated=[]
    for sm,csvs in _nak_lat_sms.items():
      csv_files=[]
      for i in range(len(csvs)):
        csv_files+=files(sm+'/'+csvs[i])
    
    
      _lat_rs_generated+=custom_target(sm+'.rs',input=[_lat_rs_gen,csv_files],output=[sm+'.rs'],command=[prog_python,'@INPUT@','-p',nouveau_util_py_path,'--out-rs','@OUTPUT0@','--sm',sm,],depend_files=nouveau_util_py,)
    
    
    _nak_latencies_lib_rs=custom_target('lib.rs',input=['lib_rs_gen.py',_lat_rs_generated,nouveau_util_py],output=['lib.rs'],command=[prog_python,'@INPUT0@','-p',nouveau_util_py_path,'--out-rs','@OUTPUT0@',_nak_lat_sms.keys()],)
    
    libnak_latencies_rs=static_library('nak_latencies',_nak_latencies_lib_rs,gnu_symbol_visibility='hidden',rust_abi='rust',)
    
    ########################################################################################################################
    ### End conversion from: src/nouveau/compiler/latencies/meson.build
    ########################################################################################################################
    set_relative_dir('src/nouveau/compiler')
    
    dep_paste=dependency('paste',version='>= 1.0.14',fallback=['paste-1-rs','dep_paste'],required=True,)
    
    dep_rustc_hash=dependency('rustc-hash',fallback=['rustc-hash-2-rs','dep_rustc_hash'],required=True,)
    
    libnak_c_files=files('nak.h','nak_nir.c','nak_nir_lower_cf.c','nak_nir_lower_cmat.c','nak_nir_lower_fs_inputs.c','nak_nir_lower_gs_intrinsics.c','nak_nir_lower_image_addrs.c','nak_nir_lower_non_uniform_ldcx.c','nak_nir_lower_scan_reduce.c','nak_nir_lower_kepler_shared_atomics.c','nak_nir_lower_tex.c','nak_nir_lower_vtg_io.c','nak_nir_mark_lcssa_invariants.c','nak_nir_rematerialize_load_const.c',)
    
    _libacorn_rs=static_library('acorn',files('acorn/lib.rs'),gnu_symbol_visibility='hidden',rust_abi='rust',rust_args=nouveau_rust_args,)
    
    idep_acorn_rs=declare_dependency(link_with=_libacorn_rs,)
    
    libnak_deps=[idep_mesautil,idep_nir_headers,idep_nvidia_headers,idep_nil_h,]
    
    _nak_bindings_rs=rust.bindgen(input=['nak_bindings.h'],output='nak_bindings.rs',c_args=[pre_args,],args=[bindgen_output_args,compiler_rs_bindgen_blocklist,'--raw-line','use compiler::bindings::*;','--allowlist-type','drm.*','--allowlist-type','nak_.*','--allowlist-type','nouveau_ws_.*','--allowlist-var','DRM_.*','--allowlist-var','NVIDIA_VENDOR_ID','--allowlist-function','drm.*','--allowlist-function','nak_.*','--allowlist-function','nouveau_ws_.*','--blocklist-type','glsl_.*','--no-prepend-enum-name','--with-derive-default',],dependencies=[dep_libdrm,idep_nouveau_ws,libnak_deps,],)
    
    _libnak_bindings_rs=static_library('nak_bindings',_nak_bindings_rs,gnu_symbol_visibility='hidden',dependencies=[idep_compiler_rs,],rust_abi='rust',)
    
    _libnak_ir_proc_rs=rust.proc_macro('nak_ir_proc',files('nak/ir_proc.rs'),dependencies=[idep_compiler_proc_rs],)
    
    _libnak_rs=static_library('nak_rs',files('nak/lib.rs'),gnu_symbol_visibility='hidden',rust_abi='c',rust_args=[nouveau_rust_args,'-Clink-arg=-Wno-unused-command-line-argument',],dependencies=[dep_paste,dep_rustc_hash,idep_bitview_rs,idep_compiler_rs,idep_nvidia_headers_rs,],link_with=[_libnak_bindings_rs,_libnak_ir_proc_rs,libnak_latencies_rs,],)
    
    # TODO: Linking Rust executables (such as unit tests) doesn't play nicely
    # with the sanitizers because meson doesn't know to pass -fsanitize to the
    # Rust linker.  See also https://github.com/mesonbuild/meson/issues/11741
    if with_tests and get_option('b_sanitize')=='none':
      rust.test('nak',_libnak_rs,args=['--skip','hw_tests::','--skip','nvdisasm_tests::',],suite=['nouveau'],dependencies=[dep_libdrm,idep_nouveau_ws,idep_compiler.partial_dependency(link_args=True,links=True),idep_mesautil.partial_dependency(link_args=True,links=True),idep_nv_push_rs,],rust_args=['-C','default-linker-libraries'],link_with=[_libacorn_rs,],)
  ...
  if with_tools.count('drm-shim') > 0:
    set_relative_dir('src/nouveau/drm-shim')
    ########################################################################################################################
    ### Begin conversion from: src/nouveau/drm-shim/meson.build
    ########################################################################################################################
    libnouveau_noop_drm_shim=shared_library('nouveau_noop_drm_shim','nouveau_noop.c',include_directories=[inc_include,inc_src],c_args=[cc.get_supported_arguments('-Wno-gnu-variable-sized-type-not-at-end')],dependencies=dep_drm_shim,gnu_symbol_visibility='hidden',install=True,)
    
    ########################################################################################################################
    ### End conversion from: src/nouveau/drm-shim/meson.build
    ########################################################################################################################
    set_relative_dir('src/nouveau')
  ...
  if with_nouveau_vk:
    ...
    ########################################################################################################################
    libnouveau_mme_files=files('mme_builder.h','mme_fermi.c','mme_fermi.h','mme_fermi_builder.c','mme_fermi_sim.c','mme_sim.h','mme_sim.c','mme_tu104.c','mme_tu104.h','mme_tu104_builder.c','mme_tu104_sim.c','mme_tu104_sim.h',)
    ...
    if with_tests and not with_platform_android:
      ...
      executable('mme_fermi_sim_hw_test',files('tests/mme_runner.cpp','tests/mme_fermi_sim_hw_test.cpp'),gnu_symbol_visibility='hidden',include_directories=[inc_include,inc_src],dependencies=[dep_libdrm,idep_gtest,idep_mesautil,idep_nvidia_headers,idep_nouveau_mme,idep_nouveau_ws],install=False,)
    
      executable('mme_tu104_sim_hw_test',files('tests/mme_runner.cpp','tests/mme_tu104_sim_hw_test.cpp'),gnu_symbol_visibility='hidden',include_directories=[inc_include,inc_src],dependencies=[dep_libdrm,idep_gtest,idep_mesautil,idep_nvidia_headers,idep_nouveau_mme,idep_nouveau_ws],install=False,)
    ...
    ########################################################################################################################
    nvk_files=files('nvk_acceleration_structure.c','nvk_buffer.c','nvk_buffer.h','nvk_buffer_view.c','nvk_buffer_view.h','nvk_cmd_buffer.c','nvk_cmd_buffer.h','nvk_cmd_clear.c','nvk_cmd_copy.c','nvk_cmd_dispatch.c','nvk_cmd_draw.c','nvk_cmd_indirect.c','nvk_cmd_meta.c','nvk_cmd_pool.c','nvk_cmd_pool.h','nvk_debug.h','nvk_descriptor_set.h','nvk_descriptor_set.c','nvk_descriptor_set_layout.c','nvk_descriptor_set_layout.h','nvk_descriptor_table.c','nvk_descriptor_table.h','nvk_descriptor_types.h','nvk_device.c','nvk_device.h','nvk_device_memory.c','nvk_device_memory.h','nvk_edb_bview_cache.c','nvk_edb_bview_cache.h','nvk_event.c','nvk_event.h','nvk_format.c','nvk_format.h','nvk_heap.c','nvk_heap.h','nvk_host_copy.c','nvk_image.c','nvk_image.h','nvk_image_view.c','nvk_image_view.h','nvk_indirect_execution_set.c','nvk_indirect_execution_set.h','nvk_instance.c','nvk_instance.h','nvk_mem_arena.c','nvk_mem_arena.h','nvk_mem_stream.c','nvk_mem_stream.h','nvk_mme.c','nvk_mme.h','nvk_nir_lower_descriptors.c','nvk_physical_device.c','nvk_physical_device.h','nvk_private.h','nvk_query_pool.c','nvk_query_pool.h','nvk_queue.c','nvk_queue.h','nvk_sampler.c','nvk_sampler.h','nvk_shader.c','nvk_shader.h','nvk_upload_queue.c','nvk_upload_queue.h','nvk_wsi.c','nvk_wsi.h','nvkmd/nouveau/nvkmd_nouveau.h','nvkmd/nouveau/nvkmd_nouveau_ctx.c','nvkmd/nouveau/nvkmd_nouveau_dev.c','nvkmd/nouveau/nvkmd_nouveau_mem.c','nvkmd/nouveau/nvkmd_nouveau_pdev.c','nvkmd/nouveau/nvkmd_nouveau_va.c','nvkmd/nvkmd.c','nvkmd/nvkmd.h',)
    
    nvkcl_files=files('cl/nvk_query.cl',)
    ...
    nvkcl_spv=custom_target(input=nvkcl_files,output='nvkcl.spv',command=[prog_mesa_clc,'-o','@OUTPUT@','--depfile','@DEPFILE@',nvkcl_files,'--','-I'+join_paths(meson.project_source_root(),'src/compiler/libcl'),'-I'+join_paths(meson.current_source_dir(),'.'),'-I'+join_paths(meson.project_source_root(),'src'),cl_args,],depfile='@PLAINNAME@.d',)
    
    nvkcl=custom_target(input=nvkcl_spv,output=['nvkcl.cpp','nvkcl.h'],command=[prog_vtn_bindgen2,'@INPUT@','@OUTPUT@'],)
    
    nvk_deps=[dep_libdrm,idep_nak,idep_nil,idep_nir,idep_mesautil,idep_nouveau_mme,idep_nouveau_ws,idep_nvidia_headers,idep_vulkan_runtime,idep_vulkan_util,idep_vulkan_wsi,idep_vulkan_wsi_headers,]
    
    nvk_flags=[]
    
    if with_platform_android:
      nvk_deps+=[dep_android]
      nvk_flags += ['-DVK_USE_PLATFORM_ANDROID_KHR'] if isinstance(nvk_flags, list) else '-DVK_USE_PLATFORM_ANDROID_KHR'
      nvk_files+=files('nvk_android.c')
    
    
    libnvk=static_library('nvk',[nvk_entrypoints,cl_generated,nvk_files,nvkcl,sha1_h,],include_directories=[inc_gallium,inc_gallium_aux,inc_include,inc_src,],dependencies=nvk_deps,c_args=[no_override_init_args,nvk_flags],gnu_symbol_visibility='hidden',)
    
    libvulkan_nouveau=shared_library('vulkan_nouveau',link_whole=[libnvk],link_args=[ld_args_build_id,ld_args_bsymbolic,ld_args_gc_sections],gnu_symbol_visibility='hidden',install=True,)
    ...
    nouveau_icd=custom_target('nouveau_icd',input=[vk_icd_gen,vk_api_xml],output='nouveau_icd.{}.json'.format(host_machine.cpu()),command=[prog_python,'@INPUT0@','--api-version','1.4','--xml','@INPUT1@','--sizeof-pointer',sizeof_pointer,'--lib-path',join_paths(icd_lib_path,icd_file_name),'--out','@OUTPUT@',],build_by_default=True,install_dir=with_vulkan_icd_dir,install_tag='runtime',install=True,)
    
    _dev_icdname='nouveau_devenv_icd.{}.json'.format(host_machine.cpu())
    custom_target('nouveau_devenv_icd',input=[vk_icd_gen,vk_api_xml],output=_dev_icdname,command=[prog_python,'@INPUT0@','--api-version','1.4','--xml','@INPUT1@','--sizeof-pointer',sizeof_pointer,'--lib-path',(meson.current_build_dir() + icd_file_name) if isinstance(meson.current_build_dir(), str) else (meson.current_build_dir() / icd_file_name),'--out','@OUTPUT@',],build_by_default=True,)
...
if with_gfxstream_vk:
  ...
  if with_virtgpu_kumquat:
    set_relative_dir('src/util/rust')
    ########################################################################################################################
    ### Begin conversion from: src/util/rust/meson.build
    ########################################################################################################################
    dep_cfg_if=dependency('cfg-if',version='>= 1.0.0',fallback=['cfg-if-1-rs','dep_cfg_if'],required=True,)
    
    dep_thiserror=dependency('thiserror',version='>= 2.0.11',fallback=['thiserror-2-rs','dep_thiserror'],required=True,)
    
    dep_remain=dependency('remain',version='>= 0.2.12',fallback=['remain-0.2-rs','dep_remain'],required=True,)
    
    dep_zerocopy=dependency('zerocopy',version='>= 0.8.13',fallback=['zerocopy-0.8-rs','dep_zerocopy'],required=True,)
    
    dep_zerocopy_derive=dependency('zerocopy-derive',version='>= 0.8.13',fallback=['zerocopy-derive-0.8-rs','dep_zerocopy_derive'],required=True,)
    
    dep_mesa3d_util=[dep_cfg_if,dep_thiserror,dep_remain,dep_zerocopy,dep_zerocopy_derive]
    
    if host_machine.system()=='linux':
      dep_rustix=dependency('rustix',version='>= 0.38.31',fallback=['rustix-1-rs','dep_rustix'],required=True,)
    
      dep_bitflags=dependency('bitflags',version='>= 2.6.0',fallback=['bitflags-2-rs','dep_bitflags'],required=True,)
    
      dep_errno=dependency('errno',version='>= 0.3.8',fallback=['errno-0.3-rs','dep_errno'],required=True,)
    
      dep_mesa3d_util+=[dep_rustix,dep_bitflags,dep_errno]
    
    
    libmesa_rust_util=static_library('mesa3d_util','lib.rs',gnu_symbol_visibility='hidden',rust_abi='rust',dependencies=dep_mesa3d_util,)
    
    ########################################################################################################################
    ### End conversion from: src/util/rust/meson.build
    ########################################################################################################################
    set_relative_dir('src')
    set_relative_dir('src/virtio/protocols')
    ########################################################################################################################
    ### Begin conversion from: src/virtio/protocols/meson.build
    ########################################################################################################################
    libmesa_protocols=static_library('mesa3d_protocols','lib.rs',gnu_symbol_visibility='hidden',rust_abi='rust',link_with=[libmesa_rust_util],dependencies=dep_mesa3d_util)
    
    ########################################################################################################################
    ### End conversion from: src/virtio/protocols/meson.build
    ########################################################################################################################
    set_relative_dir('src')
    set_relative_dir('src/virtio/virtgpu_kumquat')
    ########################################################################################################################
    ### Begin conversion from: src/virtio/virtgpu_kumquat/meson.build
    ########################################################################################################################
    libvirtgpu_kumquat=static_library('virtgpu_kumquat','lib.rs',gnu_symbol_visibility='hidden',rust_abi='rust',link_with=[libmesa_protocols,libmesa_rust_util],dependencies=[dep_mesa3d_util])
    
    ########################################################################################################################
    ### End conversion from: src/virtio/virtgpu_kumquat/meson.build
    ########################################################################################################################
    set_relative_dir('src')
    set_relative_dir('src/virtio/virtgpu_kumquat_ffi')
    ########################################################################################################################
    ### Begin conversion from: src/virtio/virtgpu_kumquat_ffi/meson.build
    ########################################################################################################################
    inc_virtgpu_kumquat_ffi=include_directories('include')
    
    dep_log=dependency('log',version='>= 0.4.22',fallback=['log-0.4-rs','dep_log'],required=True,)
    
    virtgpu_kumquat_ffi_args=['-Dunsafe_op_in_unsafe_fn',]
    
    libvirtgpu_kumquat_ffi=static_library('virtgpu_kumquat_ffi','lib.rs',gnu_symbol_visibility='hidden',rust_abi='c',rust_args=[virtgpu_kumquat_ffi_args,],link_with=[libmesa_protocols,libmesa_rust_util,libvirtgpu_kumquat],dependencies=[dep_mesa3d_util,dep_log])
    
    dep_virtgpu_kumquat_ffi=declare_dependency(include_directories=[inc_virtgpu_kumquat_ffi],link_with=[libvirtgpu_kumquat_ffi])
    
    ########################################################################################################################
    ### End conversion from: src/virtio/virtgpu_kumquat_ffi/meson.build
    ########################################################################################################################
    set_relative_dir('src')

  set_relative_dir('src/gfxstream')
  ########################################################################################################################
  ### Begin conversion from: src/gfxstream/meson.build
  ########################################################################################################################
  genvk=files('codegen/scripts/genvk.py')
  vk_gfxstream_xml=files('codegen/xml/vk_gfxstream.xml')
  
  set_relative_dir('src/gfxstream/aemu')
  ########################################################################################################################
  ### Begin conversion from: src/gfxstream/aemu/meson.build
  ########################################################################################################################
  inc_aemu=include_directories('include')
  
  files_libaemu=files('ring_buffer.cpp','Stream.cpp',)
  
  libaemu=static_library('aemu',files_libaemu,include_directories=[inc_aemu],)
  
  ########################################################################################################################
  ### End conversion from: src/gfxstream/aemu/meson.build
  ########################################################################################################################
  set_relative_dir('src/gfxstream')
  set_relative_dir('src/gfxstream/guest')
  ########################################################################################################################
  ### Begin conversion from: src/gfxstream/guest/meson.build
  ########################################################################################################################
  with_vulkan_icd_dir=get_option('vulkan-icd-dir')
  if with_vulkan_icd_dir=='':
    with_vulkan_icd_dir=join_paths(get_option('datadir'),'vulkan/icd.d')
  ...
  if host_machine.system()=='windows':
    gfxstream_guest_args += ['-DWINDOWS_GUEST_BUILD'] if isinstance(gfxstream_guest_args, list) else '-DWINDOWS_GUEST_BUILD'
  else:
    gfxstream_guest_args += ['-DLINUX_GUEST_BUILD'] if isinstance(gfxstream_guest_args, list) else '-DLINUX_GUEST_BUILD'
  
  # Include the gfxstream private VkStructureType definitions
  gfxstream_guest_args += ['-DVK_GFXSTREAM_STRUCTURE_TYPE_EXT'] if isinstance(gfxstream_guest_args, list) else '-DVK_GFXSTREAM_STRUCTURE_TYPE_EXT'
  ...
  # Subdirectories #
  #================#
  set_relative_dir('src/gfxstream/guest/iostream')
  ########################################################################################################################
  ### Begin conversion from: src/gfxstream/guest/iostream/meson.build
  ########################################################################################################################
  inc_guest_iostream=include_directories('include')
  
  ########################################################################################################################
  ### End conversion from: src/gfxstream/guest/iostream/meson.build
  ########################################################################################################################
  set_relative_dir('src/gfxstream/guest')
  set_relative_dir('src/gfxstream/guest/platform')
  ########################################################################################################################
  ### Begin conversion from: src/gfxstream/guest/platform/meson.build
  ########################################################################################################################
  inc_platform_virtgpu=include_directories('include')
  
  set_relative_dir('src/gfxstream/guest/platform/kumquat')
  ########################################################################################################################
  ### Begin conversion from: src/gfxstream/guest/platform/kumquat/meson.build
  ########################################################################################################################
  if with_virtgpu_kumquat:
    set_relative_dir('src/gfxstream/guest/platform/kumquat/vulkan-mapper')
    ########################################################################################################################
    ### Begin conversion from: src/gfxstream/guest/platform/kumquat/vulkan-mapper/meson.build
    ########################################################################################################################
    inc_gfxstream_vulkan_mapper=include_directories('.')
    
    files_gfxstream_vulkan_mapper=files('GfxStreamVulkanMapper.cpp',)
    
    libgfxstream_vulkan_mapper=static_library('gfxstream_vulkan_mapper',files_gfxstream_vulkan_mapper,cpp_args=gfxstream_guest_args,include_directories=[inc_src,inc_vulkan_util,inc_gfxstream_vulkan_mapper,inc_include],dependencies=[idep_vulkan_util_headers,idep_vulkan_util,idep_mesautil,dep_virtgpu_kumquat_ffi],)
    
    dep_gfxstream_vulkan_mapper=declare_dependency(include_directories=[inc_gfxstream_vulkan_mapper],link_with=[libgfxstream_vulkan_mapper])
    
    ########################################################################################################################
    ### End conversion from: src/gfxstream/guest/platform/kumquat/vulkan-mapper/meson.build
    ########################################################################################################################
    set_relative_dir('src/gfxstream/guest/platform/kumquat')
    files_libplatform_virtgpu_kumquat=files('VirtGpuKumquatDevice.cpp','VirtGpuKumquatBlobMapping.cpp','VirtGpuKumquatBlob.cpp','VirtGpuKumquatSync.cpp',)
  else:
    dep_gfxstream_vulkan_mapper=null_dep
    files_libplatform_virtgpu_kumquat=files('VirtGpuKumquatStub.cpp')
  
  
  libplatform_virtgpu_kumquat=static_library('platform_virtgpu_kumquat',files_libplatform_virtgpu_kumquat,cpp_args=gfxstream_guest_args,include_directories=[inc_platform_virtgpu,inc_src,inc_vulkan_util,inc_include],dependencies=[idep_vulkan_util_headers,dep_virtgpu_kumquat_ffi,dep_gfxstream_vulkan_mapper],)
  
  ########################################################################################################################
  ### End conversion from: src/gfxstream/guest/platform/kumquat/meson.build
  ########################################################################################################################
  set_relative_dir('src/gfxstream/guest/platform')
  ...
  if system_has_kms_drm:
    set_relative_dir('src/gfxstream/guest/platform/drm')
    ########################################################################################################################
    ### Begin conversion from: src/gfxstream/guest/platform/drm/meson.build
    ########################################################################################################################
    files_libplatform_virtgpu_drm=files('DrmVirtGpuDevice.cpp','DrmVirtGpuBlobMapping.cpp','DrmVirtGpuBlob.cpp','DrmSync.cpp',)
    
    libplatform_virtgpu_drm=static_library('platform_virtgpu_drm',files_libplatform_virtgpu_drm,cpp_args=gfxstream_guest_args,include_directories=[inc_platform_virtgpu,inc_src,inc_include],dependencies=dep_libdrm,)
    
    ########################################################################################################################
    ### End conversion from: src/gfxstream/guest/platform/drm/meson.build
    ########################################################################################################################
    set_relative_dir('src/gfxstream/guest/platform')
    libplatform_virtgpu+=libplatform_virtgpu_drm
  elif host_machine.system()=='windows':
    set_relative_dir('src/gfxstream/guest/platform/windows')
    ########################################################################################################################
    ### Begin conversion from: src/gfxstream/guest/platform/windows/meson.build
    ########################################################################################################################
    files_libplatform_virtgpu_windows=files('WindowsVirtGpuDevice.cpp','WindowsVirtGpuBlobMapping.cpp','WindowsVirtGpuBlob.cpp','WindowsSync.cpp',)
    
    libplatform_virtgpu_windows=static_library('platform_virtgpu_windows',files_libplatform_virtgpu_windows,cpp_args=gfxstream_guest_args,include_directories=[inc_platform_virtgpu,inc_src,inc_include],)
    
    ########################################################################################################################
    ### End conversion from: src/gfxstream/guest/platform/windows/meson.build
    ########################################################################################################################
    set_relative_dir('src/gfxstream/guest/platform')
  ...
  libplatform_virtgpu=static_library('platform_virtgpu',files_libplatform_virtgpu,cpp_args=gfxstream_guest_args,include_directories=[inc_platform_virtgpu,inc_src],link_with=libplatform_virtgpu)
  
  ########################################################################################################################
  ### End conversion from: src/gfxstream/guest/platform/meson.build
  ########################################################################################################################
  set_relative_dir('src/gfxstream/guest')
  set_relative_dir('src/gfxstream/guest/GoldfishAddressSpace')
  ########################################################################################################################
  ### Begin conversion from: src/gfxstream/guest/GoldfishAddressSpace/meson.build
  ########################################################################################################################
  inc_goldfish_address_space=include_directories('include')
  
  files_lib_goldfish_address_space=files('AddressSpaceStream.cpp','VirtioGpuAddressSpaceStream.cpp',)
  ...
  lib_goldfish_address_space=static_library('goldfish_address_space',files_lib_goldfish_address_space,cpp_args=gfxstream_guest_args,include_directories=[inc_goldfish_address_space,inc_guest_iostream,inc_platform_virtgpu,inc_include,inc_aemu,inc_src],link_with=[libplatform_virtgpu],dependencies=dep_libdrm)
  
  ########################################################################################################################
  ### End conversion from: src/gfxstream/guest/GoldfishAddressSpace/meson.build
  ########################################################################################################################
  set_relative_dir('src/gfxstream/guest')
  set_relative_dir('src/gfxstream/guest/connection-manager')
  ########################################################################################################################
  ### Begin conversion from: src/gfxstream/guest/connection-manager/meson.build
  ########################################################################################################################
  inc_connection_manager=include_directories('.')
  
  files_libconnection_manager=files('GfxStreamConnectionManager.cpp','GfxStreamConnection.cpp','QemuPipeStreamStub.cpp','VirtioGpuPipeStream.cpp',)
  
  libconnection_manager=static_library('connection_manager',files_libconnection_manager,cpp_args=gfxstream_guest_args,include_directories=[inc_connection_manager,inc_guest_iostream,inc_guest_iostream,inc_goldfish_address_space,inc_platform_virtgpu,inc_src,inc_aemu],link_with=[lib_goldfish_address_space,libplatform_virtgpu],dependencies=dep_libdrm)
  
  ########################################################################################################################
  ### End conversion from: src/gfxstream/guest/connection-manager/meson.build
  ########################################################################################################################
  set_relative_dir('src/gfxstream/guest')
  set_relative_dir('src/gfxstream/guest/vulkan_enc')
  ########################################################################################################################
  ### Begin conversion from: src/gfxstream/guest/vulkan_enc/meson.build
  ########################################################################################################################
  ...
  if build_machine.system()!='android':
    gfxstream_guest_vk_autogen=custom_target('gfxstream_guest_vk_autogen',output=['VkEncoder.h','goldfish_vk_counting_guest.h','goldfish_vk_deepcopy_guest.h','goldfish_vk_marshaling_guest.h','goldfish_vk_extension_structs_guest.h','goldfish_vk_reserved_marshaling_guest.h','goldfish_vk_transform_guest.h','goldfish_vk_counting_guest.cpp','goldfish_vk_deepcopy_guest.cpp','goldfish_vk_extension_structs_guest.cpp','goldfish_vk_marshaling_guest.cpp','goldfish_vk_reserved_marshaling_guest.cpp','goldfish_vk_transform_guest.cpp','VkEncoder.cpp','func_table.cpp',],env={'GFXSTREAM_NO_CLANG_FMT':'1','CEREAL_VARIANT':'guest','GFXSTREAM_GUEST_ENCODER_DIR':guest_enc_path},input=[genvk,vk_api_xml,vk_gfxstream_xml],command=[prog_python,'@INPUT0@','-registry','@INPUT1@','-registryGfxstream','@INPUT2@','cereal','-o',output_host_path],)
  
    vulkan_gfxstream_h=custom_target('vulkan_gfxstream_header',output=['vulkan_gfxstream.h',],env={'GFXSTREAM_NO_CLANG_FMT':'1','CEREAL_VARIANT':'guest'},input=[genvk,vk_gfxstream_xml],command=[prog_python,'@INPUT0@','-registry','@INPUT1@','vulkan_gfxstream.h','-o',guest_enc_path],)
  else:
    gfxstream_guest_vk_autogen=files('VkEncoder.h','goldfish_vk_counting_guest.h','goldfish_vk_deepcopy_guest.h','goldfish_vk_marshaling_guest.h','goldfish_vk_extension_structs_guest.h','goldfish_vk_reserved_marshaling_guest.h','goldfish_vk_transform_guest.h','goldfish_vk_counting_guest.cpp','goldfish_vk_deepcopy_guest.cpp','goldfish_vk_extension_structs_guest.cpp','goldfish_vk_marshaling_guest.cpp','goldfish_vk_reserved_marshaling_guest.cpp','goldfish_vk_transform_guest.cpp','VkEncoder.cpp','func_table.cpp')
  
    vulkan_gfxstream_h=files('vulkan_gfxstream.h')
  
  
  gfxstream_vk_entrypoints=custom_target('gfxstream_vk_entrypoints',input=[vk_entrypoints_gen,vk_api_xml],output=['gfxstream_vk_entrypoints.h','gfxstream_vk_entrypoints.c'],command=[prog_python,'@INPUT0@','--xml','@INPUT1@','--proto','--weak','--out-h','@OUTPUT0@','--out-c','@OUTPUT1@','--prefix','gfxstream_vk','--beta',str(with_vulkan_beta)],)
  
  files_lib_vulkan_enc=files('CommandBufferStagingStream.cpp','DescriptorSetVirtualization.cpp','HostVisibleMemoryVirtualization.cpp','ResourceTracker.cpp','Resources.cpp','Validation.cpp','VulkanHandleMapping.cpp','VulkanStreamGuest.cpp','gfxstream_vk_private.cpp','GfxStreamVulkanConnection.cpp',)
  
  ########################################################################################################################
  ### End conversion from: src/gfxstream/guest/vulkan_enc/meson.build
  ########################################################################################################################
  set_relative_dir('src/gfxstream/guest')
  set_relative_dir('src/gfxstream/guest/vulkan')
  ########################################################################################################################
  ### Begin conversion from: src/gfxstream/guest/vulkan/meson.build
  ########################################################################################################################
  set_relative_dir('src/gfxstream/guest/vulkan/stubs')
  ########################################################################################################################
  ### Begin conversion from: src/gfxstream/guest/vulkan/stubs/meson.build
  ########################################################################################################################
  inc_gfxstream_vk_stubs=include_directories('.')
  files_libgfxstream_vk_stubs=files('GfxStreamRenderControl.cpp',)
  
  libgfxstream_vk_stubs=static_library('gfxstream_vk_stubs',files_libgfxstream_vk_stubs,cpp_args=gfxstream_guest_args,include_directories=[inc_connection_manager,inc_platform_virtgpu,inc_guest_iostream,inc_src],)
  
  ########################################################################################################################
  ### End conversion from: src/gfxstream/guest/vulkan/stubs/meson.build
  ########################################################################################################################
  set_relative_dir('src/gfxstream/guest/vulkan')
  
  files_lib_vulkan_gfxstream=files('gfxstream_vk_device.cpp','gfxstream_vk_cmd.cpp','gfxstream_vk_wsi.cpp')
  
  gfxstream_vk_wsi_args=[]
  if with_platform_x11:
    gfxstream_vk_wsi_args += ['-DGFXSTREAM_VK_X11'] if isinstance(gfxstream_vk_wsi_args, list) else '-DGFXSTREAM_VK_X11'
  
  
  if with_platform_wayland:
    gfxstream_vk_wsi_args += ['-DGFXSTREAM_VK_WAYLAND'] if isinstance(gfxstream_vk_wsi_args, list) else '-DGFXSTREAM_VK_WAYLAND'
  
  
  lib_vulkan_gfxstream=shared_library('vulkan_gfxstream',files_lib_vulkan_gfxstream+files_lib_vulkan_enc+gfxstream_vk_entrypoints+gfxstream_guest_vk_autogen+vulkan_gfxstream_h,cpp_args=gfxstream_guest_args+gfxstream_vk_wsi_args,include_directories=[inc_vulkan_headers,inc_guest_iostream,inc_vulkan_enc,inc_platform_virtgpu,inc_gfxstream_vk_stubs,inc_goldfish_address_space,inc_include,inc_src,inc_aemu,inc_platform_virtgpu,inc_connection_manager],link_with=[libgfxstream_vk_stubs,libvulkan_wsi,libplatform_virtgpu,libconnection_manager,libaemu],link_args=[vulkan_icd_link_args,ld_args_bsymbolic,ld_args_gc_sections],link_depends=vulkan_icd_link_depends,dependencies=[dep_libdrm,idep_vulkan_wsi_headers,idep_vulkan_lite_runtime,idep_vulkan_util_headers,idep_vulkan_wsi,dep_valgrind],install=True,)
  
  gfxstream_icd=custom_target('gfxstream_vk_icd',input=[vk_icd_gen,vk_api_xml],output='gfxstream_vk_icd.{}.json'.format(host_machine.cpu()),command=[prog_python,'@INPUT0@','--api-version','1.1','--xml','@INPUT1@','--sizeof-pointer',sizeof_pointer,'--lib-path',join_paths(get_option('prefix'),get_option('libdir'),'libvulkan_gfxstream.so'),'--out','@OUTPUT@',],build_by_default=True,install_dir=with_vulkan_icd_dir,install=True,)
  
  _dev_icdname='gfxstream_vk_devenv_icd.{}.json'.format(host_machine.cpu())
  _dev_icd=custom_target('gfxstream_vk_devenv_icd',input=[vk_icd_gen,vk_api_xml],output=_dev_icdname,command=[prog_python,'@INPUT0@','--api-version','1.3','--xml','@INPUT1@','--sizeof-pointer',sizeof_pointer,'--lib-path',(meson.current_build_dir() + 'libvulkan_gfxstream.so') if isinstance(meson.current_build_dir(), str) else (meson.current_build_dir() / 'libvulkan_gfxstream.so'),'--out','@OUTPUT@',],build_by_default=True,)
  ...
  ########################################################################################################################
  ### End conversion from: src/gfxstream/guest/vulkan/meson.build
  ########################################################################################################################
  set_relative_dir('src/gfxstream/guest')
  
  ########################################################################################################################
  ### End conversion from: src/gfxstream/guest/meson.build
  ########################################################################################################################
  set_relative_dir('src/gfxstream')
  
  ########################################################################################################################
  ### End conversion from: src/gfxstream/meson.build
  ########################################################################################################################
  set_relative_dir('src')

if with_gallium_asahi or with_asahi_vk or with_tools.count('asahi') > 0:
  ...
  set_relative_dir('src/asahi/isa')
  ########################################################################################################################
  ### Begin conversion from: src/asahi/isa/meson.build
  ########################################################################################################################
  agx2_disasm_c=custom_target(input=['gen-disasm.py','AGX2.xml'],output='agx2_disasm.c',command=[prog_python,'@INPUT@'],capture=True,depend_files=files('isa.py'),)
  
  libagx2_disasm=static_library('agx2_disasm',agx2_disasm_c,include_directories=[inc_include,inc_src],c_args=[no_override_init_args],gnu_symbol_visibility='hidden',build_by_default=True,)
  
  if with_tests:
    test('agx2_disasm',executable('agx2_disasm_test',files('test/test-disassembler.c'),c_args=[c_msvc_compat_args,no_override_init_args],gnu_symbol_visibility='hidden',include_directories=[inc_include,inc_src],link_with=[libagx2_disasm],),suite=['asahi'],args=files('test/cases.txt'),)
  
    test('agx2_python_disasm',prog_python,args=files('test-disasm.py','test/cases.txt'),suite=['asahi'])
  
  
  ########################################################################################################################
  ### End conversion from: src/asahi/isa/meson.build
  ########################################################################################################################
  set_relative_dir('src/asahi')
  ...
  if with_gallium_asahi or with_asahi_vk or with_tools.count('asahi') > 0:
    ...
    ########################################################################################################################
    agx_pack=custom_target(input=['gen_pack.py','cmdbuf.xml','agx_pack_header.h'],output='agx_pack.h',command=[prog_python,'@INPUT@'],capture=True,)
    ...
    ########################################################################################################################
    set_relative_dir('src/asahi')
    set_relative_dir('src/asahi/libagx')
    ########################################################################################################################
    ### Begin conversion from: src/asahi/libagx/meson.build
    ########################################################################################################################
    libagx_shader_files=files('compression.cl','copy.cl','draws.cl','geometry.cl','query.cl','tessellation.cl','tessellator.cl','texture.cl','helper.cl',)
    
    libagx_spv=custom_target(input=libagx_shader_files,output='libagx.spv',command=[prog_mesa_clc,'-o','@OUTPUT@','--depfile','@DEPFILE@',libagx_shader_files,'--','-I'+join_paths(meson.project_source_root(),'include'),'-I'+join_paths(meson.project_source_root(),'src/compiler/libcl'),'-I'+join_paths(meson.current_source_dir(),'.'),'-I'+join_paths(meson.current_source_dir(),'../../'),'-I'+join_paths(meson.current_source_dir(),'shaders'),'-I'+join_paths(meson.current_build_dir(),'../genxml'),cl_args,],depends=[agx_pack],depfile='libagx_shaders.h.d',)
    
    libagx=custom_target(input=libagx_spv,output=['libagx.cpp','libagx.h'],command=[prog_vtn_bindgen2,libagx_spv,'@OUTPUT0@','@OUTPUT1@'],)
    
    idep_libagx=declare_dependency(sources=[libagx],include_directories=include_directories('.'),)
    
    ########################################################################################################################
    ### End conversion from: src/asahi/libagx/meson.build
    ########################################################################################################################
    set_relative_dir('src/asahi')
    ...
    if get_option('precomp-compiler')=='system':
      ...
      prog_asahi_clc=executable('asahi_clc',['asahi_clc.c'],link_with=[libasahi_compiler],include_directories=[inc_include,inc_src],c_args=[pre_args,no_override_init_args],link_args=[ld_args_build_id],dependencies=[idep_vtn,idep_nir,idep_mesautil],native=not meson.can_run_host_binaries(),install=get_option('install-precomp-compiler'),)
  ...
  if with_gallium_asahi or with_asahi_vk:
    ...
    ########################################################################################################################
    libasahi_layout_files=files('formats.c','layout.c','tiling.cc',)
    
    libasahi_layout=static_library('asahi_layout',[libasahi_layout_files],include_directories=[inc_include,inc_src],dependencies=[idep_mesautil,idep_agx_pack],c_args=[no_override_init_args],gnu_symbol_visibility='hidden',build_by_default=False,)
    ...
    ########################################################################################################################
    libasahi_lib_files=files('agx_bo.c','agx_border.c','agx_device.c','agx_device_virtio.c','agx_linker.c','agx_bg_eot.c','agx_tilebuffer.c','agx_nir_lower_gs.c','agx_nir_lower_ia.c','agx_nir_lower_msaa.c','agx_nir_lower_sample_intrinsics.c','agx_nir_lower_tess.c','agx_nir_lower_tilebuffer.c','agx_nir_lower_uvs.c','agx_nir_lower_vbo.c','agx_nir_prolog_epilog.c','agx_ppp.h','agx_scratch.c','agx_va.c','pool.c',)
    
    libasahi_decode_files=files('decode.c',)
    
    libasahi_decode=static_library('asahi_decode',[libasahi_decode_files,agx_pack],dependencies=[dep_valgrind,idep_mesautil],c_args=[no_override_init_args,'-Wno-c2x-extensions'],gnu_symbol_visibility='hidden',link_with=[libagx2_disasm],build_by_default=False,)
    
    libasahi_decode_shared=shared_library('asahi_decode',[libasahi_decode_files,agx_pack],dependencies=[dep_valgrind,idep_mesautil],c_args=[no_override_init_args,'-Wno-c2x-extensions'],build_by_default=with_tools.count('asahi') > 0,link_with=[libagx2_disasm],)
    
    libagx_shaders=custom_target(input=libagx_spv,output=['libagx_shaders.h','libagx_shaders.c'],command=[prog_asahi_clc,libagx_spv,'@OUTPUT0@','@OUTPUT1@'],)
    
    idep_libagx_shaders_h=declare_dependency(sources=[libagx_shaders],include_directories=include_directories('.'),)
    
    libasahi_lib=static_library('asahi_lib',[libasahi_lib_files,libagx_shaders,agx_pack,sha1_h],include_directories=[inc_asahi,inc_virtio_gpu,inc_virtio_vdrm],c_args=[no_override_init_args,'-Wno-c2x-extensions'],gnu_symbol_visibility='hidden',link_with=[libasahi_decode,libvdrm],dependencies=[dep_libdrm,dep_valgrind,idep_nir,idep_mesautil,idep_libagx],build_by_default=False,)
    
    if with_tests:
      test('libasahi_tests',executable('libasahi_tests',files('tests/test-packing.cpp','tests/test-tilebuffer.cpp',),c_args=[c_msvc_compat_args,no_override_init_args],gnu_symbol_visibility='hidden',dependencies=[idep_gtest,idep_agx_pack,idep_mesautil],link_with=[libasahi_lib,libasahi_layout],),suite=['asahi'],protocol='gtest',)
  ...
  if with_asahi_vk:
    set_relative_dir('src/asahi/vulkan')
    ########################################################################################################################
    ### Begin conversion from: src/asahi/vulkan/meson.build
    ########################################################################################################################
    hk_files=files('hk_buffer.c','hk_buffer.h','hk_buffer_view.c','hk_buffer_view.h','hk_cmd_buffer.c','hk_cmd_buffer.h','hk_cmd_clear.c','hk_cmd_dispatch.c','hk_cmd_draw.c','hk_cmd_meta.c','hk_cmd_pool.c','hk_cmd_pool.h','hk_descriptor_set.h','hk_descriptor_set.c','hk_descriptor_set_layout.c','hk_descriptor_set_layout.h','hk_descriptor_table.c','hk_descriptor_table.h','hk_device.c','hk_device.h','hk_device_memory.c','hk_device_memory.h','hk_event.c','hk_event.h','hk_format.c','hk_image.c','hk_image.h','hk_image_view.c','hk_image_view.h','hk_instance.c','hk_instance.h','hk_nir_lower_descriptors.c','hk_nir_passthrough_gs.c','hk_physical_device.c','hk_physical_device.h','hk_private.h','hk_query_pool.c','hk_query_pool.h','hk_queue.c','hk_queue.h','hk_sampler.c','hk_sampler.h','hk_shader.c','hk_shader.h','hk_wsi.c','hk_wsi.h')
    
    hk_entrypoints=custom_target(input=[vk_entrypoints_gen,vk_api_xml],output=['hk_entrypoints.h','hk_entrypoints.c'],command=[prog_python,'@INPUT0@','--xml','@INPUT1@','--proto','--weak','--out-h','@OUTPUT0@','--out-c','@OUTPUT1@','--prefix','hk','--beta',str(with_vulkan_beta),],depend_files=vk_entrypoints_gen_depend_files,)
    
    hk_deps=[dep_libdrm,idep_nir,idep_vulkan_runtime,idep_vulkan_util,idep_vulkan_wsi,idep_vulkan_wsi_headers,idep_agx_pack,idep_libagx,idep_libagx_shaders_h,]
    
    libhk=static_library('hk',[hk_entrypoints,hk_files,sha1_h,],include_directories=[inc_gallium,inc_gallium_aux,inc_include,inc_src,inc_asahi,inc_virtio_gpu,inc_virtio_vdrm,],link_with=[libasahi_lib,libasahi_layout,libasahi_compiler],c_args=['-Wno-c2x-extensions'],dependencies=[hk_deps],gnu_symbol_visibility='hidden',)
    
    libvulkan_asahi=shared_library('vulkan_asahi',link_whole=[libhk],link_args=[ld_args_build_id],gnu_symbol_visibility='hidden',install=True,)
    ...
    asahi_icd=custom_target(input=[vk_icd_gen,vk_api_xml],output='asahi_icd.{}.json'.format(host_machine.cpu()),command=[prog_python,'@INPUT0@','--api-version','1.4','--xml','@INPUT1@','--sizeof-pointer',sizeof_pointer,'--lib-path',join_paths(icd_lib_path,icd_file_name),'--out','@OUTPUT@',],build_by_default=True,install_dir=with_vulkan_icd_dir,install_tag='runtime',install=True,)
    
    _dev_icdname='asahi_devenv_icd.{}.json'.format(host_machine.cpu())
    custom_target(input=[vk_icd_gen,vk_api_xml],output=_dev_icdname,command=[prog_python,'@INPUT0@','--api-version','1.4','--xml','@INPUT1@','--sizeof-pointer',sizeof_pointer,'--lib-path',(meson.current_build_dir() + icd_file_name) if isinstance(meson.current_build_dir(), str) else (meson.current_build_dir() / icd_file_name),'--out','@OUTPUT@',],build_by_default=True,)
    ...
    ########################################################################################################################
    ### End conversion from: src/asahi/vulkan/meson.build
    ########################################################################################################################
    set_relative_dir('src/asahi')
  
  
  if with_tools.count('drm-shim') > 0:
    set_relative_dir('src/asahi/drm-shim')
    ########################################################################################################################
    ### Begin conversion from: src/asahi/drm-shim/meson.build
    ########################################################################################################################
    libasahi_noop_drm_shim=shared_library('asahi_noop_drm_shim','asahi_noop.c',include_directories=[inc_include,inc_src],dependencies=[dep_drm_shim,dep_valgrind],gnu_symbol_visibility='hidden',install=True,)
    
    ########################################################################################################################
    ### End conversion from: src/asahi/drm-shim/meson.build
    ########################################################################################################################
    set_relative_dir('src/asahi')
...
if with_gallium:
  ...
  get_hash_h=custom_target('get_hash.h',input=['main/get_hash_generator.py','../mapi/glapi/gen/gl_and_es_API.xml'],output='get_hash.h',command=[prog_python,'@INPUT0@','-f','@INPUT1@'],depend_files=files('main/get_hash_params.py')+glapi_xml_py_deps,capture=True,)
...
if with_gallium_or_lvp:
  ...
  files_libgalliumvl=files('vl/vl_bicubic_filter.c','vl/vl_bicubic_filter.h','vl/vl_codec.c','vl/vl_codec.h','vl/vl_compositor.c','vl/vl_compositor.h','vl/vl_compositor_gfx.c','vl/vl_compositor_gfx.h','vl/vl_compositor_cs.c','vl/vl_compositor_cs.h','vl/vl_csc.c','vl/vl_csc.h','vl/vl_decoder.c','vl/vl_decoder.h','vl/vl_defines.h','vl/vl_deint_filter.c','vl/vl_deint_filter.h','vl/vl_deint_filter_cs.c','vl/vl_deint_filter_cs.h','vl/vl_idct.c','vl/vl_idct.h','vl/vl_matrix_filter.c','vl/vl_matrix_filter.h','vl/vl_mc.c','vl/vl_mc.h','vl/vl_median_filter.c','vl/vl_median_filter.h','vl/vl_mpeg12_bitstream.c','vl/vl_mpeg12_bitstream.h','vl/vl_mpeg12_decoder.c','vl/vl_mpeg12_decoder.h','vl/vl_types.h','vl/vl_vertex_buffers.c','vl/vl_vertex_buffers.h','vl/vl_video_buffer.c','vl/vl_video_buffer.h','vl/vl_zscan.c','vl/vl_zscan.h',)
  ...
  if with_tests:
    test('gallium-aux',executable('gallium-aux','util/u_surface_test.cpp',include_directories=[inc_include,inc_src,inc_mapi,inc_mesa,inc_gallium,inc_gallium_aux],link_with=libgallium,dependencies=[idep_gtest,idep_mesautil],),suite='gallium',protocol='gtest',)
  ...
  libgalliumvl=static_library('galliumvl',files_libgalliumvl,c_args=[c_msvc_compat_args],cpp_args=[cpp_msvc_compat_args],gnu_symbol_visibility='hidden',include_directories=[inc_gallium,inc_include,inc_src],dependencies=[idep_nir,idep_mesautil],build_by_default=False,)
  ...
  libgalliumvlwinsys=static_library('galliumvlwinsys',files_libgalliumvlwinsys,include_directories=[inc_gallium,inc_include,inc_loader,inc_src,include_directories('../drivers/zink')],dependencies=[dep_libdrm,vlwinsys_deps,idep_mesautil],link_with=links_libgalliumvlwinsys,build_by_default=False,)
  
  ########################################################################################################################
  ### End conversion from: src/gallium/auxiliary/meson.build
  ########################################################################################################################
  set_relative_dir('src/gallium')
  set_relative_dir('src/gallium/auxiliary/pipe-loader')
  ########################################################################################################################
  ### Begin conversion from: src/gallium/auxiliary/pipe-loader/meson.build
  ########################################################################################################################
  files_pipe_loader=files('pipe_loader.c','pipe_loader.h','pipe_loader_priv.h','pipe_loader_sw.c','driinfo_gallium.h',)
  ...
  if with_gallium_etnaviv:
    renderonly_drivers_c_args += ['-DGALLIUM_ETNAVIV'] if isinstance(renderonly_drivers_c_args, list) else '-DGALLIUM_ETNAVIV'
  
  if with_gallium_lima:
    renderonly_drivers_c_args += ['-DGALLIUM_LIMA'] if isinstance(renderonly_drivers_c_args, list) else '-DGALLIUM_LIMA'
  
  if with_gallium_v3d:
    renderonly_drivers_c_args += ['-DGALLIUM_V3D'] if isinstance(renderonly_drivers_c_args, list) else '-DGALLIUM_V3D'
  
  if with_gallium_vc4:
    renderonly_drivers_c_args += ['-DGALLIUM_VC4'] if isinstance(renderonly_drivers_c_args, list) else '-DGALLIUM_VC4'
  
  if with_gallium_freedreno:
    renderonly_drivers_c_args += ['-DGALLIUM_FREEDRENO'] if isinstance(renderonly_drivers_c_args, list) else '-DGALLIUM_FREEDRENO'
  
  if with_gallium_panfrost:
    renderonly_drivers_c_args += ['-DGALLIUM_PANFROST'] if isinstance(renderonly_drivers_c_args, list) else '-DGALLIUM_PANFROST'
  
  if with_gallium_asahi:
    renderonly_drivers_c_args += ['-DGALLIUM_ASAHI'] if isinstance(renderonly_drivers_c_args, list) else '-DGALLIUM_ASAHI'
  
  if with_gallium_rocket:
    renderonly_drivers_c_args += ['-DGALLIUM_ROCKET'] if isinstance(renderonly_drivers_c_args, list) else '-DGALLIUM_ROCKET'
  
  
  libpipe_loader_static=static_library('pipe_loader_static',files_pipe_loader,include_directories=[inc_util,inc_loader,inc_gallium,inc_include,inc_src,inc_gallium_aux,inc_gallium_winsys,inc_gallium_drivers,],c_args=[libpipe_loader_defines,renderonly_drivers_c_args],gnu_symbol_visibility='hidden',link_with=[libpipe_loader_links],dependencies=[dep_libdrm,idep_xmlconfig,idep_mesautil],build_by_default=False,)
  
  ########################################################################################################################
  ### End conversion from: src/gallium/auxiliary/pipe-loader/meson.build
  ########################################################################################################################
  set_relative_dir('src/gallium')
  ...
  if with_gallium_drisw_kms:
    set_relative_dir('src/gallium/winsys/sw/kms-dri')
    ########################################################################################################################
    ### Begin conversion from: src/gallium/winsys/sw/kms-dri/meson.build
    ########################################################################################################################
    libswkmsdri=static_library('swkmsdri',files('kms_dri_sw_winsys.c','kms_dri_sw_winsys.h'),gnu_symbol_visibility='hidden',include_directories=[inc_gallium,inc_include,inc_src,inc_gallium_aux],dependencies=[dep_libdrm,idep_mesautil],)
    
    ########################################################################################################################
    ### End conversion from: src/gallium/winsys/sw/kms-dri/meson.build
    ########################################################################################################################
    set_relative_dir('src/gallium')
  else:
    libswkmsdri=[]
  
  if False and with_platform_windows:
    set_relative_dir('src/gallium/winsys/sw/gdi')
    ########################################################################################################################
    ### Begin conversion from: src/gallium/winsys/sw/gdi/meson.build
    ########################################################################################################################
    libwsgdi=static_library('wsgdi','gdi_sw_winsys.c',include_directories=[inc_src,inc_include,inc_gallium,inc_gallium_aux,inc_gallium_drivers,inc_frontends],dependencies=idep_mesautil,)
  ...
  ########################################################################################################################
  set_relative_dir('src/gallium')
  if False and with_platform_haiku:
    noop()
  ...
  if with_gallium_softpipe:
    ...
    libsoftpipe=static_library('softpipe',files_softpipe,include_directories=[inc_gallium_aux,inc_gallium,inc_include,inc_src],c_args=[c_msvc_compat_args],gnu_symbol_visibility='hidden',dependencies=[idep_nir,idep_mesautil],)
    
    driver_softpipe=declare_dependency(compile_args='-DGALLIUM_SOFTPIPE',link_with=libsoftpipe)
  ...
  if with_any_llvmpipe:
    ...
    libllvmpipe=static_library('llvmpipe',[files_llvmpipe,sha1_h],c_args=[c_msvc_compat_args],cpp_args=[cpp_msvc_compat_args],gnu_symbol_visibility='hidden',include_directories=[inc_gallium,inc_gallium_aux,inc_include,inc_src],dependencies=[dep_llvm,idep_nir_headers,idep_mesautil,dep_libdrm],)
    
    driver_llvmpipe=declare_dependency(compile_args='-DGALLIUM_LLVMPIPE',link_with=libllvmpipe,dependencies=[dep_llvm,idep_mesautil],)
    
    if with_tests:
      for t in ['lp_test_format','lp_test_arit','lp_test_blend','lp_test_conv','lp_test_printf','lp_test_lookup_multiple']:
  ...
  if with_any_llvmpipe and with_gallium_softpipe:
    driver_swrast=declare_dependency(dependencies=[driver_softpipe,driver_llvmpipe],)
  ...
  if with_gallium_asahi:
    ...
    libasahi=static_library('asahi',[files_asahi],include_directories=[inc_gallium_aux,inc_gallium,inc_include,inc_src,inc_asahi,inc_virtio_gpu,inc_virtio_vdrm],c_args=[c_msvc_compat_args,'-Wno-c2x-extensions'],gnu_symbol_visibility='hidden',dependencies=[idep_nir,idep_mesautil,idep_agx_pack,dep_libdrm,idep_libagx,idep_libagx_shaders_h],)
  ...
  if with_gallium_r300 or with_gallium_radeonsi or with_gallium_r600:
    ...
    ########################################################################################################################
    libradeonwinsys=static_library('radeonwinsys',files('radeon_drm_bo.c','radeon_drm_bo.h','radeon_drm_cs.c','radeon_drm_cs.h','radeon_drm_surface.c','radeon_drm_winsys.c','radeon_drm_winsys.h','radeon_surface.c','radeon_surface.h'),include_directories=[inc_src,inc_include,inc_gallium,inc_gallium_aux],gnu_symbol_visibility='hidden',dependencies=[idep_mesautil,dep_libdrm],)
  ...
  if with_gallium_r300:
    ...
    libr300=static_library('r300',files_r300,r300_nir_algebraic_c,include_directories=[inc_src,inc_include,inc_gallium,inc_gallium_aux,inc_gallium_drivers,inc_mesa,],gnu_symbol_visibility='hidden',dependencies=[dep_llvm,idep_mesautil,idep_nir,idep_xmlconfig],)
  ...
  if with_gallium_r600:
    ...
    if with_tests:
      ...
      ########################################################################################################################
      r600_test_lib=static_library('r600_test','sfn_test_shaders.cpp',cpp_args=r600_cpp_args,include_directories=[inc_src,inc_mapi,inc_mesa,inc_include,inc_gallium,inc_gallium_aux,inc_amd_common,inc_gallium_drivers,],dependencies=[idep_gtest,dep_thread,dep_llvm,idep_nir,idep_nir_headers,idep_mesautil],)
      
      r600_test_dep=declare_dependency(include_directories=[inc_src,inc_mapi,inc_mesa,inc_include,inc_gallium,inc_gallium_aux,inc_amd_common,inc_gallium_drivers,],link_with=[libr600,libmesa,libgalliumvl,libgallium,libradeonwinsys,r600_test_lib],dependencies=[idep_gtest,dep_thread,dep_llvm,idep_nir,idep_nir_headers,idep_mesautil])
      
      
      
      if with_tests:
        for t in ['valuefactory','value','instr','instrfromstring','liverange','optimizer','shaderfromstring','split_address_loads']:
          test(t,executable('test-{}-r600-sfn'.format(t),['sfn_{}_test.cpp'.format(t)],dependencies=[r600_test_dep,idep_mesautil],),suite=['r600'])
  ...
  if with_gallium_radeonsi:
    ...
    c_args=[]
    if with_amdgpu_virtio:
      c_args += ['-DHAVE_AMDGPU_VIRTIO'] if isinstance(c_args, list) else '-DHAVE_AMDGPU_VIRTIO'
    
    
    libamdgpuwinsys=static_library('amdgpuwinsys',files('amdgpu_bo.c','amdgpu_bo.h','amdgpu_cs.cpp','amdgpu_cs.h','amdgpu_surface.c','amdgpu_userq.c','amdgpu_userq.h','amdgpu_winsys.c','amdgpu_winsys.h',),include_directories=[inc_amd,inc_gallium,inc_gallium_aux,inc_include,inc_src,inc_amd_common,inc_amd_common_llvm,inc_virtio_gpu],gnu_symbol_visibility='hidden',link_with=libamdgpu_addrlib,dependencies=[dep_llvm,dep_libdrm_amdgpu,idep_amdgfxregs_h,idep_mesautil],c_args=c_args)
    ...
    files_libradeonsi=files('driinfo_radeonsi.h','gfx11_query.c','si_barrier.c','si_blit.c','si_buffer.c','si_build_pm4.h','si_clear.c','si_compute.c','si_compute_blit.c','si_cp_dma.c','si_cp_reg_shadowing.c','si_cp_utils.c','si_debug.c','si_descriptors.c','si_fence.c','si_get.c','si_gfx_cs.c','si_gpu_load.c','si_perfcounter.c','si_pipe.c','si_pipe.h','si_pm4.c','si_pm4.h','si_public.h','si_query.c','si_query.h','si_nir_clamp_shadow_comparison_value.c','si_nir_kill_outputs.c','si_nir_lower_abi.c','si_nir_lower_color_inputs_to_sysvals.c','si_nir_lower_polygon_stipple.c','si_nir_lower_ps_color_inputs.c','si_nir_lower_resource.c','si_nir_lower_vs_inputs.c','si_nir_mark_divergent_texture_non_uniform.c','si_nir_optim.c','si_sdma_copy_image.c','si_shader.c','si_shader.h','si_shader_aco.c','si_shader_args.c','si_shader_info.c','si_shader_info.h','si_shader_internal.h','si_shader_nir.c','si_shader_variant_info.c','si_shaderlib_nir.c','si_sqtt.c','si_state.c','si_state.h','si_state_binning.c','si_state_msaa.c','si_state_shaders.cpp','si_state_streamout.c','si_state_viewport.c','si_test_blit_perf.c','si_test_dma_perf.c','si_test_image_copy_region.c','si_texture.c','si_utrace.c','si_utrace.h','si_uvd.c','si_vpe.c','si_vpe.h','pspdecryptionparam.h','cencdecryptionparam.h','radeon_uvd.c','radeon_uvd.h','radeon_uvd_enc.c','radeon_uvd_enc.h','radeon_vce.c','radeon_vce.h','radeon_vcn.h','radeon_vcn.c','radeon_vcn_dec.c','radeon_vcn_dec.h','radeon_vcn_dec_jpeg.c','radeon_vcn_enc.c','radeon_vcn_enc.h','radeon_vcn_enc_1_2.c','radeon_vcn_enc_2_0.c','radeon_vcn_enc_3_0.c','radeon_vcn_enc_4_0.c','radeon_vcn_enc_5_0.c','radeon_video.c','radeon_video.h','radeon_bitstream.h','radeon_bitstream.c',)
    
    radeonsi_include_dirs=[inc_src,inc_include,inc_gallium,inc_gallium_aux,inc_amd_common,inc_amd_common_llvm,inc_gallium_drivers,inc_compiler,inc_amd,inc_amd_vpe,inc_virtio_gpu]
    ...
    if radeonsi_build_id!='':
      libradeonsi_cflags += ['-DRADEONSI_BUILD_ID_OVERRIDE="'+radeonsi_build_id+'"'] if isinstance(libradeonsi_cflags, list) else '-DRADEONSI_BUILD_ID_OVERRIDE="'+radeonsi_build_id+'"'
    
    
    libradeonsi=static_library('radeonsi',[files_libradeonsi,sid_tables_h],c_args=libradeonsi_cflags,include_directories=radeonsi_include_dirs,gnu_symbol_visibility='hidden',dependencies=radeonsi_deps,)
    
    driver_radeonsi=declare_dependency(compile_args=['-DGALLIUM_RADEONSI']+libradeonsi_cflags,link_with=radeonsi_gfx_libs+[libradeonsi,libradeonwinsys,libamdgpuwinsys,libamd_common,libamd_common_llvm,libvpe,libgm,liblanczos],dependencies=idep_nir,)
  ...
  if with_gallium_nouveau:
    ...
    ########################################################################################################################
    set_relative_dir('src/gallium/drivers/nouveau/codegen')
    ########################################################################################################################
    ### Begin conversion from: src/gallium/drivers/nouveau/codegen/meson.build
    ########################################################################################################################
    ...
    ########################################################################################################################
    ### End conversion from: src/gallium/drivers/nouveau/codegen/meson.build
    ########################################################################################################################
    set_relative_dir('src/gallium/drivers/nouveau')
    ...
    libnouveau=static_library('nouveau',[files_libnouveau,sha1_h],include_directories=[inc_src,inc_include,inc_gallium,inc_gallium_aux,inc_mapi,inc_mesa,],gnu_symbol_visibility='hidden',link_with=[libnouveauwinsys],dependencies=[dep_libdrm,idep_libnouveauwinsys,idep_mesautil,idep_nir_headers,idep_nouveau_codegen,idep_nvidia_headers,],)
  ...
  if with_gallium_freedreno:
    ...
    freedreno_c_args+=cc.get_supported_arguments(['-Wno-array-bounds','-Wno-packed-bitfield-compat',])
    
    freedreno_cpp_args=[]
    freedreno_cpp_args+=cpp.get_supported_arguments(['-fno-exceptions','-fno-rtti','-Wno-address-of-packed-member','-Wno-address-of-temporary','-Wno-array-bounds','-Wno-c++11-narrowing','-Wno-c99-designator','-Wno-missing-field-initializers','-Wno-narrowing','-Wno-overflow','-Wno-packed-bitfield-compat','-Wno-reorder','-Wno-reorder-init-list','-Wno-sign-compare','-Wno-vla-cxx-extension',])
  ...
  if with_gallium_vc4:
    ...
    vc4_deps=[dep_simpenrose,dep_libdrm,dep_valgrind,idep_nir_headers,idep_mesautil,]
    
    libvc4_neon=[]
    if host_machine.cpu_family()=='arm':
      libvc4_neon=static_library('vc4_neon','vc4_tiling_lt.c',include_directories=[inc_src,inc_include,inc_gallium,inc_gallium_aux,inc_broadcom],c_args=['-mfpu=neon','-DV3D_BUILD_NEON'],dependencies=vc4_deps,)
    ...
    libvc4=static_library('vc4',[files_libvc4,v3d_xml_pack],include_directories=[inc_src,inc_include,inc_gallium,inc_gallium_aux,inc_broadcom,inc_gallium_drivers,],link_with=libvc4_neon,c_args=[vc4_c_args],gnu_symbol_visibility='hidden',dependencies=vc4_deps,build_by_default=False,)
  ...
  if with_gallium_panfrost:
    ...
    ########################################################################################################################
    files_panfrost=files('driinfo_panfrost.h','pan_mod_conv_cso.c','pan_bo.c','pan_device.c','pan_disk_cache.c','pan_fence.c','pan_helpers.c','pan_public.h','pan_screen.c','pan_screen.h','pan_resource.c','pan_resource.h','pan_context.c','pan_blend_cso.c','pan_blit.c','pan_job.c','pan_shader.c','pan_mempool.c','pan_mempool.h','pan_nir_remove_fragcolor_stores.c','pan_nir_lower_sysvals.c','pan_nir_lower_res_indices.c',)
    ...
    for ver in panfrost_versions:
      files_panfrost_vx=['pan_cmdstream.c','pan_blend_cso.c','pan_fb_preload.c','pan_precomp.c',pan_packers,]
      if ver in ['4','5','6','7','9']:
        files_panfrost_vx+=['pan_jm.c']
      elif ver in ['10','12','13']:
        files_panfrost_vx+=['pan_csf.c']
    
      libpanfrost_versions+=static_library('panfrost-v'+ver,files_panfrost_vx,include_directories=panfrost_includes,c_args=['-DPAN_ARCH='+ver],gnu_symbol_visibility='hidden',dependencies=[idep_pan_packers,idep_nir,dep_libdrm,idep_mesautil,idep_libpan_per_arch[ver]],)
  ...
  if with_gallium_etnaviv:
    ...
    ########################################################################################################################
    libetnavivdrm=static_library('etnavivdrm','etnaviv_drm_winsys.c',include_directories=[inc_include,inc_src,inc_gallium,inc_gallium_aux,inc_gallium_drivers,inc_etnaviv,],link_with=libetnaviv_drm,dependencies=[dep_libdrm,idep_nir_headers,idep_mesautil,idep_etna_common,idep_mesautil],)
    ...
    libetnaviv=static_library('etnaviv',files_etnaviv,etnaviv_enums_h,gnu_symbol_visibility='hidden',include_directories=[inc_include,inc_src,inc_gallium,inc_gallium_aux,inc_etnaviv,],link_with=[libetnaviv_drm,libetnaviv_encode],dependencies=[dep_libdrm,idep_nir_headers,idep_mesautil,idep_libetnaviv_decode,idep_etna_common],)
    
    driver_etnaviv=declare_dependency(compile_args='-DGALLIUM_ETNAVIV',link_with=[libetnaviv,libetnavivdrm],dependencies=idep_nir,)
    
    if with_tests:
      test('lower_ubo',executable('nir_lower_ubo_test',files('tests/lower_ubo_tests.cpp'),cpp_args=[cpp_msvc_compat_args],gnu_symbol_visibility='hidden',include_directories=[inc_include,inc_src,inc_gallium],link_with=[libetnaviv],dependencies=[idep_gtest,idep_nir,idep_mesautil],),suite=['compiler','etnaviv'],protocol='gtest',)
  ...
  if with_gallium_v3d:
    ...
    ########################################################################################################################
    files_libv3d=files('v3d_blit.c','v3d_bufmgr.c','v3d_bufmgr.h','v3d_cl.c','v3d_cl.h','v3d_context.c','v3d_context.h','v3d_disk_cache.c','v3d_fence.c','v3d_formats.c','v3d_job.c','v3d_program.c','v3d_query.c','v3d_query.h','v3d_query_perfcnt.c','v3d_query_pipe.c','v3d_resource.c','v3d_resource.h','v3d_screen.c','v3d_screen.h','v3d_uniforms.c',)
    
    files_per_version=files('v3dx_draw.c','v3dx_emit.c','v3dx_format_table.c','v3dx_job.c','v3dx_rcl.c','v3dx_state.c','v3dx_tfu.c',)
    ...
    per_version_libs=[]
    for ver in v3d_versions:
      per_version_libs+=static_library('v3d-v'+ver,[files_per_version,v3d_xml_pack],include_directories=[inc_src,inc_include,inc_gallium,inc_gallium_aux,inc_broadcom,inc_gallium_drivers,],c_args=[v3d_args,'-DV3D_VERSION='+ver],gnu_symbol_visibility='hidden',dependencies=[dep_v3d_hw,dep_libdrm,dep_valgrind,idep_nir_headers,idep_mesautil],)
    
    
    
    libv3d=static_library('v3d',[files_libv3d,v3d_xml_pack,],include_directories=[inc_src,inc_include,inc_gallium,inc_gallium_aux,inc_broadcom,inc_gallium_drivers,],c_args=[v3d_args],cpp_args=[v3d_args],gnu_symbol_visibility='hidden',dependencies=[dep_v3d_hw,dep_libdrm,dep_valgrind,idep_nir_headers,idep_mesautil,],link_with=[per_version_libs],)
    
    driver_v3d=declare_dependency(compile_args='-DGALLIUM_V3D',link_with=[libv3d,libv3dwinsys,libbroadcom_cle,libbroadcom_v3d],dependencies=[idep_nir,idep_broadcom_perfcntrs],)
  ...
  if with_gallium_crocus:
    ...
    ########################################################################################################################
    libcrocuswinsys=static_library('crocuswinsys',files('crocus_drm_winsys.c'),include_directories=[inc_src,inc_include,inc_gallium,inc_gallium_aux,inc_gallium_drivers,],gnu_symbol_visibility='hidden',dependencies=[dep_valgrind,idep_intel_dev,idep_mesautil],)
    ...
    for v in ['40','45','50','60','70','75','80']:
      crocus_per_hw_ver_libs+=static_library('crocus_per_hw_ver{}'.format(v),['crocus_blorp.c','crocus_query.c','crocus_state.c','crocus_blt.c',gen_xml_pack],include_directories=[inc_include,inc_src,inc_mapi,inc_mesa,inc_gallium,inc_gallium_aux,inc_intel],c_args=[no_override_init_args,sse2_args,'-DGFX_VERx10={}'.format(v),],gnu_symbol_visibility='hidden',dependencies=[dep_libdrm,dep_valgrind,idep_mesautil,idep_genxml,idep_nir_headers,idep_intel_dev,idep_intel_blorp_elk],)
    
    
    libcrocus=static_library('crocus',[files_libcrocus,gen_xml_pack],include_directories=[inc_src,inc_include,inc_gallium,inc_gallium_aux,inc_intel,inc_gallium_drivers,inc_mesa,inc_mapi],c_args=[sse2_args],cpp_args=[sse2_args],gnu_symbol_visibility='hidden',dependencies=[dep_libdrm,dep_valgrind,idep_genxml,idep_mesautil,idep_libintel_common,idep_nir_headers,idep_intel_dev,idep_intel_blorp_elk,idep_intel_decoder_elk,idep_intel_compiler_elk,],link_with=[crocus_per_hw_ver_libs,libisl,libintel_perf],)
  ...
  if with_gallium_iris:
    ...
    ########################################################################################################################
    libiriswinsys=static_library('iriswinsys',files('iris_drm_winsys.c'),include_directories=[inc_src,inc_include,inc_gallium,inc_gallium_aux,inc_gallium_drivers,],gnu_symbol_visibility='hidden',dependencies=idep_mesautil,)
    ...
    for v in ['80','90','110','120','125','200','300']:
      if v=='80' and not with_intel_elk:
        continue
    
    
      iris_per_hw_ver_libs+=static_library('iris_per_hw_ver{}'.format(v),['iris_blorp.c','iris_query.c','iris_state.c','iris_indirect_gen.c',gen_xml_pack],include_directories=[inc_include,inc_src,inc_mapi,inc_mesa,inc_gallium,inc_gallium_aux,inc_intel],c_args=[intel_common_args,no_override_init_args,sse2_args,'-DGFX_VERx10={}'.format(v),],gnu_symbol_visibility='hidden',dependencies=[dep_libdrm,dep_valgrind,idep_genxml,idep_nir_headers,idep_intel_driver_ds_headers,idep_intel_shaders,idep_intel_blorp,idep_mesautil],)
    
    
    libiris=static_library('iris',[files_libiris,gen_xml_pack],include_directories=[inc_src,inc_include,inc_gallium,inc_gallium_aux,inc_intel,inc_gallium_drivers,inc_mesa,inc_mapi],c_args=[intel_common_args,sse2_args],cpp_args=[intel_common_args,sse2_args],gnu_symbol_visibility='hidden',dependencies=[dep_libdrm,dep_valgrind,idep_genxml,idep_nir_headers,idep_libintel_common,idep_intel_driver_ds,idep_intel_dev,idep_intel_blorp_elk,idep_intel_decoder_elk,idep_intel_blorp,idep_intel_decoder_brw,idep_intel_compiler_elk,idep_intel_compiler_brw,idep_mesautil],link_with=[iris_per_hw_ver_libs,libisl,libintel_perf],)
  ...
  if with_gallium_i915:
    ...
    ########################################################################################################################
    libi915drm=static_library('i915drm',files('i915_drm_batchbuffer.c','i915_drm_buffer.c','i915_drm_fence.c','i915_drm_winsys.c'),include_directories=[inc_include,inc_src,inc_gallium,inc_gallium_aux,inc_gallium_drivers],link_with=[libintel_common],dependencies=[dep_libdrm,dep_libdrm_intel,idep_intel_dev_wa,idep_mesautil],)
    ...
    libi915=static_library('i915',files_i915,gnu_symbol_visibility='hidden',include_directories=[inc_include,inc_src,inc_gallium,inc_gallium_aux],dependencies=[idep_nir,idep_mesautil],)
  ...
  if with_gallium_virgl:
    ...
    ########################################################################################################################
    libvirglcommon=static_library('virglcommon','virgl_resource_cache.c',include_directories=[inc_include,inc_src,inc_mapi,inc_mesa,inc_gallium,inc_gallium_aux,inc_gallium_drivers],gnu_symbol_visibility='hidden',dependencies=idep_mesautil,)
  ...
  if with_gallium_lima:
    ...
    set_relative_dir('src/gallium/drivers/lima/genxml')
    ########################################################################################################################
    ### Begin conversion from: src/gallium/drivers/lima/genxml/meson.build
    ########################################################################################################################
    lima_pack=custom_target('lima_pack.h',input=['gen_pack.py','common.xml','lima_pack_header.h'],output='lima_pack.h',command=[prog_python,'@INPUT@'],capture=True,)
    
    idep_lima_pack=declare_dependency(sources=[lima_pack],dependencies=dep_valgrind,include_directories=include_directories('.'),)
    
    
    
    ########################################################################################################################
    ### End conversion from: src/gallium/drivers/lima/genxml/meson.build
    ########################################################################################################################
    set_relative_dir('src/gallium/drivers/lima')
    
    liblima=static_library('lima',files_lima,lima_nir_algebraic_c,include_directories=[inc_src,inc_include,inc_gallium,inc_gallium_aux,inc_gallium_drivers,inc_panfrost],dependencies=[dep_libdrm,idep_nir_headers,idep_mesautil,idep_lima_pack],)
    ...
    if with_tools.count('drm-shim') > 0:
      set_relative_dir('src/gallium/drivers/lima/drm-shim')
      ########################################################################################################################
      ### Begin conversion from: src/gallium/drivers/lima/drm-shim/meson.build
      ########################################################################################################################
      liblima_noop_drm_shim=shared_library('lima_noop_drm_shim','lima_noop.c',include_directories=[inc_include,inc_src],dependencies=[dep_drm_shim,idep_mesautil],gnu_symbol_visibility='hidden',install=True,)
      
      ########################################################################################################################
      ### End conversion from: src/gallium/drivers/lima/drm-shim/meson.build
      ########################################################################################################################
      set_relative_dir('src/gallium/drivers/lima')
  ...
  if with_gallium_rocket:
    set_relative_dir('src/gallium/winsys/rocket/drm')
    ########################################################################################################################
    ### Begin conversion from: src/gallium/winsys/rocket/drm/meson.build
    ########################################################################################################################
    librocketwinsys=static_library('rocketwinsys',files('rkt_drm_winsys.c'),include_directories=[inc_src,inc_include,inc_gallium,inc_gallium_aux,inc_gallium_drivers,],gnu_symbol_visibility='hidden',dependencies=[idep_mesautil],)
    
    ########################################################################################################################
    ### End conversion from: src/gallium/winsys/rocket/drm/meson.build
    ########################################################################################################################
    set_relative_dir('src/gallium')
    set_relative_dir('src/gallium/drivers/rocket')
    ########################################################################################################################
    ### Begin conversion from: src/gallium/drivers/rocket/meson.build
    ########################################################################################################################
    rocket_registers=custom_target('rkt_registers.h',input=['gen_header.py','registers.xml'],output='rkt_registers.h',command=[prog_python,'@INPUT0@','--rnn','.','--xml','@INPUT1@','c-defines'],capture=True,)
    
    files_rocket=files('rkt_coefs.c','rkt_device.c','rkt_ml.c','rkt_regcmd.c','rkt_task.c',)
    
    librocket=static_library('rocket',[files_rocket,rocket_registers],include_directories=[inc_gallium_aux,inc_gallium,inc_include,inc_src],gnu_symbol_visibility='hidden',dependencies=[idep_mesautil,dep_libdrm],)
    
    driver_rocket=declare_dependency(compile_args='-DGALLIUM_ROCKET',link_with=[librocketwinsys,librocket])
    
    shared_library('intercept',[files('intercept.c'),rocket_registers],include_directories=[inc_include],dependencies=[dep_libdrm],c_args=['-Wno-error=missing-prototypes','-g','-O0'])
    
    ########################################################################################################################
    ### End conversion from: src/gallium/drivers/rocket/meson.build
    ########################################################################################################################
    set_relative_dir('src/gallium')
  else:
  ...
  if False and with_gallium_d3d12:
    noop()
  else:
  ...
  if with_gallium_rusticl:
    ...
    libmesa_rust_files=files('mesa/lib.rs','mesa/compiler.rs','mesa/compiler/clc.rs','mesa/compiler/clc/spirv.rs','mesa/compiler/nir.rs','mesa/pipe.rs','mesa/pipe/context.rs','mesa/pipe/device.rs','mesa/pipe/fence.rs','mesa/pipe/screen.rs','mesa/pipe/transfer.rs','mesa/util.rs','mesa/util/disk_cache.rs','mesa/util/vm.rs',)
    ...
    for driver in rusticl_drivers_enable:
      rusticl_args+=['--cfg','rusticl_enable_'+driver,]
    ...
    rusticl_bindgen_args=[bindgen_output_args,'--no-convert-floats','--default-enum-style','rust','--with-derive-partialeq','--with-derive-eq','--with-derive-partialord','--with-derive-ord','--with-derive-hash','--with-derive-default','--anon-fields-prefix','anon_',]
    ...
    rusticl_opencl_bindings_rs=rust.bindgen(input=['rusticl_opencl_bindings.h',opencl_headers,],output='rusticl_opencl_bindings.rs',include_directories=[inc_include,],dependencies=[dep_x11,],c_args=[rusticl_bindgen_c_args,pre_args,cl_c_args,],args=[rusticl_bindgen_args,'--disable-header-comment','--ignore-functions','--raw-line','unsafe impl std::marker::Sync for _cl_icd_dispatch {}','--raw-line','unsafe impl std::marker::Send for _cl_image_desc {}','--raw-line','unsafe impl std::marker::Sync for _cl_image_desc {}','--allowlist-type','cl_.*','--blocklist-type','(__)?cl_.*[2348(16)]','--allowlist-type','cl.*_fn','--allowlist-var','CL_.*','--allowlist-var','GL_.*','--allowlist-var','MESA_GLINTEROP_.*','--allowlist-type','PFNEGLGETPROCADDRESSPROC','--allowlist-type','PFNGLXGETPROCADDRESSPROC','--allowlist-type','PFNMESAGLINTEROP.*','--new-type-alias-deref','cl_(mem|image|pipe|gl_texture)_info','--new-type-alias-deref','cl_kernel_(arg|work_group)_info','--new-type-alias-deref','cl_(event|profiling)_info','--constified-enum','MESA_GLINTEROP_.*',],)
    
    rusticl_opencl_gen=static_library('rusticl_opencl_gen',rusticl_opencl_bindings_rs,gnu_symbol_visibility='hidden',rust_abi='rust',)
    
    rusticl_llvm_bindings_rs=rust.bindgen(input='rusticl_llvm_bindings.hpp',output='rusticl_llvm_bindings.rs',c_args=[rusticl_bindgen_c_args,pre_args,],dependencies=[dep_clang,dep_llvm,dep_llvmspirvlib,],args=[bindgen_output_args,'--generate','constructors,functions,types,vars','--opaque-type','.*','--allowlist-function','clang::getClangFullVersion','--allowlist-function','llvm::LLVMContext::LLVMContext','--allowlist-function','llvm::writeSpirv','--allowlist-var','LLVM_VERSION_.*'],)
    
    rusticl_llvm_gen=static_library('rusticl_llvm_gen',rusticl_llvm_bindings_rs,gnu_symbol_visibility='hidden',rust_abi='rust',)
    ...
    rusticl_mesa_bindings=rust.bindgen(input='rusticl_mesa_bindings.h',output='rusticl_mesa_bindings.rs',output_inline_wrapper='rusticl_mesa_bindings.c',include_directories=[inc_gallium,inc_gallium_aux,inc_include,inc_src,],dependencies=[_idep_mesa_bindings,idep_nir_headers,dep_valgrind,],c_args=[rusticl_bindgen_c_args,pre_args,],args=[rusticl_bindgen_args,'--use-array-pointers-in-arguments','--allowlist-function','blob_.*','--allowlist-function','disk_cache_.*','--allowlist-type','float_controls','--allowlist-function','mesa_.*','--allowlist-var','OS_.*','--allowlist-function','rz?alloc_.*','--allowlist-function','SHA1.*','--allowlist-var','SHA1_.*','--allowlist-function','u_.*','--allowlist-function','util_format_.*','--allowlist-function','util_vma_.*','--no-copy','util_vma_heap','--allowlist-type','cl_sampler_.*_mode','--constified-enum-module','cl_sampler_.*_mode','--allowlist-function','clc_.*','--allowlist-type','clc_kernel_arg_access_qualifier','--bitfield-enum','clc_kernel_arg_access_qualifier','--allowlist-type','clc_kernel_arg_type_qualifier','--bitfield-enum','clc_kernel_arg_type_qualifier','--allowlist-type','gl_access_qualifier','--bitfield-enum','gl_access_qualifier','--allowlist-function','glsl_.*','--allowlist-function','nir_.*','--allowlist-var','nir_debug','--allowlist-var','NIR_DEBUG_.*','--bitfield-enum','nir_lower_int64_options','--bitfield-enum','nir_opt_if_options','--bitfield-enum','nir_variable_mode','--allowlist-function','should_.*_nir','--allowlist-function','spirv_.*','--allowlist-function','pipe_.*','--allowlist-var','PIPE_.*','--allowlist-type','pipe_endian','--bitfield-enum','pipe_map_flags','--allowlist-type','pipe_query_type','--constified-enum-module','pipe_query_type','--allowlist-type','pipe_resource_usage','--bitfield-enum','pipe_resource_usage','--allowlist-type','pipe_tex_filter','--constified-enum-module','pipe_tex_filter','--allowlist-type','pipe_tex_wrap','--constified-enum-module','pipe_tex_wrap','--allowlist-type','pipe_swizzle','--constified-enum-module','pipe_swizzle','--allowlist-function','rusticl_.*','--allowlist-function','std(err|out)_ptr','--allowlist-function','mmap','--allowlist-function','munmap','--allowlist-var','MAP_.*','--allowlist-var','PROT_.*','--allowlist-var','WINSYS_HANDLE_TYPE_.*',],)
    ...
    libmesa_rust_gen=static_library('mesa_rust_gen',rusticl_mesa_bindings[0],gnu_symbol_visibility='hidden',link_with=[libgallium,],dependencies=[idep_mesaclc,],rust_abi='rust',)
    
    libc_rust_gen=static_library('libc_rust_gen',rusticl_libc_bindings_rs,gnu_symbol_visibility='hidden',rust_abi='rust',)
    ...
    librusticl=shared_library('RusticlOpenCL','rusticl_target.c',gnu_symbol_visibility='hidden',include_directories=[inc_include,inc_src,inc_gallium,inc_gallium_aux,inc_gallium_drivers,inc_gallium_winsys,],link_whole=librusticl,link_with=[libgalliumvl_stub,libpipe_loader_static,libswdri,libswkmsdri,libwsw,libws_null,],dependencies=[driver_asahi,driver_freedreno,driver_iris,driver_nouveau,driver_r600,driver_panfrost,driver_radeonsi,driver_swrast,driver_v3d,driver_zink,idep_nir,idep_vtn,idep_mesautil,],link_args=[ld_args_build_id,ld_args_gc_sections,ld_args_bsymbolic,rusticl_ld_args,],link_depends=rusticl_ld_depends,version='{}.0.0'.format(opencl_version),install=True,)
  ...
  if with_glx=='xlib':
    ...
    ########################################################################################################################
    libws_xlib=static_library('ws_xlib',files('xlib_sw_winsys.c'),gnu_symbol_visibility='hidden',include_directories=[inc_include,inc_src,inc_mapi,inc_mesa,inc_gallium,inc_gallium_aux],dependencies=[dep_x11,dep_xext,dep_xcb,idep_mesautil],)
    ...
    ########################################################################################################################
    libxlib=static_library('xlib',files('glx_api.c','glx_getproc.c','glx_usefont.c','xm_api.c','xm_st.c'),gnu_symbol_visibility='hidden',include_directories=[inc_include,inc_src,inc_gallium,inc_gallium_aux,inc_mapi,inc_mesa],dependencies=[dep_x11,dep_xext,dep_xcb,dep_glproto,idep_mesautil,idep_dispatch_h],)
    
    ########################################################################################################################
    ### End conversion from: src/gallium/frontends/glx/xlib/meson.build
    ########################################################################################################################
    set_relative_dir('src/gallium')
    set_relative_dir('src/gallium/targets/libgl-xlib')
    ########################################################################################################################
    ### Begin conversion from: src/gallium/targets/libgl-xlib/meson.build
    ########################################################################################################################
    ...
    libgl=shared_library('GL',files('xlib.c'),include_directories=[inc_include,inc_src,inc_mapi,inc_mesa,inc_gallium,inc_gallium_aux,inc_gallium_winsys,inc_gallium_drivers,include_directories('../../frontends/glx/xlib'),],gnu_symbol_visibility='hidden',link_args=[ld_args_bsymbolic,ld_args_gc_sections,gallium_xlib_ld_args],link_depends=gallium_xlib_link_depends,link_whole=[libxlib,libglapi_bridge],link_with=[libgalliumvl_stub,libws_xlib,libglapi,libgallium,libmesa],dependencies=[dep_x11,idep_mesautil,dep_thread,dep_clock,dep_unwind,driver_swrast,driver_virgl,driver_asahi],install=True,version='1.5.0',darwin_versions='4.0.0',)
    
    shared_glapi_lib=libgl
    
    ########################################################################################################################
    ### End conversion from: src/gallium/targets/libgl-xlib/meson.build
    ########################################################################################################################
    set_relative_dir('src/gallium')
  ...
  if with_gallium_va:
    ...
    if not with_dri:
      ...
      if host_machine.system()=='windows':
        ...
        if len(va_drivers)>0:
          meson.add_install_script(install_megadrivers,libva_gallium.full_path(),va_drivers_path,va_drivers,install_tag='runtime',)
  ...
  if with_gallium_vdpau:
    ...
    if not with_dri:
      ...
      libvdpau_gallium=shared_library('vdpau_gallium','vdpau_target.c',gnu_symbol_visibility='hidden',link_args=[vdpau_link_args,ld_args_gc_sections],include_directories=[inc_include,inc_src,inc_mapi,inc_mesa,inc_gallium,inc_gallium_aux,inc_util,inc_gallium_winsys,inc_gallium_drivers,],link_whole=[libvdpau_st],link_with=link_with_libvdpau_gallium,dependencies=[idep_mesautil,driver_r600,driver_radeonsi,driver_nouveau,driver_d3d12,driver_virgl,],link_depends=vdpau_link_depends,soversion='{}.{}.0'.format(VDPAU_MAJOR,VDPAU_MINOR),install=True,install_dir=vdpau_drivers_path,name_suffix=libname_suffix,)
      for d in [[with_gallium_r600,'r600'],[with_gallium_radeonsi,'radeonsi'],[with_gallium_nouveau,'nouveau'],[with_gallium_virgl,'virtio_gpu'],[with_gallium_d3d12_video,'d3d12']]:
        if d[0]:
      ...
      if len(vdpau_drivers)>0:
        meson.add_install_script(install_megadrivers,libvdpau_gallium.full_path(),vdpau_drivers_path,vdpau_drivers,install_tag='runtime',)
  ...
  if False and with_gallium_mediafoundation:
    noop()
  ...
  if with_dri:
    ...
    if with_platform_x11:
      ...
      if with_dri_platform=='drm':
        deps_for_libdri+=[dep_xcb_dri3,dep_xcb_present,dep_xcb_sync,dep_xshmfence,dep_xcb_xfixes]
    ...
    libdri=static_library('dri',[files_libdri,sha1_h],include_directories=[inc_include,inc_util,inc_mesa,inc_mapi,inc_src,inc_gallium,inc_gallium_aux,inc_util,inc_gallium_drivers,inc_loader,],c_args=[libdri_c_args],gnu_symbol_visibility='hidden',dependencies=[dep_libdrm,idep_mesautil,deps_for_libdri,],link_with=[libloader],)
    ...
    if get_option('unversion-libgallium') or with_platform_android:
      ...
      libgallium_name='gallium-{}'.format(meson.project_version())
    
    
    libgallium_dri=shared_library(libgallium_name,files('dri_target.c'),include_directories=[inc_include,inc_src,inc_mapi,inc_mesa,inc_gallium,inc_gallium_aux,inc_util,inc_gallium_drivers,inc_gallium_winsys,include_directories('../../frontends/dri'),],gnu_symbol_visibility='hidden',link_args=[ld_args_build_id,ld_args_gc_sections,gallium_dri_ld_args],link_depends=gallium_dri_link_depends,link_with=[libmesa,libgalliumvl,libgallium,libglapi,libpipe_loader_static,libws_null,libwsw,libswdri,libswkmsdri,gallium_dri_link_with],link_whole=[libdri,gallium_dri_link_whole],dependencies=[dep_libdrm,dep_llvm,dep_thread,idep_xmlconfig,idep_mesautil,driver_swrast,driver_r300,driver_r600,driver_radeonsi,driver_nouveau,driver_kmsro,driver_v3d,driver_vc4,driver_freedreno,driver_etnaviv,driver_tegra,driver_i915,driver_svga,driver_virgl,driver_panfrost,driver_iris,driver_lima,driver_zink,driver_d3d12,driver_asahi,driver_crocus,driver_rocket],install=True,name_suffix=libname_suffix,)
    ...
    if with_gallium_va or with_gallium_vdpau:
      ...
      if len(va_drivers)>0:
        meson.add_install_script(install_megadrivers,libgallium_dri.full_path(),va_drivers_path,va_drivers,'--megadriver-libdir',get_option('libdir'),install_tag='runtime',)
    
    
      if len(vdpau_drivers)>0:
        meson.add_install_script(install_megadrivers,libgallium_dri.full_path(),vdpau_drivers_path,vdpau_drivers,'--megadriver-libdir',get_option('libdir'),install_tag='runtime',)
  ...
  if False and with_gallium_st_d3d10umd:
    noop()
  ...
  if with_swrast_vk:
    ...
    liblvp_files=files('nir/lvp_nir_lower_exec_graph.c','nir/lvp_nir_lower_input_attachments.c','nir/lvp_nir_lower_pipeline_layout.c','nir/lvp_nir_lower_ray_queries.c','nir/lvp_nir_lower_sparse_residency.c','nir/lvp_nir_opt_robustness.c','nir/lvp_nir_ray_tracing.c','lvp_acceleration_structure.c','lvp_device.c','lvp_device_generated_commands.c','lvp_cmd_buffer.c','lvp_descriptor_set.c','lvp_execute.c','lvp_util.c','lvp_image.c','lvp_formats.c','lvp_pipe_sync.c','lvp_pipeline.c','lvp_pipeline_cache.c','lvp_query.c','lvp_ray_tracing_pipeline.c','lvp_wsi.c')
    
    if with_platform_android:
      liblvp_files += ['lvp_android.c'] if isinstance(liblvp_files, list) else 'lvp_android.c'
    ...
    ########################################################################################################################
    libvulkan_lvp=shared_library('vulkan_lvp',['lavapipe_target.c'],vs_module_defs=vulkan_api_def,include_directories=[inc_src,inc_util,inc_include,inc_gallium,inc_gallium_aux,inc_gallium_winsys,inc_gallium_drivers],link_whole=[liblavapipe_st],link_with=[libpipe_loader_static,libgallium,libwsw,libswdri,libws_null,libswkmsdri],gnu_symbol_visibility='hidden',link_args=[ld_args_bsymbolic,ld_args_gc_sections],override_options=['b_lundef={}'.format('false' if host_machine.system()=='darwin' else str(get_option('b_lundef')))],dependencies=[driver_llvmpipe,idep_mesautil],name_prefix='' if host_machine.system()=='windows' else [],install=True,)
  ...
  if with_teflon:
    ...
    ########################################################################################################################
    libteflon=shared_library('teflon',['teflon_target.c'],include_directories=[inc_src,inc_util,inc_include,inc_gallium,inc_gallium_aux,inc_gallium_winsys,inc_gallium_drivers],link_whole=[libteflon_st],link_with=[libpipe_loader_static,libws_null,libwsw,libswdri,libswkmsdri,libgallium],gnu_symbol_visibility='hidden',link_args=ld_args_build_id,dependencies=[driver_etnaviv,driver_rocket,idep_nir,idep_mesautil,],install=True,)
...
if with_gallium and (with_glx!='disabled' or with_egl):
  if with_gles1 and not with_glvnd:
    ...
    ########################################################################################################################
    es1_glapi_mapi_tmp_h=custom_target('es1_glapi_mapi_tmp.h',input=['../new/gen_gldispatch_mapi.py','../glapi/registry/gl.xml'],output='es1_glapi_mapi_tmp.h',command=[prog_python,'@INPUT0@','glesv1','@INPUT1@'],depend_files=glapi_xml_py_deps,capture=True,)
    ...
    libglesv1_cm=shared_library('GLESv1_CM'+get_option('gles-lib-suffix'),['libgles1_public.c',es1_glapi_mapi_tmp_h],vs_module_defs=gles1_def,c_args=[c_msvc_compat_args,_es1_c_args,gcc_lto_quirk,],gnu_symbol_visibility='hidden',link_args=[ld_args_gc_sections],include_directories=[inc_src,inc_include,inc_mapi],link_with=shared_glapi_lib,dependencies=[dep_thread,dep_libdrm,dep_m,dep_dl,idep_mesautilc11],soversion='' if host_machine.system()=='windows' else '1',version='1.1.0',darwin_versions='3.0.0',name_prefix='lib' if host_machine.system()=='windows' else [],install=True,)
  ...
  if with_gles2 and not with_glvnd:
    ...
    ########################################################################################################################
    es2_glapi_mapi_tmp_h=custom_target('es2_glapi_mapi_tmp.h',input=['../new/gen_gldispatch_mapi.py','../glapi/registry/gl.xml'],output='es2_glapi_mapi_tmp.h',command=[prog_python,'@INPUT0@','glesv2','@INPUT1@'],depend_files=glapi_xml_py_deps,capture=True,)
    ...
    libgles2=shared_library('GLESv2'+get_option('gles-lib-suffix'),['libgles2_public.c',es2_glapi_mapi_tmp_h],vs_module_defs=gles2_def,c_args=[c_msvc_compat_args,_es2_c_args,gcc_lto_quirk,],gnu_symbol_visibility='hidden',link_args=[ld_args_gc_sections],include_directories=[inc_src,inc_include,inc_mapi],link_with=shared_glapi_lib,dependencies=[dep_thread,dep_libdrm,dep_m,dep_dl,idep_mesautilc11],soversion='' if host_machine.system()=='windows' else '2',version='2.0.0',darwin_versions='3.0.0',name_prefix='lib' if host_machine.system()=='windows' else [],install=True,)
  ...
  if with_tests:
    ...
    ########################################################################################################################
    files_main_test=files('enum_strings.cpp','disable_windows_include.c','mesa_formats.cpp','mesa_extensions.cpp','program_state_string.cpp',)
    # disable_windows_include.c includes this generated header.
    files_main_test+=main_marshal_generated_h
    
    test('main-test',executable('main_test',[files_main_test,main_dispatch_h],include_directories=[inc_include,inc_src,inc_mapi,inc_mesa,inc_gallium,inc_gallium_aux],dependencies=[idep_gtest,dep_clock,dep_dl,dep_thread,idep_nir_headers,idep_mesautil],link_with=[libmesa,libgallium,libglapi],),suite=['mesa'],protocol='gtest',)
...
if with_glx=='dri':
  ...
  libappleglx=static_library('glxapple',[files_libappleglx,main_dispatch_h],include_directories=[inc_mesa,inc_glx,inc_src,inc_include,inc_glapi,inc_gallium,inc_loader],dependencies=[dep_xext,dep_xplugin],gnu_symbol_visibility='hidden',build_by_default=False,)
  ...
  if with_dri_platform=='windows':
    ...
    libwindowsdri=static_library('driwindows',[files_libwindowsdri,files_windowsdriproto],dependencies=dep_xext,build_by_default=False,)
  ...
  files_libglx=files('clientattrib.c','clientinfo.c','compsize.c','create_context.c','dri_common.c','dri_common.h','dri_common_query_renderer.c','drisw_glx.c','drisw_priv.h','eval.c','glxclient.h','glxcmds.c','glxconfig.c','glxconfig.h','glxcurrent.c','glx_error.c','glx_error.h','glxext.c','glxextensions.c','glxextensions.h','glxhash.c','glxhash.h','glx_pbuffer.c','glx_query.c','indirect_glx.c','indirect_init.h','indirect_texture_compression.c','indirect_transpose_matrix.c','indirect_vertex_array.c','indirect_vertex_array.h','indirect_vertex_array_priv.h','indirect_vertex_program.c','indirect_window_pos.c','packrender.h','packsingle.h','pixel.c','pixelstore.c','query_renderer.c','render2.c','renderpix.c','single2.c','vertarr.c','xfont.c',)
  ...
  libglx=static_library('glx',[files_libglx,glx_generated,main_dispatch_h],include_directories=[inc_include,inc_src,inc_glapi,inc_loader,inc_loader_x11,inc_gallium,inc_mesa,inc_st_dri,inc_gallium_aux],gnu_symbol_visibility='hidden',link_with=[libloader,libloader_x11,extra_libs_libglx,glx_gallium_link],dependencies=[idep_mesautil,idep_xmlconfig,dep_libdrm,dep_glproto,dep_x11,dep_xext,dep_glvnd,dep_xxf86vm,dep_xshmfence,],)
  
  # libglapi_bridge contains GL functions that we need to export from libGL.so.
  # Other than that, we don't need it.
  libgl_link=[] if with_glvnd else [libglapi_bridge]
  
  libgl=shared_library(gl_lib_name,[],link_whole=[libglx,libgl_link],link_args=[ld_args_bsymbolic,ld_args_gc_sections,extra_ld_args_libgl],dependencies=[dep_libdrm,dep_dl,dep_m,dep_thread,dep_x11,dep_xcb_glx,dep_xcb,dep_x11_xcb,dep_xext,dep_xxf86vm,dep_xcb_shm,extra_deps_libgl,],version=gl_lib_version,darwin_versions='4.0.0',install=True,)
  
  if with_symbols_check:
    libgl_symbols_file='glvnd-symbols.txt' if with_glvnd else 'libgl-symbols.txt'
  
    test('libGL-ABI-check',symbols_check,args=['--lib',libgl,'--symbols-file',files(libgl_symbols_file),symbols_check_args,],suite=['glx'],)
...
if with_gbm:
  if get_option('libgbm-external'):
    ...
    libgbm=shared_library(libgbm_name,files_gbm,include_directories=incs_gbm,c_args=[args_gbm],link_args=[ld_args_gc_sections],link_with=[libloader],dependencies=[dep_libdrm,idep_xmlconfig],gnu_symbol_visibility='hidden',version='1.0.0',install=True,)
    ...
    dep_gbm=declare_dependency(link_with=libgbm,include_directories=inc_gbm,)
...
if with_egl:
  ...
  g_egldispatchstubs_c=custom_target('g_egldispatchstubs.c',input=['generate/gen_egl_dispatch.py','generate/egl.xml','generate/egl_other.xml'],output='g_egldispatchstubs.c',command=[prog_python,'@INPUT0@',gen_args,'source','@INPUT1@','@INPUT2@'],depend_files=['generate/eglFunctionList.py',glapi_xml_py_deps],capture=True,)
  
  g_egldispatchstubs_h=custom_target('g_egldispatchstubs.h',input=['generate/gen_egl_dispatch.py','generate/egl.xml','generate/egl_other.xml'],output='g_egldispatchstubs.h',command=[prog_python,'@INPUT0@',gen_args,'header','@INPUT1@','@INPUT2@'],depend_files=['generate/eglFunctionList.py',glapi_xml_py_deps],capture=True,)
  ...
  if with_dri:
    ...
    if with_platform_wayland:
      ...
      if with_wayland_bind_display:
        ...
        c_args_for_egl+=['-DHAVE_BIND_WL_DISPLAY']
  ...
  if with_ld_version_script:
    if with_glvnd:
      link_args_for_egl+=['-Wl,--version-script',join_paths(meson.current_source_dir(),'egl-glvnd.sym')]
      link_deps_for_egl+=files('egl-glvnd.sym')
    else:
      link_args_for_egl+=['-Wl,--version-script',join_paths(meson.current_source_dir(),'egl.sym')]
  ...
  if with_glvnd:
    configure_file(configuration={'library_path':'libEGL_{}.so.0'.format(glvnd_vendor_name)},input='main/50_mesa.json',output='50_{}.json'.format(glvnd_vendor_name),install=True,install_tag='runtime',install_dir=join_paths(get_option('datadir'),'glvnd','egl_vendor.d'))
  
    devenv_glvnd_egl_json_file=configure_file(configuration={'library_path':libegl.full_path()},input='main/50_mesa.json',output='devenv_glvnd_egl.json',)
...
if with_gallium and with_gbm:
  if with_glx=='dri' or with_platform_x11 or with_platform_xcb:
    set_relative_dir('src/gallium/targets/dril')
    ########################################################################################################################
    ### Begin conversion from: src/gallium/targets/dril/meson.build
    ########################################################################################################################
    dril_c_args=[]
    dril_ld_args=[]
    dril_link_depends=[]
    ...
    dril_dri=shared_library('dril_dri',files('dril_target.c'),include_directories=[inc_include,inc_src,inc_mapi,inc_mesa,inc_gallium,inc_gallium_aux,inc_util,inc_gallium_drivers,inc_gallium_winsys,],gnu_symbol_visibility='hidden',link_args=[ld_args_build_id,ld_args_gc_sections,dril_ld_args],link_depends=dril_link_depends,link_with=[libgallium,],dependencies=[idep_mesautil,dep_gbm,],install=True,install_dir=dri_drivers_path,name_suffix=libname_suffix,)
    
    for d in [[with_gallium_kmsro,['apple_dri.so','armada-drm_dri.so','exynos_dri.so','gm12u320_dri.so','hdlcd_dri.so','hx8357d_dri.so','ili9163_dri.so','ili9225_dri.so','ili9341_dri.so','ili9486_dri.so','imx-drm_dri.so','imx-dcss_dri.so','imx-lcdif_dri.so','ingenic-drm_dri.so','kirin_dri.so','komeda_dri.so','mali-dp_dri.so','mcde_dri.so','mediatek_dri.so','meson_dri.so','mi0283qt_dri.so','mxsfb-drm_dri.so','panel-mipi-dbi_dri.so','pl111_dri.so','rcar-du_dri.so','repaper_dri.so','rockchip_dri.so','rzg2l-du_dri.so','ssd130x_dri.so','st7586_dri.so','st7735r_dri.so','sti_dri.so','stm_dri.so','sun4i-drm_dri.so','udl_dri.so','vkms_dri.so','zynqmp-dpsub_dri.so',]],[with_gallium_radeonsi,'radeonsi_dri.so'],[with_gallium_nouveau,'nouveau_dri.so'],[with_gallium_freedreno,['msm_dri.so','kgsl_dri.so']],[with_gallium_swrast,'swrast_dri.so'],[with_gallium_swrast and with_gallium_drisw_kms,'kms_swrast_dri.so'],[with_gallium_v3d,'v3d_dri.so'],[with_gallium_vc4,'vc4_dri.so'],[with_gallium_panfrost,['panfrost_dri.so','panthor_dri.so']],[with_gallium_etnaviv,'etnaviv_dri.so'],[with_gallium_tegra,'tegra_dri.so'],[with_gallium_crocus,'crocus_dri.so'],[with_gallium_iris,'iris_dri.so'],[with_gallium_i915,'i915_dri.so'],[with_gallium_r300,'r300_dri.so'],[with_gallium_r600,'r600_dri.so'],[with_gallium_svga,'vmwgfx_dri.so'],[with_gallium_virgl or (with_gallium_freedreno and freedreno_kmds.count('virtio') > 0) or (with_gallium_radeonsi and with_amdgpu_virtio),'virtio_gpu_dri.so'],[with_gallium_lima,'lima_dri.so'],[with_gallium_d3d12,'d3d12_dri.so'],[with_gallium_zink,'zink_dri.so'],[with_gallium_asahi,'asahi_dri.so'],[with_gallium_rocket,'rocket_dri.so']]:
      if d[0]:
        dril_drivers+=d[1]
    ...
    if prog_ln.found():
      for d in dril_drivers:
        custom_target(d,output=d,command=[prog_ln,'-sf','libdril_dri.so','@OUTPUT@'],build_by_default=True,)
    
    
    
    if len(dril_drivers)>0:
      meson.add_install_script(install_megadrivers,dril_dri.full_path(),dri_drivers_path,dril_drivers,install_tag='runtime',)
    
    
    ########################################################################################################################
    ### End conversion from: src/gallium/targets/dril/meson.build
    ########################################################################################################################
    set_relative_dir('src')
...
if with_gbm and with_dri2:
  set_relative_dir('src/gbm/backends/dri')
  ########################################################################################################################
  ### Begin conversion from: src/gbm/backends/dri/meson.build
  ########################################################################################################################
  deps_gbm_dri=[]
  flags_gbm_dri=[]
  ...
  shared_library('dri_gbm',files('gbm_dri.c','gbm_driint.h'),include_directories=[inc_gallium,incs_gbm_dri,inc_loader,inc_st_dri,inc_gallium_aux],c_args=[flags_gbm_dri],link_args=[ld_args_gc_sections],link_with=[libloader,libgallium_dri],dependencies=[deps_gbm_dri,dep_dl,dep_gbm,dep_libdrm,idep_mesautil,idep_xmlconfig],gnu_symbol_visibility='hidden',install=True,install_dir=join_paths(get_option('libdir'),'gbm'),name_prefix='',)
  
  devenv.set('GBM_BACKENDS_PATH',meson.current_build_dir())
  
  ########################################################################################################################
  ### End conversion from: src/gbm/backends/dri/meson.build
  ########################################################################################################################
  set_relative_dir('src')
...
if with_perfetto and dep_libdrm.found():
  ...
  if with_platform_android:
    shared_library('gpudataproducer',sources=producer_sources,include_directories=[include_pps,inc_src],dependencies=[dep_pps,dep_perfetto],install=True)
...
if with_tools.count('dlclose-skip') > 0:
  set_relative_dir('src/tool/dlclose-skip')
  ########################################################################################################################
  ### Begin conversion from: src/tool/dlclose-skip/meson.build
  ########################################################################################################################
  shared_library('dlclose-skip','dlclose-skip.c',install=True,)
  
  ########################################################################################################################
  ### End conversion from: src/tool/dlclose-skip/meson.build
  ########################################################################################################################
  set_relative_dir('src/tool')
...
if sphinx.found():
  ...
  if get_option('werror'):
    sphinx_args += ['-W'] if isinstance(sphinx_args, list) else '-W'
  
  
  html_docs=custom_target('html',depfile='docs.d',depend_files=files('conf.py'),command=[sphinx,'-b','html','-q','-Ddepfile=@DEPFILE@',sphinx_args,meson.current_source_dir(),'@OUTPUT@'],output='html',build_by_default=get_option('html-docs').enabled(),)
...
summary({'OpenGL':with_opengl,'ES1':with_gles1,'ES2':with_gles2,'GLVND':with_glvnd,},section='OpenGL',bool_yn=True)
...
if with_gallium_mediafoundation:
  video_apis += ['mediafoundation'] if isinstance(video_apis, list) else 'mediafoundation'
...
if with_gallium:
  ...
  if with_gallium_mediafoundation:
    gallium_frontends += ['mediafoundation'] if isinstance(gallium_frontends, list) else 'mediafoundation'
...
meson_translator.generator.generate(meson_translator)
